---
url: "https://docs.gravityforms.com/how-to-populate-image-choices-with-woocommerce-products/"
title: "How To Populate Image Choices With WooCommerce Products - Gravity Forms Documentation"
---
...
- [Introduction](https://docs.gravityforms.com/how-to-populate-image-choices-with-woocommerce-products/#h-introduction)
- [Example One](https://docs.gravityforms.com/how-to-populate-image-choices-with-woocommerce-products/#h-example-one)
- [Example Two](https://docs.gravityforms.com/how-to-populate-image-choices-with-woocommerce-products/#h-example-two)
- [Example Three](https://docs.gravityforms.com/how-to-populate-image-choices-with-woocommerce-products/#h-example-three)
- [Resources](https://docs.gravityforms.com/how-to-populate-image-choices-with-woocommerce-products/#h-resources)
...
This article demonstrates using a custom snippet to retrieve WooCommerce products and populate a Gravity Forms [Image Choice](https://docs.gravityforms.com/image-choice-field/) field.
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>17<br>18<br>19<br>20<br>21<br>22<br>23<br>24<br>25<br>26<br>27<br>28<br>29<br>30<br>31<br>32<br>33<br>34<br>35<br>36<br>37<br>38<br>39<br>40<br>41<br>42<br>43<br>44<br>45<br>46 | `add_filter(``'gform_pre_render'``,``'populate_image_choices'``);`<br>`add_filter(``'gform_pre_validation'``,``'populate_image_choices'``);`<br>`add_filter(``'gform_admin_pre_render'``,``'populate_image_choices'``);`<br>`add_filter(``'gform_pre_submission_filter'``,``'populate_image_choices'``);`<br>`function``populate_image_choices(``$form``) {`<br>```if``( rgar(``$form``,``'id'``) != 123 ) {``// replace 123 with your form id`<br>```return``$form``;`<br>```}`<br>```$posts``= get_posts(``array``(`<br>```'post_type'``=>``'product'``,`<br>```'post_status'``=>``'publish'``,`<br>```'posts_per_page'``=> -1,`<br>```));`<br>```$items``=``array``();`<br>```foreach``(``$posts``as``$post``) {`<br>```$product``= wc_get_product(``$post``->ID );`<br>```if``( !``$product``) {`<br>```continue``;`<br>```}`<br>```$image_id``=``$product``->get_image_id();`<br>```$image_url``= wp_get_attachment_url(``$image_id``);`<br>```if``(``$image_url``) {`<br>```$items``[] =``array``(`<br>```'text'``=>``$product``->get_name(),`<br>```'value'``=>``$product``->get_name(),`<br>```'image'``=>``$image_url``,`<br>```'file_url'``=>``$image_url``,`<br>```'attachment_id'``=>``$image_id`<br>```);`<br>```}`<br>```}`<br>```foreach``(``$form``[``'fields'``]``as``&``$field``) {`<br>```if``(``$field``->id == 1 ) {``// Replace 1 with your image choice field id`<br>```$field``->choices =``$items``;`<br>```break``;`<br>```}`<br>```}`<br>```return``$form``;`<br>`}` |

[![Image showing Woocommerce products populated in an Image Choice field](https://docs.gravityforms.com/wp-content/uploads/2025/04/image-16-1024x858.png)](https://docs.gravityforms.com/wp-content/uploads/2025/04/image-16.png)
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>17<br>18<br>19<br>20<br>21<br>22<br>23<br>24<br>25<br>26<br>27<br>28<br>29<br>30<br>31<br>32<br>33<br>34<br>35<br>36<br>37<br>38<br>39<br>40<br>41<br>42<br>43<br>44<br>45<br>46<br>47<br>48<br>49<br>50 | `add_filter(``'gform_pre_render'``,``'populate_image_choices'``);`<br>`add_filter(``'gform_pre_validation'``,``'populate_image_choices'``);`<br>`add_filter(``'gform_admin_pre_render'``,``'populate_image_choices'``);`<br>`add_filter(``'gform_pre_submission_filter'``,``'populate_image_choices'``);`<br>`function``populate_image_choices(``$form``) {`<br>```if``( rgar(``$form``,``'id'``) != 535 ) {`<br>```return``$form``;`<br>```}`<br>```$posts``= get_posts(``array``(`<br>```'post_type'``=>``'product'``,`<br>```'post_status'``=>``'publish'``,`<br>```'posts_per_page'``=> 6,`<br>```));`<br>```$items``=``array``();`<br>```foreach``(``$posts``as``$post``) {`<br>```$product``= wc_get_product(``$post``->ID );`<br>```if``( !``$product``) {`<br>```continue``;`<br>```}`<br>```$image_id``=``$product``->get_image_id();`<br>```$image_url``= wp_get_attachment_url(``$image_id``);`<br>```if``(``$image_url``) {`<br>```// Append price to product name`<br>```$price``= wc_format_decimal(``$product``->get_price(), 2 );``// Ensures proper decimal formatting`<br>```$name_with_price``=``$product``->get_name() .``' ('``. wc_price(``$price``) .``')'``;`<br>```$items``[] =``array``(`<br>```'text'``=>``$name_with_price``,`<br>```'value'``=>``$product``->get_id(),``// Use product ID as value`<br>```'image'``=>``$image_url``,`<br>```'file_url'``=>``$image_url``,`<br>```'attachment_id'``=>``$image_id`<br>```);`<br>```}`<br>```}`<br>```foreach``(``$form``[``'fields'``]``as``&``$field``) {`<br>```if``(``$field``->id == 1 ) {`<br>```$field``->choices =``$items``;`<br>```break``;`<br>```}`<br>```}`<br>```return``$form``;`<br>`}` |

[![Image showing Woocommerce products populated in an Image Choice field with Price](https://docs.gravityforms.com/wp-content/uploads/2025/04/image-15-1024x858.png)](https://docs.gravityforms.com/wp-content/uploads/2025/04/image-15.png)
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>17 | `add_action(``'gform_after_submission_123'``,``function``(``$entry``) {`<br>```$product_id``= rgar(``$entry``,``'1'``);``// Ensure this matches your actual field ID`<br>```if``(!``$product_id``) {`<br>```return``;`<br>```}`<br>```$product``= wc_get_product(``$product_id``);`<br>```if``(``$product``) {`<br>```WC()->cart->empty_cart();``// Optional: Clear previous cart items`<br>```WC()->cart->add_to_cart(``$product_id``, 1);``// Add selected product`<br>```// Simulate WooCommerce Add to Cart action`<br>```do_action(``'woocommerce_add_to_cart'``,``$product_id``);`<br>```}`<br>`}, 10, 2);` |

You can redirect the user to the checkout page after submitting the form with a custom [Confirmation](https://docs.gravityforms.com/confirmation-type-page/) page.

[![Image showing custom confirmation page redirecting to WooCommerce checkout.](https://docs.gravityforms.com/wp-content/uploads/2025/04/image-18-1024x410.png)](https://docs.gravityforms.com/wp-content/uploads/2025/04/image-18.png)
...
[Algolia](https://docs.gravityforms.com/how-to-populate-image-choices-with-woocommerce-products/# "Algolia")
