---
url: "https://docs.gravityforms.com/gform_mailchimp_subscription/"
title: "gform_mailchimp_subscription - Gravity Forms Documentation"
---
...
# gform\_mailchimp\_subscription

- [Description](https://docs.gravityforms.com/gform_mailchimp_subscription/#h-description)
- [Usage](https://docs.gravityforms.com/gform_mailchimp_subscription/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_mailchimp_subscription/#h-parameters)
- [Examples](https://docs.gravityforms.com/gform_mailchimp_subscription/#h-examples)
- [If a subscriber already exists, set them as subscribed](https://docs.gravityforms.com/gform_mailchimp_subscription/#h-if-a-subscriber-already-exists-set-them-as-subscribed)
- [Set the signup timestamp](https://docs.gravityforms.com/gform_mailchimp_subscription/#h-set-the-signup-timestamp)
- [Set the Subscriber language](https://docs.gravityforms.com/gform_mailchimp_subscription/#h-set-the-subscriber-language)
- [Change email\_type to text from the default HTML](https://docs.gravityforms.com/gform_mailchimp_subscription/#h-change-email-type-to-text-from-the-default-html)
- [Set the skip\_merge\_validation argument](https://docs.gravityforms.com/gform_mailchimp_subscription/#h-set-the-skip-merge-validation-argument)
- [Since](https://docs.gravityforms.com/gform_mailchimp_subscription/#h-since)
- [Placement](https://docs.gravityforms.com/gform_mailchimp_subscription/#h-placement)
- [Source Code](https://docs.gravityforms.com/gform_mailchimp_subscription/#h-source-code)

## Description

The “gform\_mailchimp\_subscription” filter in Gravity Forms Mailchimp Add-On modifies the subscription object before it is executed.
...
- **$subscription** [array](https://docs.gravityforms.com/array)

Subscription arguments. See the [Mailchimp API documentation](https://developer.mailchimp.com/documentation/mailchimp/reference/lists/members/) for a full list of supported properties.
- **$list\_id** [string](https://docs.gravityforms.com/string)
...
The feed object.
- **$member** [array](https://docs.gravityforms.com/array)
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9 | `add_filter(``'gform_mailchimp_subscription'``,``'update_existing'``, 10, 6 );`<br>`function``update_existing(``$subscription``,``$list_id``,``$form``,``$entry``,``$feed``,``$member``) {`<br>```if``(``$member``) {`<br>```$subscription``[``'status'``] =``'subscribed'``;`<br>```}`<br>```return``$subscription``;`<br>`}` |

### Set the signup timestamp

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7 | `add_filter(``'gform_mailchimp_subscription'``,``function``(``$subscription``,``$list_id``,``$form``,``$entry``,``$feed``,``$member``) {`<br>```if``( !``$member``) {`<br>```$subscription``[``'timestamp_signup'``] =``date``( DATE_ATOM );`<br>```}`<br>```return``$subscription``;`<br>`}, 10, 6 );` |

### Set the Subscriber language

The following example uses the ICL\_LANGUAGE\_CODE constant provided by the WPML plugin to set the subscriber language.

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11 | `add_filter(``'gform_mailchimp_subscription'``,``'gf_set_subscriber_language'``);`<br>`function``gf_set_subscriber_language(``$subscription``) {`<br>```GFCommon::log_debug(``__METHOD__``.``'(): running.'``);`<br>```// Use the language code provided by WPML to set the subscriber language.`<br>```if``( defined(``'ICL_LANGUAGE_CODE'``) ) {`<br>```GFCommon::log_debug(``__METHOD__``.``'(): Setting value for ICL_LANGUAGE_CODE as subscriber language: '``. ICL_LANGUAGE_CODE );`<br>```$subscription``[``'language'``] = ICL_LANGUAGE_CODE;`<br>```}`<br>```return``$subscription``;`<br>`}` |

### Change email\_type to text from the default HTML

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5 | `add_filter(``'gform_mailchimp_subscription'``,``function``(``$subscription``,``$list_id``,``$form``,``$entry``,``$feed``,``$member``) {`<br>```// this will force all subscriptions to text format`<br>```$subscription``[``'email_type'``] =``'text'``;`<br>```return``$subscription``;`<br>`}, 10, 6 );` |

### Set the skip\_merge\_validation argument
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6 | `add_filter(``'gform_mailchimp_subscription'``,``function``(``$subscription``,``$list_id``,``$form``,``$entry``,``$feed``,``$member``) {`<br>```if``(``$member``) {`<br>```$subscription``[``'skip_merge_validation'``] = true;`<br>```}`<br>```return``$subscription``;`<br>`}, 10, 6 );` |
...
This filter is located in _includes/addon/class-gf-mailchimp.php_.
...
[Algolia](https://docs.gravityforms.com/gform_mailchimp_subscription/# "Algolia")
