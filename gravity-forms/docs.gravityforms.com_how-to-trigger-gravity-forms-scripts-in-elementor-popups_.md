---
url: "https://docs.gravityforms.com/how-to-trigger-gravity-forms-scripts-in-elementor-popups/"
title: "How to Trigger Gravity Forms Scripts in Elementor Popups - Gravity Forms Documentation"
---
...
- [Introduction](https://docs.gravityforms.com/how-to-trigger-gravity-forms-scripts-in-elementor-popups/#h-introduction)
- [Script To Parse the Popup Content](https://docs.gravityforms.com/how-to-trigger-gravity-forms-scripts-in-elementor-popups/#h-script-to-parse-the-popup-content)
- [Placement](https://docs.gravityforms.com/how-to-trigger-gravity-forms-scripts-in-elementor-popups/#h-placement)
...
|     |
| --- |
| `window.addEventListener(``'elementor/popup/show'``, ( event ) => {`<br>```if``( ! gform?.utils ) {`<br>```return``;`<br>```}`<br>```const form = gform.utils.getNode(``'.gform_wrapper form'``, event.detail.instance.$elements,``true``);`<br>```if``( ! form ) {`<br>```return``;`<br>```}`<br>```const formId = form.dataset.formid;`<br>```const currentPage = 1;`<br>```console.log(``'elementor/popup/show: triggering post render scripts for form'``, formId );`<br>```if``( gform.core?.triggerPostRenderEvents ) {`<br>```// For Gravity Forms 2.9.5 and newer.`<br>```gform.core.triggerPostRenderEvents( formId, currentPage );`<br>```}``else``{`<br>```// For older versions.`<br>```jQuery( document ).trigger(``'gform_post_render'``, [ formId, currentPage ] );`<br>```gform.utils.trigger( { event:``'gform/postRender'``, native:``false``, data: { formId, currentPage } } );`<br>```gform.utils.trigger( { event:``'gform/post_render'``, native:``false``, data: { formId, currentPage } } );`<br>```}`<br>`} );` |
...
[Algolia](https://docs.gravityforms.com/how-to-trigger-gravity-forms-scripts-in-elementor-popups/# "Algolia")
