---
url: "https://docs.gravityforms.com/custom-logging-statements/"
title: "Custom Logging Statements - Gravity Forms Documentation"
---
...
- [Introduction](https://docs.gravityforms.com/custom-logging-statements/#h-introduction)
- [Logging evaluation of conditional logic](https://docs.gravityforms.com/custom-logging-statements/#h-logging-evaluation-of-conditional-logic)
- [Logging field validation errors](https://docs.gravityforms.com/custom-logging-statements/#h-logging-field-validation-errors)
- [Logging saved values](https://docs.gravityforms.com/custom-logging-statements/#h-logging-saved-values)
- [Writing to the core log](https://docs.gravityforms.com/custom-logging-statements/#h-writing-to-the-core-log)
- [Writing to the add-on log](https://docs.gravityforms.com/custom-logging-statements/#h-writing-to-the-add-on-log)
...
Find out how to enable logging in the [Logging and Debugging](https://docs.gravityforms.com/logging-and-debugging/) article.

## Logging evaluation of conditional logic

If you are having an issue with notifications not being sent due to the conditional logic rules not being met you can see exactly how the conditional logic rule is configured and what the actual field value was by using the [gform\_is\_value\_match](https://docs.gravityforms.com/gform_is_value_match) hook in your theme [functions.php](http://codex.wordpress.org/Theme_Development#Functions_File) file.

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8 | `add_filter(``'gform_is_value_match'``,``'log_conditional_logic_evaluation'``, PHP_INT_MAX, 6 );`<br>`function``log_conditional_logic_evaluation(``$is_match``,``$field_value``,``$target_value``,``$operation``,``$source_field``,``$rule``) {`<br>```GFCommon::log_debug(``'gform_is_value_match: $rule => '``. print_r(``$rule``, 1 ) );`<br>```GFCommon::log_debug(``'gform_is_value_match: $field_value => '``. print_r(``$field_value``, 1 ) );`<br>```GFCommon::log_debug(``'gform_is_value_match: $is_match => '``. var_export(``$is_match``, 1 ) );`<br>```return``$is_match``;`<br>`}` |

## Logging field validation errors

Field validation failures can be logged using the [gform\_validation](https://docs.gravityforms.com/gform_validation) hook in your theme [functions.php](http://codex.wordpress.org/Theme_Development#Functions_File) file.

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11 | `add_filter(``'gform_validation'``,``'log_validation_errors'``, PHP_INT_MAX );`<br>`function``log_validation_errors(``$validation_result``) {`<br>```$form``=``$validation_result``[``'form'``];`<br>```foreach``(``$form``[``'fields'``]``as``$field``) {`<br>```if``(``$field``->failed_validation ) {`<br>```GFCommon::log_error(``"form #{$form['id']}: validate() - failed: {$field->label} ({$field->id} - {$field->type}) - message: {$field->validation_message}"``);`<br>```}`<br>```}`<br>```return``$validation_result``;`<br>`}` |

## Logging saved values

Values being saved can be logged using the [gform\_save\_field\_value](https://docs.gravityforms.com/gform_save_field_value) hook in your theme [functions.php](http://codex.wordpress.org/Theme_Development#Functions_File) file.

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9 | `add_filter(``'gform_save_field_value'``,``'log_saved_values'``, PHP_INT_MAX, 5 );`<br>`function``log_saved_values(``$value``,``$entry``,``$field``,``$form``,``$input_id``) {`<br>```$input_name``=``'input_'``.``str_replace``(``'.'``,``'_'``,``$input_id``);`<br>```GFCommon::log_debug(``"log_save_field_value: Input ID: {$input_id}. POST value => "``. print_r( rgpost(``$input_name``), true ) );`<br>```GFCommon::log_debug(``'log_save_field_value: Saved value => '``. print_r(``$value``, true ) );`<br>```return``$value``;`<br>`}` |
...
To also include a variable which contains a [string](https://docs.gravityforms.com/string) in the logging statement you could do this:

|     |
| --- |
| `GFCommon::log_debug(``__METHOD__``.``"(): Nothing to do for the {$event} event."``);` |

If you wanted to output another type of variable such as an [array](https://docs.gravityforms.com/array) you could use the following:
...
If you are creating your own add-on which uses the [Add-on Framework](https://docs.gravityforms.com/category/developers/php-api/add-on-framework) you can use the following to write the logging statement to the log file for the add-on:

|     |
| --- |
| `$this``->log_debug(``__METHOD__``.``'(): feed => '``. print_r(``$feed``, true ) );` |

`__METHOD__` is a PHP [constant](http://php.net/manual/en/language.constants.predefined.php), it will be replaced with the class method name. You could remove that and include the function name or some other identifier in the logging statement itself.
...
[Algolia](https://docs.gravityforms.com/custom-logging-statements/# "Algolia")
