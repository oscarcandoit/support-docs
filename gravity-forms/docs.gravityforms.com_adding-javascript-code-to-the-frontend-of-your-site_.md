---
url: "https://docs.gravityforms.com/adding-javascript-code-to-the-frontend-of-your-site/"
title: "Adding JavaScript Code to the Frontend of Your Site - Gravity Forms Documentation"
---
...
- [Introduction](https://docs.gravityforms.com/adding-javascript-code-to-the-frontend-of-your-site/#h-introduction)
- [Filters](https://docs.gravityforms.com/adding-javascript-code-to-the-frontend-of-your-site/#h-filters)
- [Add Javascript Using wp\_footer.](https://docs.gravityforms.com/adding-javascript-code-to-the-frontend-of-your-site/#h-add-javascript-using-wp-footer)
- [Using a Third Party Plugin](https://docs.gravityforms.com/adding-javascript-code-to-the-frontend-of-your-site/#h-using-a-third-party-plugin)
...
The `gform/theme/scripts_loaded` event can be used to initialize scripts that depend on Gravity Forms front-end form scripts, including the JavaScript `gform` object. Refer to [this](https://docs.gravityforms.com/gform-theme-scripts_loaded/) article for more information about usage.

Alternatively, JavaScript or jQuery code will typically look something like this:

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13 | `document.addEventListener(``'DOMContentLoaded'``,``function``() {`<br>```document.querySelectorAll(``'.gfield_checkbox input'``).forEach(``function``(checkbox) {`<br>```checkbox.addEventListener(``'click'``,``function``() {`<br>```var``isChecked = checkbox.checked;`<br>```var``parent = checkbox.closest(``'li'``);`<br>```if``(isChecked) {`<br>```parent.classList.add(``'selected'``);`<br>```}``else``{`<br>```parent.classList.remove(``'selected'``);`<br>```}`<br>```});`<br>```});`<br>`});` |

Some themes will provide an _init.js_ or a _custom.js_ file. If so, pasting the custom JavaScript in either of these files would be ideal.

If neither an events section nor a filters header is present, you can add your JavaScript code using a <script> block; it is generally recommended to add scripts to the footer using the wp\_footer or wp\_print\_footer\_scripts action hooks for better performance.

## Add Javascript Using wp\_footer.

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10 | `function``custom_enqueue_script() {`<br>```?>`<br>```<script type=``"text/javascript"``>`<br>```document.addEventListener(``'DOMContentLoaded'``,``function``() {`<br>```// custom code`<br>```});`<br>```</script>`<br>```<?php`<br>`}`<br>`add_action(``'wp_footer'``,``'custom_enqueue_script'``);` |

Additional Information on using JavaScript with WordPress can be found here: [https://codex.wordpress.org/Using\_Javascript](https://codex.wordpress.org/Using_Javascript)
...
[Algolia](https://docs.gravityforms.com/adding-javascript-code-to-the-frontend-of-your-site/# "Algolia")
