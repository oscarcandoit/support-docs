---
url: "https://docs.gravityforms.com/gform_stripe_charge_description/"
title: "gform_stripe_charge_description - Gravity Forms Documentation"
---
...
# gform\_stripe\_charge\_description

- [Description](https://docs.gravityforms.com/gform_stripe_charge_description/#h-description)
- [Usage](https://docs.gravityforms.com/gform_stripe_charge_description/#h-usage)
- [Examples](https://docs.gravityforms.com/gform_stripe_charge_description/#h-examples)
- [1\. Remove the entry ID](https://docs.gravityforms.com/gform_stripe_charge_description/#h-1-remove-the-entry-id)
- [2\. Include a field value](https://docs.gravityforms.com/gform_stripe_charge_description/#h-2-include-a-field-value)
- [3\. Override the default description completely](https://docs.gravityforms.com/gform_stripe_charge_description/#h-3-override-the-default-description-completely)
- [Placement](https://docs.gravityforms.com/gform_stripe_charge_description/#h-placement)
- [Source Code](https://docs.gravityforms.com/gform_stripe_charge_description/#h-source-code)

## Description

This filter can be used to modify the charge description for **Products and Services** feeds using any of the [Payment Collection Methods](https://docs.gravityforms.com/stripe-payment-collection-methods/) available before it is sent to Stripe.
...
For Subscription transactions, please see the [gform\_submission\_data\_pre\_process\_payment](https://docs.gravityforms.com/gform_submission_data_pre_process_payment/) filter.

You may also want to check [gform\_stripe\_charge\_pre\_create](https://docs.gravityforms.com/gform_stripe_charge_pre_create/) to add the **statement\_descriptor** property used on bank statements.
...
|     |     |
| --- | --- |
| 1 | `add_filter(``'gform_stripe_charge_description'``,``'your_function_name'``, 10, 5 );` |
...
- **$strings** [array](https://docs.gravityforms.com/array)
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5 | `add_filter(``'gform_stripe_charge_description'``,``'remove_entry_id'``, 10, 2 );`<br>`function``remove_entry_id(``$description``,``$strings``) {`<br>```unset(``$strings``[``'entry_id'``] );`<br>```return``implode(``', '``,``$strings``);`<br>`}` |
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4 | `add_filter(``'gform_stripe_charge_description'``,``'append_field_value'``, 10, 3 );`<br>`function``append_field_value(``$description``,``$strings``,``$entry``) {`<br>```return``$description``.``', Name: '``. rgar(``$entry``,``'1.3'``) .``' '``. rgar(``$entry``,``'1.6'``);`<br>`}` |
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>17<br>18 | `add_filter(``'gform_stripe_charge_description'``,``'gf_replace_charge_description'``, 10, 5 );`<br>`function``gf_replace_charge_description(``$description``,``$strings``,``$entry``,``$submission_data``,``$feed``) {`<br>```GFCommon::log_debug(``__METHOD__``.``'(): running.'``);`<br>```$feed_name``= rgars(``$feed``,``'meta/feedName'``);`<br>```// Update the following line to use your feed name.`<br>```if``(``'Invoice Payment via Stripe'``!=``$feed_name``){`<br>```return``$description``;`<br>```}`<br>```GFCommon::log_debug(``__METHOD__``.``'(): running for feed '``.``$feed_name``);`<br>```// Change 21 to your field id number.`<br>```$description``=``'Invoice Payment for #'``. rgar(``$entry``,``'21'``);`<br>```GFCommon::log_debug(``__METHOD__``.``'(): Custom Description: '``.``$description``);`<br>```return``$description``;`<br>`}` |
...
|     |     |
| --- | --- |
| 1 | `apply_filters(``'gform_stripe_charge_description'``, implode(``', '``,``$strings``),``$strings``,``$entry``,``$submission_data``,``$feed``)` |

This filter is located in GFStripe::get\_payment\_description() in _class-gf-stripe.php_.
...
[Algolia](https://docs.gravityforms.com/gform_stripe_charge_description/# "Algolia")
