---
url: "https://docs.gravityforms.com/gform_multifile_upload_field/"
title: "gform_multifile_upload_field - Gravity Forms Documentation"
---
...
# gform\_multifile\_upload\_field

- [Description](https://docs.gravityforms.com/gform_multifile_upload_field/#h-description)
- [Usage](https://docs.gravityforms.com/gform_multifile_upload_field/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_multifile_upload_field/#h-parameters)
- [Example(s)](https://docs.gravityforms.com/gform_multifile_upload_field/#h-examples)
- [Placement](https://docs.gravityforms.com/gform_multifile_upload_field/#h-placement)
- [Since](https://docs.gravityforms.com/gform_multifile_upload_field/#h-since)
- [Source Code](https://docs.gravityforms.com/gform_multifile_upload_field/#h-source-code)

## Description

The “gform\_multifile\_upload\_field” filter in Gravity Forms is used to filter the field object that will be associated with the uploaded file. This is useful when you want to use Gravity Forms upload system to upload files. Using this filter you can return a one-time-use field object to process the file as desired.
...
|     |     |
| --- | --- |
| 1 | `add_filter(``'gform_multifile_upload_field'``,``'your_function_name'``, 10, 3 );` |

To target a specific form, append the form id to the hook name. (format: gform\_multifile\_upload\_field\_FORMID)

|     |     |
| --- | --- |
| 1 | `add_filter(``'gform_multifile_upload_field_21'``,``'your_function_name'``, 10, 3 );` |

To target a specific form and field, append the form id and field id to the hook name. (format: gform\_multifile\_upload\_field\_FORMID\_FIELDID)

|     |     |
| --- | --- |
| 1 | `add_filter(``'gform_multifile_upload_field_21_3'``,``'your_function_name'``, 10, 3 );` |
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13 | `add_filter(``'gform_multifile_upload_field'``,``'create_custom_file_upload_field'``, 10, 3 );`<br>`function``create_custom_file_upload_field(``$field``,``$form``,``$field_id``) {`<br>```$field``=``new``GF_Field_FileUpload(``array``(`<br>```'id'``=>``$field_id``,`<br>```'multipleFiles'``=> true,`<br>```'maxFiles'``=> 1,`<br>```'maxFileSize'``=>``''``,`<br>```'allowedExtensions'``=>``'csv'`<br>```) );`<br>```return``$field``;`<br>```}` |
...
This filter is located in GFAsyncUpload::upload in _gravityforms/upload.php_.
...
[Algolia](https://docs.gravityforms.com/gform_multifile_upload_field/# "Algolia")
