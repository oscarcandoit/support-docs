---
url: "https://docs.gravityforms.com/gform_is_valid_notification_to/"
title: "gform_is_valid_notification_to - Gravity Forms Documentation"
---
...
# gform\_is\_valid\_notification\_to

- [Description](https://docs.gravityforms.com/gform_is_valid_notification_to/#h-description)
- [Usage](https://docs.gravityforms.com/gform_is_valid_notification_to/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_is_valid_notification_to/#h-parameters)
- [Examples](https://docs.gravityforms.com/gform_is_valid_notification_to/#h-examples)
- [Placement](https://docs.gravityforms.com/gform_is_valid_notification_to/#h-placement)
- [Source Code](https://docs.gravityforms.com/gform_is_valid_notification_to/#h-source-code)
...
The value being filtered. True if the to email is valid. False otherwise. Gravity Forms performs the validation before executing this hook, so $is\_valid will contain the result of that validation. To override Gravity Form’s validation, simply set the value of this variable to true or false and return it.

- **$to\_type** [string](https://docs.gravityforms.com/string)

The type of “Send To” that is configured. The possible values are “email”, “field” and “routing”.

- **$to\_email** [string](https://docs.gravityforms.com/string)

The email address configured. Only applies when type ($to\_type) is set to “email”

- **$to\_field** [string](https://docs.gravityforms.com/string)

The ID of the field selected to be used as the “Send To”. Only applies when type ($to\_type) is set to “field”


## Examples

The following example demostrates how to allow the merge tag {user:user\_email} to be used for notificatons “Send To” email

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9 | `add_filter(``'gform_is_valid_notification_to'``,``'validate_to_email'``, 10, 4 );`<br>`function``validate_to_email(``$is_valid``,``$to_type``,``$to_email``,``$to_field``) {`<br>```if``(``$to_email``==``'{user:user_email}'``) {`<br>```return``true;`<br>```}`<br>```return``$is_valid``;`<br>`}` |
...
This filter is located in GFNotification::is\_valid\_notification\_to() in _notification.php_
...
[Algolia](https://docs.gravityforms.com/gform_is_valid_notification_to/# "Algolia")
