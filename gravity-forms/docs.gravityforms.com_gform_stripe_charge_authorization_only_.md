---
url: "https://docs.gravityforms.com/gform_stripe_charge_authorization_only/"
title: "gform_stripe_charge_authorization_only - Gravity Forms Documentation"
---
...
- [Description](https://docs.gravityforms.com/gform_stripe_charge_authorization_only/#h-description)
- [Usage](https://docs.gravityforms.com/gform_stripe_charge_authorization_only/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_stripe_charge_authorization_only/#h-parameters)
- [Examples](https://docs.gravityforms.com/gform_stripe_charge_authorization_only/#h-examples)
- [1\. Apply to all product and service feeds.](https://docs.gravityforms.com/gform_stripe_charge_authorization_only/#h-1-apply-to-all-product-and-service-feeds)
- [2\. Apply to a specific feed.](https://docs.gravityforms.com/gform_stripe_charge_authorization_only/#h-2-apply-to-a-specific-feed)
- [Placement](https://docs.gravityforms.com/gform_stripe_charge_authorization_only/#h-placement)
- [Since](https://docs.gravityforms.com/gform_stripe_charge_authorization_only/#h-since)
- [Source Code](https://docs.gravityforms.com/gform_stripe_charge_authorization_only/#h-source-code)

## Description

The gform\_stripe\_charge\_authorization\_only filter allows authorization-only transactions by preventing the capture request from being made after the saved entry. Use this filter when using the **Stripe Card Element** in your form. If you are using the **Stripe Payment Element**, use the [gform\_stripe\_payment\_element\_authorization\_only](https://docs.gravityforms.com/gform_stripe_payment_element_authorization_only/) filter instead.

**Note:** This filter **does not apply to** **subscriptions** because[subscriptions](https://docs.stripe.com/api/subscriptions/create) require immediate capture of the initial payment. It applies only to Product and Service feeds and should only be used when [Use Stripe’s Payment Element](https://docs.gravityforms.com/stripe-payment-methods/#h-enable-additional-payment-methods-setting) is **unchecked**. If you **are** using Stripe’s Payment Element, please use the  [gform\_stripe\_payment\_element\_authorization\_only](https://docs.gravityforms.com/gform_stripe_payment_element_authorization_only/) filter instead.

[![Stripe Field with Use Stripe's Payment Element unchecked which enables Stripe's Card Element](https://docs.gravityforms.com/wp-content/uploads/2025/08/stripe-card-elements-setting.png)](https://docs.gravityforms.com/wp-content/uploads/2025/08/stripe-card-elements-setting.png)
...
|     |     |
| --- | --- |
| 1 | `add_filter(``'gform_stripe_charge_authorization_only'``,``'your_function_name'``, 10, 5 );` |

## Parameters

| **Variable** | **Type** | **Description** |
| --- | --- | --- |
| **$authorization\_only** | bool | Defaults to false; return true to prevent payment from being captured. |
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14 | `add_filter(``'gform_stripe_charge_authorization_only'``,``'stripe_charge_authorization_only'``, 10, 2 );`<br>`function``stripe_charge_authorization_only(``$authorization_only``,``$feed``) {`<br>```gf_stripe()->log_debug(``__METHOD__``.``'(): Running...'``);`<br>```$feed_name``= rgars(``$feed``,``'meta/feedName'``);`<br>```if``(``$feed_name``===``'your feed name here'``) {`<br>```gf_stripe()->log_debug(``__METHOD__``.``'(): Authorization only charge for feed '``.``$feed_name``);`<br>```return``true;`<br>```}`<br>```return``$authorization_only``;`<br>`}` |
...
This hook is located in GFStripe::capture() in _class-gf-stripe.php_.
...
[Algolia](https://docs.gravityforms.com/gform_stripe_charge_authorization_only/# "Algolia")
