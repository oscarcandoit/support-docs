---
url: "https://docs.gravityforms.com/gform_source_id_pre_save_entry/"
title: "gform_source_id_pre_save_entry - Gravity Forms Documentation"
---
...
# gform\_source\_id\_pre\_save\_entry

- [Description](https://docs.gravityforms.com/gform_source_id_pre_save_entry/#h-description)
- [Usage](https://docs.gravityforms.com/gform_source_id_pre_save_entry/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_source_id_pre_save_entry/#h-parameters)
- [Examples](https://docs.gravityforms.com/gform_source_id_pre_save_entry/#h-examples)
- [Custom Post Type](https://docs.gravityforms.com/gform_source_id_pre_save_entry/#h-custom-post-type)
- [Placement](https://docs.gravityforms.com/gform_source_id_pre_save_entry/#h-placement)
- [Source Code](https://docs.gravityforms.com/gform_source_id_pre_save_entry/#h-source-code)
- [Since](https://docs.gravityforms.com/gform_source_id_pre_save_entry/#h-since)
...
To target a specific form, append the form id to the hook name. (format: gform\_source\_id\_pre\_save\_entry\_FORMID)
...
- **$id** [Integer](https://docs.gravityforms.com/integer/) \| [Null](https://docs.gravityforms.com/null)
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>17<br>18<br>19<br>20<br>21<br>22 | `add_filter(``'gform_source_id_pre_save_entry'``,``function``(``$id``,``$form``) {`<br>```if``( !``empty``(``$id``) || ! defined(``'DOING_AJAX'``) || ! DOING_AJAX ) ) {`<br>```return``$id``;`<br>```}`<br>```$referer``= wp_get_referer();`<br>```if``(``empty``(``$referer``) ) {`<br>```return``$id``;`<br>```}`<br>```$path``=``parse_url``(``$referer``, PHP_URL_PATH );`<br>```if``(``empty``(``$path``) ) {`<br>```return``$id``;`<br>```}`<br>```$page``= get_page_by_path(``$path``, OBJECT,``'your_custom_post_type'``);`<br>```if``(``empty``(``$page``) ) {`<br>```return``$id``;`<br>```}`<br>```return``$page``->ID;`<br>`}, 10, 2 );` |
...
This filter is located in `GFFormsModel::get_source_id()` in / _forms\_model.php_
...
[Algolia](https://docs.gravityforms.com/gform_source_id_pre_save_entry/# "Algolia")
