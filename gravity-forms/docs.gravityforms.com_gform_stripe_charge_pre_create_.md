---
url: "https://docs.gravityforms.com/gform_stripe_charge_pre_create/"
title: "gform_stripe_charge_pre_create - Gravity Forms Documentation"
---
...
# gform\_stripe\_charge\_pre\_create

- [Description](https://docs.gravityforms.com/gform_stripe_charge_pre_create/#h-description)
- [Usage](https://docs.gravityforms.com/gform_stripe_charge_pre_create/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_stripe_charge_pre_create/#h-parameters)
- [Examples](https://docs.gravityforms.com/gform_stripe_charge_pre_create/#h-examples)
- [1\. Add the statement\_descriptor property.](https://docs.gravityforms.com/gform_stripe_charge_pre_create/#h-1-add-the-statement-descriptor-property)
- [2\. Attach payment method to the customer.](https://docs.gravityforms.com/gform_stripe_charge_pre_create/#h-2-attach-payment-method-to-the-customer)
- [3\. Add the description property.](https://docs.gravityforms.com/gform_stripe_charge_pre_create/#h-3-add-the-description-property)
- [Placement](https://docs.gravityforms.com/gform_stripe_charge_pre_create/#h-placement)
- [Since](https://docs.gravityforms.com/gform_stripe_charge_pre_create/#h-since)
- [Source Code](https://docs.gravityforms.com/gform_stripe_charge_pre_create/#h-source-code)

**Note:** As of Stripe version 3.0 this filter is **not available for use with the Stripe Checkout** payment collection method. Use the [gform\_stripe\_session\_data](https://docs.gravityforms.com/gform_stripe_session_data) filter instead **if using Stripe Checkout**.

## Description

The gform\_stripe\_charge\_pre\_create filter allows the charge properties to be overridden before the charge is created by the Stripe API. This filter should not be used to override the $charge\_meta\[‘metadata’\] property.

**Note:** This filter applies only to Products and Services feeds. When “Use Stripe’s Payment Element” is enabled in the Stripe field settings you need to use this filter along with [gform\_stripe\_payment\_element\_initial\_payment\_information](https://docs.gravityforms.com/gform_stripe_payment_element_initial_payment_information/)
...
|     |     |
| --- | --- |
| 1 | `add_filter(``'gform_stripe_charge_pre_create'``,``'your_function_name'``, 10, 5 );` |

## Parameters

| **Parameter** | **Type** | **Description** |
| --- | --- | --- |
| **$charge\_meta** | array | The properties for the charge to be created. |
| **$feed** | [Feed Object](https://docs.gravityforms.com/feed-object/) | The feed object currently being processed. |
| **$submission\_data** | [Submission Data](https://docs.gravityforms.com/submission-data/) | Contains the form title, payment amount, setup fee amount, trial amount, line items created using the submitted pricing field values and any discounts from coupons. |
| **$form** | [Form Object](https://docs.gravityforms.com/form-object/) | The Form which is currently being processed. |
| **$entry** | [Entry Object](https://docs.gravityforms.com/entry-object/) | The entry from which the subscription was created. |

## Examples

### 1\. Add the statement\_descriptor property.

The following example shows how you can add the statement\_descriptor property to the charge meta. Please check [Stripe’s documentation](https://stripe.com/docs/statement-descriptors#requirements) for requirements.

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6 | `add_filter(``'gform_stripe_charge_pre_create'``,``'stripe_charge_pre_create'``, 10, 5 );`<br>`function``stripe_charge_pre_create(``$charge_meta``,``$feed``,``$submission_data``,``$form``,``$entry``) {`<br>```$charge_meta``[``'statement_descriptor'``] =``'STATEMENTDESC'``;`<br>```return``$charge_meta``;`<br>`}` |
...
If the _Enable additional payment methods_ setting is not active in the Stripe field, you would need to use it in combination with [gform\_stripe\_customer\_id](https://docs.gravityforms.com/gform_stripe_customer_id/#h-2-create-new-customer) to create the customer object.

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8 | `// This sets setup_future_usage parameter to off_session to save the payment method.`<br>`add_filter(``'gform_stripe_charge_pre_create'``,``function``(``$charge_meta``,``$feed``,``$submission_data``,``$form``,``$entry``) {`<br>```gf_stripe()->log_debug(``__METHOD__``.``'(): Adding setup_future_usage for feed '``. rgars(``$feed``,``'meta/feedName'``) );`<br>```$charge_meta``[``'setup_future_usage'``] =``'off_session'``;`<br>```return``$charge_meta``;`<br>`}, 10, 5 );` |
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7 | `add_filter(``'gform_stripe_charge_pre_create'``,``function``(``$charge_meta``,``$feed``,``$submission_data``,``$form``,``$entry``) {`<br>```if``(``empty``(``$charge_meta``[``'description'``] ) ) {`<br>```$charge_meta``[``'description'``] = gf_stripe()->get_payment_description(``$entry``,``$submission_data``,``$feed``);`<br>```}`<br>```return``$charge_meta``;`<br>`}, 10, 5 );` |
...
|     |     |
| --- | --- |
| 1 | `$charge_meta``= apply_filters(``'gform_stripe_charge_pre_create'``,``$charge_meta``,``$feed``,``$submission_data``,``$form``,``$entry``);` |

This hook is located in GFStripe::authorize\_product() in _class-gf-stripe.php_.
...
[Algolia](https://docs.gravityforms.com/gform_stripe_charge_pre_create/# "Algolia")
