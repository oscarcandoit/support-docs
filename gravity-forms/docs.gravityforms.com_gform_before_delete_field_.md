---
url: "https://docs.gravityforms.com/gform_before_delete_field/"
title: "gform_before_delete_field - Gravity Forms Documentation"
---
...
# gform\_before\_delete\_field

- [Description](https://docs.gravityforms.com/gform_before_delete_field/#h-description)
- [Usage](https://docs.gravityforms.com/gform_before_delete_field/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_before_delete_field/#h-parameters)
- [Examples](https://docs.gravityforms.com/gform_before_delete_field/#h-examples)
- [Placement](https://docs.gravityforms.com/gform_before_delete_field/#h-placement)
- [Source Code](https://docs.gravityforms.com/gform_before_delete_field/#h-source-code)
...
|     |     |
| --- | --- |
| 1 | `add_action(``'gform_before_delete_field'``,``'do_cleanup'``, 10, 2 );` |
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13 | `add_action(``'gform_before_delete_field'``,``'log_field_deleted'``, 10, 2 );`<br>`function``log_field_deleted(``$form_id``,``$field_id``) {`<br>```$log_file``= ABSPATH .``'/gf_deleted_fields.log'``;`<br>```$f``=``fopen``(``$log_file``,``'a'``);`<br>```$user``= wp_get_current_user();`<br>```$form``= GFAPI::get_form(``$form_id``);`<br>```$field``= GFFormsModel::get_field(``$form``,``$field_id``);`<br>```$label``=``is_object``(``$field``) ?``$field``->label :``'Unknown'``;`<br>```fwrite(``$f``,``date``(``'c'``) .``" - Field deleted by {$user->user_login}. Form ID: {$form_id}. Field: {$label} (ID: {$field_id})\n"``);`<br>```fclose(``$f``);`<br>`}` |
...
[Algolia](https://docs.gravityforms.com/gform_before_delete_field/# "Algolia")
