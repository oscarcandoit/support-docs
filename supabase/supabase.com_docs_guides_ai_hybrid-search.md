...
create table documents (  id bigint primary key generated always as identity,  content text,  fts tsvector generated always as (to_tsvector('english', content)) stored,  embedding extensions.vector(512));
```
...
select  *from  hybrid_search(    'Italian recipes with tomato sauce', -- user query    '[...]'::extensions.vector(512), -- embedding generated from user query    10  );
```
