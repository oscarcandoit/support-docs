...
If you are using [`vercel build`](https://vercel.com/docs/cli/build) or [`vercel dev`](https://vercel.com/docs/cli/dev), you should use [`vercel pull`](https://vercel.com/docs/cli/pull) instead. Those commands
operate on a local copy of environment variables and Project settings that are
saved under `.vercel/`, which`vercel pull` provides.
...
Environment Variable and piping that value into the`vercel dev` command. Warning: this will save the value
...
Environment Variables from the cloud and write to the`.env.local` file.
...
"feature-branch" Environment Variables from the cloud and write to the`.env.local` file.
...
Using the `vercel env pull` command with the`--yes` option to overwrite an existing environment
file.
