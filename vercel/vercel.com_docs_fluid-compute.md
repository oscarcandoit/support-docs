...
Fluid compute offers a blend of serverless flexibility and server-like capabilities. Unlike traditional [serverless architectures](https://vercel.com/docs/fundamentals/what-is-compute#serverless), which can face issues such as cold starts and [limited functionalities](https://vercel.com/docs/fundamentals/what-is-compute#serverless-disadvantages), fluid compute provides a hybrid solution. It overcomes the limitations of both serverless and server-based approaches, delivering the advantages of both worlds, including:

- [Zero configuration out of the box](https://vercel.com/docs/fluid-compute#default-settings-by-plan): Fluid compute comes with preset defaults that automatically optimize your functions for both performance and cost efficiency.
- [Optimized concurrency](https://vercel.com/docs/fluid-compute#optimized-concurrency): Optimize resource usage by handling multiple invocations within a single function instance. Can be used with the Node.js and Python runtimes.
...
- Background processing: After fulfilling user requests, you can continue executing background tasks using [`waitUntil`](https://vercel.com/docs/functions/functions-api-reference/vercel-functions-package#waituntil). This allows for a responsive user experience while performing time-consuming operations like logging and analytics in the background.
- Automatic cold start optimizations: Reduces the effects of cold starts through [automatic bytecode optimization](https://vercel.com/docs/fluid-compute#bytecode-caching), and function pre-warming on production deployments.
- Cross-region and availability zone failover: Ensure high availability by first failing over to [another availability zone (AZ)](https://vercel.com/docs/functions/configuring-functions/region#automatic-failover) within the same region if one goes down. If all zones in that region are unavailable, Vercel automatically redirects traffic to the next closest region. Zone-level failover also applies to non-fluid deployments.
...
See [What is compute?](https://vercel.com/docs/fundamentals/what-is-compute) to learn more about fluid compute and how it compares to traditional serverless models.

## [Enabling fluid compute](https://vercel.com/docs/fluid-compute\#enabling-fluid-compute)
...
### [Enable for entire project](https://vercel.com/docs/fluid-compute\#enable-for-entire-project)
...
### [Enable for specific environments and deployments](https://vercel.com/docs/fluid-compute\#enable-for-specific-environments-and-deployments)

You can programmatically enable fluid compute using the [`fluid` property](https://vercel.com/docs/project-configuration#fluid) in your `vercel.json` file. This approach is particularly useful for:
...
## [Available runtime support](https://vercel.com/docs/fluid-compute\#available-runtime-support)

Fluid compute is available for the following runtimes:

- [Node.js](https://vercel.com/docs/functions/runtimes/node-js)
- [Python](https://vercel.com/docs/functions/runtimes/python)
- [Edge](https://vercel.com/docs/functions/runtimes/edge)

## [Optimized concurrency](https://vercel.com/docs/fluid-compute\#optimized-concurrency)
...
![How multiple requests are processed in the fluid compute model with optimized concurrency.](https://vercel.com/vc-ap-vercel-docs/_next/image?url=https%3A%2F%2F7nyt0uhk7sse4zvn.public.blob.vercel-storage.com%2Fdocs-assets%2Fstatic%2Fdocs%2Ffluid%2Fserverless-active-light.avif&w=3840&q=75)![How multiple requests are processed in the fluid compute model with optimized concurrency.](https://vercel.com/vc-ap-vercel-docs/_next/image?url=https%3A%2F%2F7nyt0uhk7sse4zvn.public.blob.vercel-storage.com%2Fdocs-assets%2Fstatic%2Fdocs%2Ffluid%2Fserverless-active-dark.avif&w=3840&q=75)How multiple requests are processed in the fluid compute model with optimized concurrency.
...
Optimized concurrency in fluid compute is available when using Node.js or Python runtimes. See the [efficient serverless Node.js with in-function concurrency](https://vercel.com/blog/serverless-servers-node-js-with-in-function-concurrency) blog post to learn more.

## [Bytecode caching](https://vercel.com/docs/fluid-compute\#bytecode-caching)

When using [Node.js version 20+](https://vercel.com/docs/functions/runtimes/node-js/node-js-versions), Vercel Functions use bytecode caching to reduce cold start times. This stores the compiled bytecode of JavaScript files after their first execution, eliminating the need for recompilation during subsequent cold starts.
...
For [frameworks](https://vercel.com/docs/frameworks) that output ESM, all CommonJS dependencies
(for example, `react`, `node-fetch`) will be opted into bytecode caching.

## [Isolation boundaries and global state](https://vercel.com/docs/fluid-compute\#isolation-boundaries-and-global-state)
...
## [Default settings by plan](https://vercel.com/docs/fluid-compute\#default-settings-by-plan)

Fluid Compute includes default settings that vary by plan:

| Settings | Hobby | Pro | Enterprise |
| --- | --- | --- | --- |
| [CPU configuration](https://vercel.com/docs/functions/configuring-functions/memory#memory-/-cpu-type) | Standard | Standard / Performance | Standard / Performance |
| [Default / Max duration](https://vercel.com/docs/functions/limitations#max-duration) | 300s (5 minutes) / 300s (5 minutes) | 300s (5 minutes) / 800s (13 minutes) | 300s (5 minutes) / 800s (13 minutes) |
| [Multi-region failover](https://vercel.com/docs/functions/configuring-functions/region#automatic-failover) |  |  |  |
| [Multi-region functions](https://vercel.com/docs/functions/runtimes#location) |  | Up to 3 | All |

## [Order of settings precedence](https://vercel.com/docs/fluid-compute\#order-of-settings-precedence)

The settings you configure in your [function code](https://vercel.com/docs/functions/configuring-functions), [dashboard](https://vercel.com/dashboard), or [`vercel.json`](https://vercel.com/docs/project-configuration) file will override the default fluid compute settings.
...
| --- | --- | --- | --- |
| 1 | Function code | Settings in your function code always take top priority. These include max duration defined directly in your code. | [`maxDuration`](https://vercel.com/docs/functions/configuring-functions/duration) |
| 2 | `vercel.json` | Any settings in your [`vercel.json`](https://vercel.com/docs/project-configuration) file, like max duration, and region, will override dashboard and Fluid defaults. | [`maxDuration`](https://vercel.com/docs/functions/configuring-functions/duration), [`region`](https://vercel.com/docs/functions/configuring-functions/region) |
| 3 | Dashboard | Changes made in the dashboard, such as max duration, region, or CPU, override Fluid defaults. | [`maxDuration`](https://vercel.com/docs/functions/configuring-functions/duration), [`region`](https://vercel.com/docs/functions/configuring-functions/region), [`memory`](https://vercel.com/docs/functions/configuring-functions/memory) |
...
## [Pricing and usage](https://vercel.com/docs/fluid-compute\#pricing-and-usage)

See the [fluid compute pricing](https://vercel.com/docs/functions/usage-and-pricing) documentation for details on how fluid compute is priced, including active CPU, provisioned memory, and invocations.

* * *

[Previous\\
\\
Collaboration / Toolbar](https://vercel.com/docs/vercel-toolbar)
