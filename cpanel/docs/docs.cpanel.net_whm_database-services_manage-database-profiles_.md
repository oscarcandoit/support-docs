---
url: "https://docs.cpanel.net/whm/database-services/manage-database-profiles/"
title: "Manage Database Profiles | cPanel & WHM Documentation"
---

[Skip to main content](https://docs.cpanel.net/whm/database-services/manage-database-profiles/#main-content)
...
[Overview](https://docs.cpanel.net/whm/database-services/manage-database-profiles/#overview)

* * *

[Feature requirements](https://docs.cpanel.net/whm/database-services/manage-database-profiles/#feature-requirements)

* * *

[Amazon RDS servers](https://docs.cpanel.net/whm/database-services/manage-database-profiles/#amazon-rds-servers)

* * *

[Remote database server information](https://docs.cpanel.net/whm/database-services/manage-database-profiles/#remote-database-server-information)

* * *

[MySQL profile information](https://docs.cpanel.net/whm/database-services/manage-database-profiles/#mysql-profile-information)

* * *

[Add profile](https://docs.cpanel.net/whm/database-services/manage-database-profiles/#add-profile)

* * *

[Edit profile](https://docs.cpanel.net/whm/database-services/manage-database-profiles/#edit-profile)

* * *

Was this helpful?

[**Table of Contents**](https://docs.cpanel.net/whm/database-services/manage-database-profiles/#table-of-contents-toggle)
...
#### [120](https://docs.cpanel.net/whm/database-services/manage-database-profiles/120/)

#### [124](https://docs.cpanel.net/whm/database-services/manage-database-profiles/124/)

#### [126](https://docs.cpanel.net/whm/database-services/manage-database-profiles/)
...
This interface allows you to manage multiple MySQL® or MariaDB® profiles. A profile defines the connection information for a local or remote database server. The interface also allows you to set which profile is [active](https://docs.cpanel.net/whm/database-services/manage-database-profiles/#database-profile-information). The active profile determines how the `root` database user connects to databases, as well as the location of newly-created users and databases.

Note:

In cPanel & WHM version 118 and earlier, we titled this interface [_Manage MySQL® Profiles_](https://docs.cpanel.net/whm/sql-services/manage-mysql-profiles) and its section in the WHM interface [SQL Services](https://docs.cpanel.net/whm/sql-services/).

You may wish to use a remote server if, for example, you manage particularly busy servers or servers with large databases. You can offload database-related work to a remote database server.

Remote servers include other cPanel & WHM servers that run MariaDB or MySQL, a dedicated MariaDB or MySQL server, and the Amazon Relational Database™ Service (RDS).
...
- Upgrade the local cPanel & WHM server to MySQL 8.0 or later.
- Add a `root` user to the remote server that is **only** allowed to log in from the local server’s IP address.

If the remote server runs MySQL 8.0 or later installed from the community repository and you have authentication issues, read our [Troubleshoot MySQL® Profiles](https://docs.cpanel.net/knowledge-base/sql/troubleshoot-mysql-profiles/) documentation.
...
- MariaDB® versions 10.1, 10.2, 10.3, 10.5, 10.6, and 11.4.
...
Additionally, a `/root/.my.cnf` file **must** exist on any remote MySQL or MariaDB servers, and this file **must** contain the MySQL or MariaDB `root` user and password.
...
- We **strongly** recommend that you **only** connect one cPanel & WHM server to each remote database server. If you connect multiple cPanel & WHM servers to one remote database server, you may experience database and username conflicts.
- This feature does **not** automatically transfer your database data.
- Do **not** use the `skip-name-resolve` option in your server’s database configuration. This option can cause problems on **any** server. It will create **more** problems on remote servers, during account transfers and restorations, and with phpMyAdmin.

### MySQL profile information

The interface displays the following columns for each database profile:

- _Profile_ — The name of the database profile.

- _Host_ — The database server’s IP address or hostname.

- _Port_ — The database server’s port number.

- _User_ — The SSH or database username that you will use to authenticate to this host.

- _Type_ — A description of the profile data.

- _Actions_ — Click one of the following icons to perform the appropriate action:
  - ![Validate icon](https://docs.cpanel.net/img/managemysqlprofilesvalidate.png)_Validate_ — Test the profile’s database server settings.
  - ![Activate icon](https://docs.cpanel.net/img/managemysqlprofilesactivate.png)_Activate_ — Set this profile as the active profile. A green lightning bolt icon (![lightning bolt icon](https://docs.cpanel.net/img/managemysqlprofilesbolt.png)) indicates the currently-active profile.
...
To create a new database profile, perform the following steps:

1. Click _Add Profile_. A new interface will appear.

2. Enter the desired database profile name in the _Profile Name_ text box.
...
After you save the profile name, you cannot change it.


3. Select a method to use to configure the new profile:
   - _Automatically create a database superuser via SSH_ — Select this method to create the new profile manually.
   - _Manually enter an existing database superuser’s credentials_ — Select this method to manually enter the new profile’s information.
...
- **Automatically create a MySQL superuser via SSH**
  - _Host_ — The database server’s IP address or hostname.
    ...
    You can enter a public or private IP address. However, make certain that the database server can resolve the cPanel & WHM server’s IP address to its fully-qualified hostname.
...
- **Manually enter an existing MySQL superuser’s credentials**
  ...
  - _Port_ — The database server’s port number. You **must** enter a port number between `1` and `65535`.
  ...
  - _Username_ — The database superuser’s username.
  - _Password_ — The database superuser’s password.
...
To edit a database profile, perform the following steps:
...
2. If you wish to change the hostname, enter the new database server’s IP address or hostname in the Host text box.
...
4. If you wish to change the database superuser’s username for the connection, enter the new username in the _Username_ text box.

5. If you wish to change the database superuser’s password for the connection, enter the new password in the _Password_ text box.
