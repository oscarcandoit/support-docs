---
url: "https://docs.cpanel.net/whm/server-contacts/contact-manager/"
title: "Contact Manager | cPanel & WHM Documentation"
---

[Skip to main content](https://docs.cpanel.net/whm/server-contacts/contact-manager/#main-content)
...
[Overview](https://docs.cpanel.net/whm/server-contacts/contact-manager/#overview)

* * *

[Communication Type](https://docs.cpanel.net/whm/server-contacts/contact-manager/#communication-type)

* * *

[Test a communication type](https://docs.cpanel.net/whm/server-contacts/contact-manager/#test-a-communication-type)

* * *

[Notifications](https://docs.cpanel.net/whm/server-contacts/contact-manager/#notifications)

* * *

Was this helpful?

[**Table of Contents**](https://docs.cpanel.net/whm/server-contacts/contact-manager/#table-of-contents-toggle)
...
Last modified: _2025 August 13_
...
Additional WP Toolkit templates may be available based on 3rd party installations. For more information, read the [WP Toolkit](https://docs.plesk.com/en-US/obsidian/administrator-guide/plesk-administration/wp-toolkit-in-plesk.73391/) documentation.
...
| _API Token Created_ | A user creates an API Token. | `APIToken/Created.*.tmpl` |
...
| _AutoSSL has installed a certificate successfully._ | AutoSSL installs an SSL certificate. | `AutoSSL/CertificateInstalled.*.tmpl` |
...
| _AutoSSL has renewed a certificate, but the new certificate lacks at least one domain that the previous certificate secured._ | AutoSSL renews a certificate but the new certificate lacks at least one domain that the previous certificate secured. | `AutoSSL/CertificateInstalledReducedCoverage.*.tmpl` |
...
| _Bandwidth Limits_ | An account exceeds its bandwidth limit. <br>Important:<br>This option will **only** send notifications if you enable the _Send bandwidth limit notification emails_ option in WHM’s [_Tweak Settings_](https://docs.cpanel.net/whm/server-configuration/tweak-settings/) interface ( _WHM » Home » Server Configuration » Tweak Settings_). | `BandwidthUsageExceeded/Owner.*.tmpl` |
...
| _cPanel Account Password_ | A user changes their password. <br>Note:<br>This event applies to the `root` user. | `ChangePassword/User.*.tmpl` |
...
| _cPanel service SSL certificate warnings_ | The [`/usr/local/cpanel/bin/checkallsslcerts`](https://docs.cpanel.net/whm/scripts/the-checkallsslcerts-script/) script detected issues with the cPanel & WHM services’ SSL certificates. | `SSL/CheckAllCertsWarnings.*.tmpl` |
...
| _DNSSEC key synchronization failure_ | [DNSSEC keys](https://docs.cpanel.net/knowledge-base/dns/dnssec/) don’t sync properly. This means servers in a DNS cluster don’t currently serve DNSSEC records for a DNS zone. | `DnsAdmin/DnssecError.*.tmpl` |
| _Dovecot Solr maintenance task errors._ | The [`/usr/local/cpanel/3rdparty/scripts/cpanel_dovecot_solr_maintenance`](https://docs.cpanel.net/whm/scripts/the-cpanel-dovecot-solr-rebuild-index-script) script fails. | `Solr/Maintenance.*.tmpl` |
...
| _Filesystem quotas are currently broken._ | The system cannot enable disk quotas automatically. <br>Note:<br>This notification only affects systems that run [Ubuntu®](https://docs.cpanel.net/installation-guide/system-requirements-ubuntu/). | `Quota/Broken.*.tmpl` |
...
| _Greylist System Changes_ | An administrator or the system removes a mail provider from the [Greylisting Common Mail Providers](https://docs.cpanel.net/whm/email/greylisting/#common-mail-providers) list. | `Greylist/CommonProviderRemoval.*.tmpl` |
...
| _Installation of purchased SSL certificates_ | The system installs SSL certificates that a user purchases through cPanel’s [_SSL/TLS Wizard_](https://docs.cpanel.net/cpanel/security/ssl-tls-wizard/) interface ( _cPanel » Home » Security » SSL/TLS Wizard_). | `Market/SSLWebInstall.*.tmpl` |
...
| _Notices concerning goods and services purchased via the cPanel Market_ | A user purchases an SSL certificate or other product through the cPanel Market. | `Market/SSLWebInstall.*.tmpl` |
...
| _Reconfigure CalDAV/CardDAV clients_ | The system sends a notification email to all users with [Calendar and Contacts Server (CCS)](https://docs.cpanel.net/knowledge-base/cpanel-developed-plugins/calendar-and-contacts-server/) data. The email tells them to reconfigure their calendars and contacts applications for the [replacement of CCS by the `cpdavd` service](https://docs.cpanel.net/knowledge-base/webmail/cpdavd-service-replaces-ccs-plugin/). | `Mail/ReconfigureCalendars.*.tmpl` |
...
| _Service SSL Certificate Expires Soon_ | A service-level SSL certificate will expire within 20 days. | `Check/SSLCertExpiresSoon.*.tmpl` |
| _SSHD Configuration Error_ | The system detects an attempt to change the `sshd_config` file. | `SSHD/ConfigError.*.tmpl` |
| _SSL certificates expiring_ | An account’s **non-AutoSSL** SSL certificate expires soon. | `SSL/CertificateExpiring.*.tmpl` |
