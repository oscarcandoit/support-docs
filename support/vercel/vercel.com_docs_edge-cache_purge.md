---
url: "https://vercel.com/docs/edge-cache/purge"
...
Cache purgingis availableon [all plans](https://vercel.com/docs/plans)

Those with the[owner](https://vercel.com/docs/rbac/access-roles#owner-role) and [member](https://vercel.com/docs/rbac/access-roles#member-role)rolescan access this feature

Learn how to [invalidate and purge](https://vercel.com/docs/edge-cache/purge#programmatically-purging-vercel-cache) cached content on Vercel's CDN, including cache keys and manual purging options.

## [Cache keys](https://vercel.com/docs/edge-cache/purge\#cache-keys)
...
Since each deployment has a different cache key, you can [promote a new deployment](https://vercel.com/docs/deployments/promoting-a-deployment) to production without affecting the cache of the previous deployment.

The cache key for Image Optimization behaves differently for [static\\
images](https://vercel.com/docs/image-optimization#local-images-cache-key) and [remote\\
images](https://vercel.com/docs/image-optimization#remote-images-cache-key).

## [Understanding cache purging](https://vercel.com/docs/edge-cache/purge\#understanding-cache-purging)

### [Invalidating the cache](https://vercel.com/docs/edge-cache/purge\#invalidating-the-cache)
...
### [Deleting the cache](https://vercel.com/docs/edge-cache/purge\#deleting-the-cache)
...
### [Cache tags](https://vercel.com/docs/edge-cache/purge\#cache-tags)

Cache tags are case-sensitive. The tags `product` and `Product` are treated as different tags.

#### [Cache tag scope](https://vercel.com/docs/edge-cache/purge\#cache-tag-scope)
...
- [@vercel/functions](https://vercel.com/docs/functions/functions-api-reference/vercel-functions-package): Use helper methods like `invalidateByTag()`, `dangerouslyDeleteByTag()`, `invalidateBySrcImage()`, or `dangerouslyDeleteBySrcImage()`
- [Vercel CLI](https://vercel.com/docs/cli/cache): Use the `vercel cache` command with `--tag` or `--srcimg` options
- [REST API](https://vercel.com/docs/rest-api/reference/endpoints/edge-cache/invalidate-by-tag): Make direct API calls to the edge cache endpoint
...
5. In the dialog, you'll see a dropdown with two options:
   ...
   - Source Image: Purge [Image Optimization](https://vercel.com/docs/image-optimization) transformed images based on the original source image URL.
...
Purge is not the same as creating a new deployment because it will also purge
...
## [Limits](https://vercel.com/docs/edge-cache/purge\#limits)
