...
## [Use cases](https://vercel.com/docs/ai/replicate\#use-cases)
...
### [Available models](https://vercel.com/docs/ai/replicate\#available-models)
...
## [Getting started](https://vercel.com/docs/ai/replicate\#getting-started)

The Vercel Replicate integration can be accessed through the AI tab on your [Vercel dashboard](https://vercel.com/dashboard).

### [Prerequisites](https://vercel.com/docs/ai/replicate\#prerequisites)
...
### [Add the provider to your project](https://vercel.com/docs/ai/replicate\#add-the-provider-to-your-project)

#### [Using the dashboard](https://vercel.com/docs/ai/replicate\#using-the-dashboard)

01. Navigate to the AI tab in your [Vercel dashboard](https://vercel.com/dashboard)
02. Select Replicate from the list of providers, and press Add
03. Review the provider information, and press Add Provider
04. You can now select which projects the provider will have access to. You can choose from All Projects or Specific Projects
    - If you select Specific Projects, you'll be prompted to select the projects you want to connect to the provider. The list will display projects associated with your scoped team
    - Multiple projects can be selected during this step
05. Select the Connect to Project button
06. You'll be redirected to the provider's website to complete the connection process
07. Once the connection is complete, you'll be redirected back to the Vercel dashboard, and the provider integration dashboard page. From here you can manage your provider settings, view usage, and more
08. Pull the environment variables into your project using [Vercel CLI](https://vercel.com/docs/cli/env)
...
09. Install the providers package
...
10. Connect your project using the code below:
    ...
    4import Replicate from 'replicate';
    ...
    10// In production and preview deployments (on Vercel), the VERCEL_URL environment variable is set.

    11// In development (on your local machine), the NGROK_HOST environment variable is set.

    12const WEBHOOK_HOST = process.env.VERCEL_URL

    13  ? `https://${process.env.VERCEL_URL}`

    14  : process.env.NGROK_HOST;

    15


    16export async function POST(request) {

    17  if (!process.env.REPLICATE_API_TOKEN) {

    18    throw new Error(

    19      'The REPLICATE_API_TOKEN environment variable is not set. See README.md for instructions on how to set it.',
    ...
    23  const { prompt } = await request.json();
    ...
    26    version: '8beff3369e81422112d93b89ca01426147de542cd4684c244b673b105188fe5f',
    ...
    31    options.webhook = `${WEBHOOK_HOST}/api/webhooks`;

    32    options.webhook_events_filter = ['start', 'completed'];
    ...
    35  // A prediction is the result you get when you run a model, including the input, output, and other details

    36  const prediction = await replicate.predictions.create(options);

    37


    38  if (prediction?.error) {

    39    return NextResponse.json({ detail: prediction.error }, { status: 500 });
    ...
    42  return NextResponse.json(prediction, { status: 201 });
    ...
    45// app/api/predictions/[id]/route.ts
    ...
    48import Replicate from 'replicate';

    49


    50const replicate = new Replicate({

    51  auth: process.env.REPLICATE_API_TOKEN,
    ...
    54// Poll for the prediction's status

    55export async function GET(request, { params }) {

    56  const { id } = params;

    57  const prediction = await replicate.predictions.get(id);

    58


    59  if (prediction?.error) {

    60    return NextResponse.json({ detail: prediction.error }, { status: 500 });
    ...
    63  return NextResponse.json(prediction);
...
## [Deploy a template](https://vercel.com/docs/ai/replicate\#deploy-a-template)

You can deploy a template to Vercel that uses a pre-trained model from Replicate:

[![Scribble Diffusion](https://vercel.com/vc-ap-vercel-docs/_next/image?url=https%3A%2F%2Fimages.ctfassets.net%2Fe5382hct74si%2F21QBhvRvCXdu6KZQGQ7MMQ%2F11fb6b76593cf66aea0f8a0f9a5c38b6%2FCleanShot_2023-02-16_at_22.53.58.png&w=3840&q=75)\\
\\
Scribble Diffusion\\
\\
Turn your rough sketch into a refined image using AI. Powered by Replicate and ControlNet.](https://vercel.com/templates/next.js/scribble-diffusion) [![Inpainter with Stable Diffusion](https://vercel.com/vc-ap-vercel-docs/_next/image?url=https%3A%2F%2Fimages.ctfassets.net%2Fe5382hct74si%2FBHQac6GdSOrGw5xvKBdcR%2F2f0613991a93e5e8f6bc53a434c1f5f7%2FCleanShot_2022-12-09_at_13.19.13.png&w=3840&q=75)\\
\\
Inpainter with Stable Diffusion\\
\\
Next.js application for inpainting with Stable Diffusion using the Replicate API.](https://vercel.com/templates/next.js/inpainter-stable-diffusion) [![Paint by Text](https://vercel.com/vc-ap-vercel-docs/_next/image?url=https%3A%2F%2Fimages.ctfassets.net%2Fe5382hct74si%2F4MJWAAgQaFRRb0vVgTg4XL%2F4ef163972253a781ebf9596e60837885%2Fpaintbytext.png&w=3840&q=75)\\
\\
Paint by Text\\
\\
...
## [More resources](https://vercel.com/docs/ai/replicate\#more-resources)
