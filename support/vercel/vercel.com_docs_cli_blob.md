...
- [`list`](https://vercel.com/docs/cli/blob#list-ls) \- List all files in the Blob store
- [`put`](https://vercel.com/docs/cli/blob#put) \- Upload a file to the Blob store
- [`del`](https://vercel.com/docs/cli/blob#del) \- Delete a file from the Blob store
- [`copy`](https://vercel.com/docs/cli/blob#copy-cp) \- Copy a file in the Blob store
- [`store add`](https://vercel.com/docs/cli/blob#store-add) \- Add a new Blob store
- [`store remove`](https://vercel.com/docs/cli/blob#store-remove-rm) \- Remove a Blob store
- [`store get`](https://vercel.com/docs/cli/blob#store-get) \- Get a Blob store

For authentication, the CLI reads the `BLOB_READ_WRITE_TOKEN` value from your env file or you can use the [`--rw-token` option](https://vercel.com/docs/cli/blob#rw-token).
...
The following [global options](https://vercel.com/docs/cli/global-options) can be passed when using the `vercel blob` command:
