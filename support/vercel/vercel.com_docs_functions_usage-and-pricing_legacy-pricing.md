...
Legacy Billing Model: This page describes the legacy billing model and
relates to functions which **do not** use Fluid Compute. All new projects
use [Fluid Compute](https://vercel.com/docs/fluid-compute) by default, which bills separately
...
See [viewing function usage](https://vercel.com/docs/functions/usage-and-pricing/legacy-pricing#viewing-function-usage) for more information on how to track your usage.

## [Pricing](https://vercel.com/docs/functions/usage-and-pricing/legacy-pricing\#pricing)

This information relates to functions which **do not** use Fluid Compute.
Fluid Compute is the default for all new functions. To learn about pricing for
functions that use Fluid Compute, see
[Pricing](https://vercel.com/docs/functions/usage-and-pricing).

The following table outlines the price for functions which do not use [Fluid Compute](https://vercel.com/docs/fluid-compute).
...
### [Hobby](https://vercel.com/docs/functions/usage-and-pricing/legacy-pricing\#hobby)
...
When your [Hobby team](https://vercel.com/docs/plans/hobby) is set to paused, it remains in this state indefinitely unless you take action. This means all new and existing [deployments](https://vercel.com/docs/deployments) will be paused.

If you have reached this state, your application is likely a good candidate
for a [Pro account](https://vercel.com/docs/plans/pro).
...
If your Hobby team is paused, you won't have the option to initiate a [Pro trial](https://vercel.com/docs/plans/pro-plan/trials). Instead, you can set up a Pro team:
...
### [Pro](https://vercel.com/docs/functions/usage-and-pricing/legacy-pricing\#pro)

For teams on a Pro trial, the [trial will end](https://vercel.com/docs/plans/pro/trials#post-trial-decision) when your team reaches the [trial limits](https://vercel.com/docs/plans/pro/trials#trial-limitations).
...
### [Enterprise](https://vercel.com/docs/functions/usage-and-pricing/legacy-pricing\#enterprise)

Enterprise agreements provide custom usage and pricing for Vercel Functions, including:

- Custom [execution units](https://vercel.com/docs/functions/runtimes/edge/edge-functions#managing-execution-units)
- Increased [maximum duration](https://vercel.com/docs/functions/configuring-functions/duration) up to 900 seconds
- Multi-region deployments
- [Vercel Function failover](https://vercel.com/docs/functions/configuring-functions/region#automatic-failover)

See [Vercel Enterprise plans](https://vercel.com/docs/plans/enterprise) for more information.

## [Viewing Function Usage](https://vercel.com/docs/functions/usage-and-pricing/legacy-pricing\#viewing-function-usage)
...
| --- | --- | --- | --- |
| Function Invocations | The number of times your Functions have been invoked | [Yes](https://vercel.com/docs/pricing#managed-infrastructure-billable-resources) | [Learn More](https://vercel.com/docs/functions/usage-and-pricing/legacy-pricing#optimizing-function-invocations) |
| Function Duration | The time your Vercel Functions have spent responding to requests | [Yes](https://vercel.com/docs/pricing#managed-infrastructure-billable-resources) | [Learn More](https://vercel.com/docs/functions/usage-and-pricing/legacy-pricing#optimizing-function-duration) |
| Throttling | The number of instances where Functions did not execute due to concurrency limits being reached | No | N/A |

## [Managing function invocations](https://vercel.com/docs/functions/usage-and-pricing/legacy-pricing\#managing-function-invocations)

You are charged based on the number of times your [functions](https://vercel.com/docs/functions) are invoked, including both successful and errored invocations, excluding cache hits. The number of invocations is calculated by the number of times your function is called, regardless of the response status code.
...
### [Optimizing function invocations](https://vercel.com/docs/functions/usage-and-pricing/legacy-pricing\#optimizing-function-invocations)

- Use the Projects option to identify which projects have the most invocations and where you can optimize.
- Cache your responses using [edge caching](https://vercel.com/docs/edge-network/caching#using-vercel-functions) and [Cache-Control headers](https://vercel.com/docs/headers#cache-control-header) to reduce the number of invocations and speed up responses for users.
- See [How can I reduce my Serverless Execution usage on Vercel?](https://vercel.com/kb/guide/how-can-i-reduce-my-serverless-execution-usage-on-vercel) for more general information on how to reduce your Vercel functions usage.

## [Managing function duration](https://vercel.com/docs/functions/usage-and-pricing/legacy-pricing\#managing-function-duration)

Legacy Billing Model: This describes the legacy Function duration billing
model based on wall-clock time. For new projects, we recommend [Fluid\\
Compute](https://vercel.com/docs/functions/usage-and-pricing) which bills separately for active
CPU time and provisioned memory time for more cost-effective and transparent
pricing.

You are charged based on the duration your Vercel functions have run. This is sometimes called " [wall-clock time](https://vercel.com/kb/guide/what-are-gb-hrs-for-serverless-function-execution)", which refers to the _actual time_ elapsed during a process, similar to how you would measure time passing on a wall clock. It includes all time spent from start to finish of the process, regardless of whether that time was actively used for processing or spent waiting for a streamed response. Function Duration is calculated in [GB-Hours](https://vercel.com/kb/guide/what-are-gb-hrs-for-serverless-function-execution), which is the memory allocated for each Function in GB x the time in hours they were running.
...
### [Optimizing function duration](https://vercel.com/docs/functions/usage-and-pricing/legacy-pricing\#optimizing-function-duration)

Recommended: Upgrade to Fluid compute

- Enable [Fluid compute](https://vercel.com/docs/fluid-compute) for more cost-effective billing that separates active CPU time from provisioned memory time. This replaces the legacy wall-clock time billing model with transparent, usage-based pricing.
...
- You can adjust the [maximum duration](https://vercel.com/docs/functions/configuring-functions/duration) for your functions to prevent excessive run times.
- To reduce the GB-hours (Execution) of your functions, ensure you are using [edge caching](https://vercel.com/docs/edge-network/caching#using-vercel-functions) and Cache-Control headers. If using [Incremental Static Regeneration](https://vercel.com/docs/incremental-static-regeneration), note that Vercel counts Function invocations on page revalidation towards both GB-hours and [Fast Origin Transfer](https://vercel.com/docs/edge-network/manage-usage#fast-origin-transfer).
- For troubleshooting issues causing functions to run longer than expected or timeout, see [What can I do about Vercel Serverless Functions timing out?](https://vercel.com/kb/guide/what-can-i-do-about-vercel-serverless-functions-timing-out)

## [Throttles](https://vercel.com/docs/functions/usage-and-pricing/legacy-pricing\#throttles)

This counts the number of times that a request to your Functions could not be served because the [concurrency limit](https://vercel.com/docs/functions/concurrency-scaling#automatic-concurrency-scaling) was hit.

While this is not a chargeable metric, it will cause a `503: FUNCTION_THROTTLED` error. To learn more, see [What should I do if I receive a 503 error on Vercel?](https://vercel.com/kb/guide/what-should-i-do-if-i-receive-a-503-error-on-vercel).
