...
Cron Jobsare availableon [all plans](https://vercel.com/docs/plans)

## [Viewing cron jobs](https://vercel.com/docs/cron-jobs/manage-cron-jobs\#viewing-cron-jobs)
...
## [Cron jobs maintenance](https://vercel.com/docs/cron-jobs/manage-cron-jobs\#cron-jobs-maintenance)
...
limits](https://vercel.com/docs/cron-jobs/usage-and-pricing)
...
## [Cron job duration](https://vercel.com/docs/cron-jobs/manage-cron-jobs\#cron-job-duration)

The duration limits for Cron jobs are identical to those of [Vercel Functions](https://vercel.com/docs/functions#limits). See the [`maxDuration`](https://vercel.com/docs/functions/runtimes#max-duration) documentation for more information.
...
## [Cron job error handling](https://vercel.com/docs/cron-jobs/manage-cron-jobs\#cron-job-error-handling)

Vercel will not retry an invocation if a cron job fails. You can check for error [logs](https://vercel.com/docs/runtime-logs) through the View Log button in the Cron Jobs tab.

## [Cron jobs with dynamic routes](https://vercel.com/docs/cron-jobs/manage-cron-jobs\#cron-jobs-with-dynamic-routes)
...
## [Handling nonexistent paths](https://vercel.com/docs/cron-jobs/manage-cron-jobs\#handling-nonexistent-paths)
...
## [Cron jobs and deployments](https://vercel.com/docs/cron-jobs/manage-cron-jobs\#cron-jobs-and-deployments)

Creating a new deployment will not interrupt your running cron jobs; they will continue until they finish.

## [Controlling cron job concurrency](https://vercel.com/docs/cron-jobs/manage-cron-jobs\#controlling-cron-job-concurrency)
...
## [Cron jobs logs](https://vercel.com/docs/cron-jobs/manage-cron-jobs\#cron-jobs-logs)

Cron jobs are logged as function invocations from the Logs tab of your projects [dashboard](https://vercel.com/dashboard). You can view the logs for a cron job from the list on the [Cron jobs settings page](https://vercel.com/docs/cron-jobs/manage-cron-jobs#viewing-cron-jobs) of the project:

1. From the list of cron jobs, select View Logs.
2. This will take you to the [runtime logs](https://vercel.com/docs/runtime-logs#request-path) view with a `requestPath` filter to your cron job such as `requestPath:/api/my-cron-job`.

See [how to view runtime logs](https://vercel.com/docs/runtime-logs#view-runtime-logs) for more information.

Note that when cron jobs respond with a redirect or a cached response, they will not be shown in the logs.

## [Cron jobs accuracy](https://vercel.com/docs/cron-jobs/manage-cron-jobs\#cron-jobs-accuracy)

Hobby users can only create cron jobs with [hourly accuracy](https://vercel.com/docs/cron-jobs/usage-and-pricing#hobby-scheduling-limits). Vercel may invoke these cron jobs at any point within the specified hour to help distribute load across all accounts. For example, an expression like `* 8 * * *` could trigger an invocation anytime between `08:00:00` and `08:59:59`.
...
## [Rollbacks with cron jobs](https://vercel.com/docs/cron-jobs/manage-cron-jobs\#rollbacks-with-cron-jobs)

If you [Instant Rollback](https://vercel.com/docs/instant-rollback) to a previous deployment, active cron jobs will not be updated. They will continue to run as scheduled until they are manually disabled or updated.
