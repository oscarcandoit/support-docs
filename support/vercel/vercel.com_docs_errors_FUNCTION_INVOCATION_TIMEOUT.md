...
The `FUNCTION_INVOCATION_TIMEOUT` error occurs when a function invocation takes longer than the [allowed execution time](https://vercel.com/docs/functions/limitations#max-duration). This could be due to an error within the function itself, a slow network call, or an issue with the environment in which the function is running.
...
#### [Troubleshoot](https://vercel.com/docs/errors/FUNCTION_INVOCATION_TIMEOUT\#troubleshoot)

I'm encountering an error and reviewing the docs at /docs/errors/FUNCTION\_INVOCATION\_TIMEOUT to understand what's happening.
...
For more information on Vercel Functions timeouts, see [What can I do about Vercel Serverless Functions timing out?](https://vercel.com/kb/guide/what-can-i-do-about-vercel-serverless-functions-timing-out)
