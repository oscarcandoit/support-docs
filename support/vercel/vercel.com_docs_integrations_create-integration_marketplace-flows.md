---
url: "https://vercel.com/docs/integrations/create-integration/marketplace-flows"
...
As a Vercel integration provider, when you [create a native product integration](https://vercel.com/docs/integrations/marketplace-product), you need to set up the [integration server](https://github.com/vercel/example-marketplace-integration) and use the [Vercel marketplace Rest API](https://vercel.com/docs/integrations/marketplace-api) to manage the interaction between the integration user and your product.
...
## [Create a storage product flow](https://vercel.com/docs/integrations/create-integration/marketplace-flows\#create-a-storage-product-flow)
...
- [Select storage product](https://vercel.com/docs/integrations/create-integration/marketplace-flows#select-storage-product)
- [Select billing plan](https://vercel.com/docs/integrations/create-integration/marketplace-flows#select-billing-plan)
- [Submit store creation](https://vercel.com/docs/integrations/create-integration/marketplace-flows#submit-store-creation)
...
After confirming the plan selection, the integration user is presented with information fields that the integration provider specified in the [metadata schema](https://vercel.com/docs/integrations/marketplace-product#metadata-schema) section of the integration settings. The user updates these fields and submits the form to initiate the creation of the store for this user on the provider platform.
...
## [Connections between Vercel and the provider](https://vercel.com/docs/integrations/create-integration/marketplace-flows\#connections-between-vercel-and-the-provider)

### [Open in Provider button flow](https://vercel.com/docs/integrations/create-integration/marketplace-flows\#open-in-provider-button-flow)
...
Provider IntegrationVercel PlatformVercel DashboardUserProvider IntegrationVercel PlatformVercel DashboardUserOpen Store Detail PageClick "Open in \[Provider\]" ButtonOpens new tab to loginRedirectUriwith code and resource\_id query paramsPOST /v1/integrations/sso/token code200 SsoExchangeResponseLog user inRender Provider Dashboard andtarget resource\_id if specified

### [Provider to Vercel data sync flow](https://vercel.com/docs/integrations/create-integration/marketplace-flows\#provider-to-vercel-data-sync-flow)

This flow happens when a provider edits information about a resource in the provider's system.

Provider IntegrationVercel PlatformUserProvider IntegrationVercel PlatformUserUpdates Resource AttributePOST /v1/installations/:installation\_id/eventsResourceUpdatedEventGET /v1/installations/:id/resources/:id200 GetResourceResponseUpdate Resource

### [Vercel to Provider data sync flow](https://vercel.com/docs/integrations/create-integration/marketplace-flows\#vercel-to-provider-data-sync-flow)
...
### [Rotate credentials in provider flow](https://vercel.com/docs/integrations/create-integration/marketplace-flows\#rotate-credentials-in-provider-flow)
...
Vercel will update the environment variables of projects connected to the
resource but will not automatically redeploy the projects. The user must
redeploy them manually.

## [Flows for the Experimentation category](https://vercel.com/docs/integrations/create-integration/marketplace-flows\#flows-for-the-experimentation-category)

### [Experimentation flow](https://vercel.com/docs/integrations/create-integration/marketplace-flows\#experimentation-flow)

This flow applies to the products in the Experimentation category, enabling providers to display [feature flags](https://vercel.com/docs/feature-flags) in the Vercel dashboard.

Provider IntegrationVercel PlatformVercel DashboardUserProvider IntegrationVercel PlatformVercel DashboardUserCreate FlagPOST /v1/installations/:integrationConfigurationId/resources/:resourceId/experimentation/itemsList FlagsOpen FlagOpens a new tab to loginRedirectUri with code,resource\_id and experimentation\_item\_id query paramsUpdate FlagPATCH /v1/installations/:integrationConfigurationId/resources/:resourceId/experimentation/items/:itemIdDelete FlagDELETE /v1/installations/:integrationConfigurationId/resources/:resourceId/experimentation/items/:itemIdList Flags

### [Experimentation Edge Config Syncing](https://vercel.com/docs/integrations/create-integration/marketplace-flows\#experimentation-edge-config-syncing)

This flow applies to integration products in the Experimentation category. It enables providers to push the necessary configuration data for resolving flags and experiments into an [Edge Config](https://vercel.com/docs/edge-config) on the team's account, ensuring near-instant resolution.

Provider IntegrationVercel PlatformSDKUserProvider IntegrationVercel PlatformSDKUserUpdate Flag targetingPOST /v1/installations/:integrationConfigurationId/resources/:resourceId/experimentation/edge-configRead Flag data through Edge Config
...
Afterward, providers can use the [Edge Config Syncing](https://vercel.com/docs/integrations/create-integration/marketplace-api#push-data-into-a-user-provided-edge-config) endpoint to push their data into the user's Edge Config.
...
## [Resources with Claim Deployments](https://vercel.com/docs/integrations/create-integration/marketplace-flows\#resources-with-claim-deployments)

When a Vercel user claims deployment ownership with the [Claim Deployments feature](https://vercel.com/docs/deployments/claim-deployments), storage integration resources associated with the project can also be transferred. To facilitate this transfer for your storage integration, use the following flows.

### [Provision flow](https://vercel.com/docs/integrations/create-integration/marketplace-flows\#provision-flow)
...
ProviderVercelClaims GeneratorProviderVercelClaims GeneratorPrerequisite: Claims Generator has alreadyinstalled Provider integrationProvision resourceProvision resourceResource provisionedResource provisionedConnect resource to projectConnected

### [Transfer request creation flow](https://vercel.com/docs/integrations/create-integration/marketplace-flows\#transfer-request-creation-flow)
...
ProviderVercelClaims GeneratorProviderVercelClaims GeneratorPrerequisite: Resource provisioned  (either manually or as a result of a Provision Flow)Create project transfer requestCreateResourceTransferProvider Claim ID(unique, generated by provider)Claim Code(unique, generated by Vercel)
...
### [Transfer request accept flow](https://vercel.com/docs/integrations/create-integration/marketplace-flows\#transfer-request-accept-flow)
...
ProviderVercelUserProviderVercelUserPrerequisites: URL includes a claim code User's Vercel team has the Provider's integration installedGET /v1/installations/{installationId}/resource-transfer-requests/{providerClaimId}/verifyPOST /v1/installations/{installationId}/resource-transfer-requests/{providerClaimId}/acceptVisit vercel.com/claim-deployment?code=<claim\_code>and initiate transfer to target teamVerifyResourceTransferVerify Provider Claim ID is still valid(not expired, resources exist, not claimed)Check if transfer will result innew billing plan for target teamNew billing info if anyAcceptResourceTransferTransfer resources from Claims Generator to user(update ownership, link to target installation)Invalidate Provider Claim IDTransfer completeTransfer complete
