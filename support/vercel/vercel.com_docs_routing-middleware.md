...
Routing Middlewareis availableon [all plans](https://vercel.com/docs/plans)

Routing Middleware executes code _before_ a request is processed on a site, and are built on top of [fluid compute](https://vercel.com/docs/fluid-compute). Based on the request, you can modify the response.
...
The default runtime for Routing Middlewares is [Edge](https://vercel.com/docs/functions/runtimes/edge). See [runtime options](https://vercel.com/docs/routing-middleware#runtime-options) for information on how to change the runtime of your Routing Middleware.

## [Creating a Routing Middleware](https://vercel.com/docs/routing-middleware\#creating-a-routing-middleware)

You can use Routing Middleware with [any framework](https://vercel.com/docs/frameworks). To add a Routing Middleware to your app, you need to create a `middleware.ts` file at your project's root directory.
...
[Open inOpen in v0](https://v0.app/chat?q=%20%20Create%20a%20minimal%20example%20app%20for%20this%20code:%20export%20default%20function%20middleware(request:%20Request)%20{%20%20const%20url%20=%20new%20URL(request.url);%20%20%20//%20Redirect%20old%20paths%20%20if%20(url.pathname%20===%20%27/old-page%27)%20{%20%20%20%20return%20new%20Response(null,%20{%20%20%20%20%20%20status:%20302,%20%20%20%20%20%20headers:%20{%20Location:%20%27/new-page%27%20},%20%20%20%20});%20%20}%20%20%20//%20Continue%20to%20next%20handler%20%20return%20new%20Response(%27Hello%20from%20your%20Middleware!%27);}%20In%20your%20response:%20%201.%20Keep%20the%20example%20simple%20and%20focused%20on%20demonstrating%20the%20core%20functionality%20of%20the%20code%20%202.%20DO%20NOT%20include%20unnecessary%20features%20or%20complexity,%20aim%20to%20only%20showcase%20the%20essential%20parts%20%203.%20Show%20the%20code%20in%20a%20way%20that%20is%20easy%20to%20understand%20and%20implement%20%204.%20Provide%20clear%20and%20concise%20explanations%20for%20each%20part%20of%20the%20code)

The `middleware.ts` file should be at the same
level as your `app` or `pages` directory (even if you're using a `src`
directory). See the [Quickstart](https://vercel.com/docs/routing-middleware/getting-started)
guide for more information.

## [Logging](https://vercel.com/docs/routing-middleware\#logging)
...
## [Using a database with Routing Middleware](https://vercel.com/docs/routing-middleware\#using-a-database-with-routing-middleware)

If your Routing Middleware depends on a database far away from one of [our supported regions](https://vercel.com/docs/regions), the overall latency of API requests could be slower than expected, due to network latency while connecting to the database from an edge region. To avoid this issue, use a global database. Vercel has multiple global storage products, including [Edge Config](https://vercel.com/docs/edge-config) and [Vercel Blob](https://vercel.com/docs/storage/vercel-blob). You can also explore the storage category of the [Vercel Marketplace](https://vercel.com/marketplace?category=storage) to learn which option is best for you.

## [Limits on requests](https://vercel.com/docs/routing-middleware\#limits-on-requests)
...
## [Runtime options](https://vercel.com/docs/routing-middleware\#runtime-options)

Routing Middleware is available on the [Node.js](https://vercel.com/docs/functions/runtimes/node-js), [Bun](https://vercel.com/docs/functions/runtimes/bun), and [Edge](https://vercel.com/docs/functions/runtimes/edge) runtimes. The default runtime for Routing Middleware is Edge. You can change the runtime to Node.js by exporting a [`config`](https://vercel.com/docs/routing-middleware/api#config-object) object with a `runtime` property in your `middleware.ts` file.

To use the Bun runtime, set [`bunVersion`](https://vercel.com/docs/project-configuration#bunversion) in your `vercel.json` file and your runtime config to `nodejs`.
...
## [Pricing](https://vercel.com/docs/routing-middleware\#pricing)

Routing Middleware is priced using the [fluid compute](https://vercel.com/docs/fluid-compute) model, which means you are charged by the amount of compute resources used by your Routing Middleware. See the [fluid compute pricing documentation](https://vercel.com/docs/functions/usage-and-pricing) for more information.

## [Observability](https://vercel.com/docs/routing-middleware\#observability)

The [Vercel Observability dashboard](https://vercel.com/docs/observability) provides visibility into your routing middleware usage, including invocation counts and performance metrics. You can get more [insights](https://vercel.com/docs/observability/insights) with [Observability Plus](https://vercel.com/docs/observability/observability-plus):
...
## [More resources](https://vercel.com/docs/routing-middleware\#more-resources)

Learn more about Routing Middleware by exploring the following resources:

- [Getting Started with Routing Middleware](https://vercel.com/docs/routing-middleware/getting-started)
- [Routing Middleware API Reference](https://vercel.com/docs/routing-middleware/api)
- [Fluid compute](https://vercel.com/docs/fluid-compute)
- [Runtimes](https://vercel.com/docs/functions/runtimes)
