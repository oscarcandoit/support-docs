---
title: "URLs"
source: "https://docs.virustotal.com/reference/url-object"
scraped_at: "2026-02-08T06:35:51.585Z"
---
...
In addition to the previously described attributes, URL objects contain relationships with other objects in our dataset that can be retrieved as explained in the [Relationships](/reference/relationships) section. The available relationships are described bellow.

| Relationship | Description | Accessibility | Return object type |
| --- | --- | --- | --- |
| analyses | Analyses for the URL. | VT Enterprise users only. | List of [Analyses](/reference/analyses-object). |
| comments | Community posted comments about the URL. | Everyone. | List of [Comments](/reference/comments). |
| communicating_files | Files that communicate with a given URL when they&#x27;re executed. | VT Enterprise users only. | List of [Files](/reference/files). |
| contacted_domains | Domains from which the URL loads some kind of resource. | VT Enterprise users only. | List of [Domains](/reference/domains-object). |
| contacted_ips | IPs from which the URL loads some kind of resource. | VT Enterprise users only. | List of [IP addresses](/reference/ip-object). |
| downloaded_files | Files downloaded from the URL. | VT Enterprise users only. | List of [Files](/reference/files). |
| graphs | Graphs including the URL. | Everyone. | List of [Graphs](/reference/graph-object). |
| last_serving_ip_address | Last IP address that served the URL. | Everyone. | A single [IP address](/reference/ip-object). |
| network_location | Domain or IP for the URL. | Everyone. | A single [IP address](/reference/ip-object) or [Domain](/reference/domains-object). |
| referrer_files | Files containing the URL. | VT Enterprise users only. | A list of [Files](/reference/files). |
| referrer_urls | URLs referring the URL. | VT Enterprise users only. | A list of [URLs](/reference/url-object). |
| redirecting_urls | URLs that redirected to the given URL. | VT Enterprise users only. | A list of [URLs](/reference/url-object). |
| redirects_to | URLs that the URL redirects to. | VT Enterprise users only. | A list of [URLs](/reference/url-object). |
| related_comments | Community posted comments in the URL&#x27;s related objects. | Everyone. | A list of [Comments](/reference/comments). |
| related_references | References related to the URL. | VT Enterprise users only. | A list of [References](/reference/references). |
| related_threat_actors | Threat actors related to the URL. | VT Enterprise users only. | A list of [Threat Actors](/reference/threat-actors-object). |
| submissions | URL&#x27;s submissions. | VT Enterprise users only. | A list of [Submissions](/reference/submission-object). |