---
title: "🔀 network_location"
source: "https://docs.virustotal.com/reference/url-object-network-location"
scraped_at: "2026-02-08T06:35:52.205Z"
---

The *network_location* relationship returns the Domain or IP address object corresponding the URL&#x27;s network location. For example, if the URL is `http://foo.com/some_path.html`, its network location would be `foo.com` (a domain); and if the URL is `http://1.2.3.4/some_path.html`, its network location would be `1.2.3.4` (an IP address).

The relationship can be retrieved using the [relationships API endpoint](/reference/urls-relationships) and the response contains either a [Domain](/reference/domains-object) object or an [IP address](/reference/ip-object) object.

/urls/{url_id}/network_location Domain exampleIP address example
```
`{
 "data": {
 <MDXishSnakeCase0>
