---
url: "https://docs.gravityforms.com/gform_incomplete_submission_pre_save/"
title: "gform_incomplete_submission_pre_save - Gravity Forms Documentation"
---
...
# gform\_incomplete\_submission\_pre\_save

- [Description](https://docs.gravityforms.com/gform_incomplete_submission_pre_save/#h-description)
- [Usage](https://docs.gravityforms.com/gform_incomplete_submission_pre_save/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_incomplete_submission_pre_save/#h-parameters)
- [Examples](https://docs.gravityforms.com/gform_incomplete_submission_pre_save/#h-examples)
- [Placement](https://docs.gravityforms.com/gform_incomplete_submission_pre_save/#h-placement)
- [Since](https://docs.gravityforms.com/gform_incomplete_submission_pre_save/#h-since)
- [Source Code](https://docs.gravityforms.com/gform_incomplete_submission_pre_save/#h-source-code)
...
- **$submission\_json** [array](https://docs.gravityforms.com/array)
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6 | `*    @type``array``$submitted_values``The submitted values.`<br>`*    @type``array``$partial_entry``The draft entry created from the submitted values.`<br>`*    @type null|``array``$field_values``The dynamic population field values.`<br>`*    @type int``$page_number``The forms current page number.`<br>`*    @type``array``$files``The uploaded file properties.`<br>`*    @type string``$gform_unique_id``The unique id``for``this submission.` |

- **$resume\_token** [string](https://docs.gravityforms.com/string)
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8 | `add_filter(``'gform_incomplete_submission_pre_save'``,``'modify_incomplete_submission'``, 10, 3 );`<br>`function``modify_incomplete_submission(``$submission_json``,``$resume_token``,``$form``){`<br>```//change the user first name to Test in the saved data`<br>```$updated_json``= json_decode(``$submission_json``);`<br>```$updated_json``->submitted_values->{``'1'``}->{``'1.3'``} =``'Test'``;`<br>```$submission_json``= json_encode(``$updated_json``);`<br>```return``$submission_json``;`<br>`}` |
...
This filter is located in GFFormsModel::filter\_draft\_submission\_pre\_save() in _forms\_model.php_.
...
[Algolia](https://docs.gravityforms.com/gform_incomplete_submission_pre_save/# "Algolia")
