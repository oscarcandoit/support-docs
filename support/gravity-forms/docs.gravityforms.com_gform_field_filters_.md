---
url: "https://docs.gravityforms.com/gform_field_filters/"
title: "gform_field_filters - Gravity Forms Documentation"
---
...
# gform\_field\_filters

- [Description](https://docs.gravityforms.com/gform_field_filters/#h-description)
- [Usage](https://docs.gravityforms.com/gform_field_filters/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_field_filters/#h-parameters)
- [Examples](https://docs.gravityforms.com/gform_field_filters/#h-examples)
- [1\. Add a custom payment status](https://docs.gravityforms.com/gform_field_filters/#h-1-add-a-custom-payment-status)
- [2\. Add an operator to a specific field](https://docs.gravityforms.com/gform_field_filters/#h-2-add-an-operator-to-a-specific-field)
- [Placement](https://docs.gravityforms.com/gform_field_filters/#h-placement)
- [Since](https://docs.gravityforms.com/gform_field_filters/#h-since)
- [Source Code](https://docs.gravityforms.com/gform_field_filters/#h-source-code)

## Description

The “gform\_field\_filters” filter in Gravity Forms enables the filter settings to be overridden for the form fields, entry properties, and entry meta.

## Usage

|     |     |
| --- | --- |
| 1 | `add_filter(``'gform_field_filters'``,``'your_function_name'``, 10, 2 );` |

## Parameters

- **$field\_filters** [array](https://docs.gravityforms.com/array)
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>17<br>18<br>19<br>20<br>21 | `add_filter(``'gform_field_filters'``,``'update_filters'``, 10, 2 );`<br>`function``update_filters(``$field_filters``,``$form``){`<br>```foreach``(``$field_filters``as``&``$filter``){`<br>```if``(``$filter``[``'key'``] ==``'payment_status'``){`<br>```//add custom payment status`<br>```$values``=``$filter``[``'values'``];`<br>```$status_exists``= false;`<br>```foreach``(``$values``as``&``$value``){`<br>```if``(``$value``[``'text'``] ==``'Test Status'``){`<br>```$status_exists``= true;`<br>```}`<br>```}`<br>```if``( !``$status_exists``){`<br>```//add status`<br>```array_push``(``$values``,``array``(``'text'``=>``'Test Status'``,``'value'``=>``'Test Status'``) );`<br>```$filter``[``'values'``] =``$values``;`<br>```}`<br>```}`<br>```}`<br>```return``$field_filters``;`<br>`}` |

### 2\. Add an operator to a specific field

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10 | `add_filter(``'gform_field_filters'``,``function``(``$field_filters``,``$form``) {`<br>```foreach``(``$field_filters``as``$filter``) {`<br>```if``( rgar(``$filter``,``'key'``) ==``'2'``) {`<br>```$filter``[``'operators'``][] =``'isnot'``;`<br>```break``;`<br>```}`<br>```}`<br>```return``$field_filters``;`<br>`}, 10, 2 );` |
...
This filter is located in GFCommon::get\_field\_filter\_settings() in _common.php_
...
[Algolia](https://docs.gravityforms.com/gform_field_filters/# "Algolia")
