---
url: "https://docs.gravityforms.com/gform_abort_submission_with_confirmation/"
title: "Gform Abort Submission with Confirmation: Explained"
---
...
- [Description](https://docs.gravityforms.com/gform_abort_submission_with_confirmation/#description)
- [Usage](https://docs.gravityforms.com/gform_abort_submission_with_confirmation/#usage)
- [Parameters](https://docs.gravityforms.com/gform_abort_submission_with_confirmation/#parameters)
- [Examples](https://docs.gravityforms.com/gform_abort_submission_with_confirmation/#examples)
- [Check a field value](https://docs.gravityforms.com/gform_abort_submission_with_confirmation/#h-check-a-field-value)
- [Run anti-spam checks before entry save](https://docs.gravityforms.com/gform_abort_submission_with_confirmation/#h-run-anti-spam-checks-before-entry-save)
- [Placement](https://docs.gravityforms.com/gform_abort_submission_with_confirmation/#h-placement)
- [Since](https://docs.gravityforms.com/gform_abort_submission_with_confirmation/#since)
- [Source Code](https://docs.gravityforms.com/gform_abort_submission_with_confirmation/#source-code)
...
To limit the scope of your function to a specific form, append the form id to the end of the hook name. (format: gform\_abort\_submission\_with\_confirmation\_FORMID)
...
- **$do\_abort** [boolean](https://docs.gravityforms.com/boolean/)
...
|     |
| --- |
| `add_filter(``'gform_abort_submission_with_confirmation'``,``function``(``$do_abort``) {`<br>```// If submission is already marked to be aborted early, don't change it.`<br>```if``(``$do_abort``) {`<br>```return``true;`<br>```}`<br>```// Abort if input 1 contains 'i am a spam'.`<br>```$is_spam``=``strpos``( rgpost(``'input_1'``),``'i am a spam'``) !== false;`<br>```return``$is_spam``;`<br>`} );` |

### Run anti-spam checks before entry save

This example shows how you can run anti-spam checks, such as Akismet, before the entry has been saved. This will also trigger the [gform\_entry\_is\_spam](https://docs.gravityforms.com/gform_entry_is_spam/) filter.
...
|     |
| --- |
| `add_filter(``'gform_abort_submission_with_confirmation'``,``'abort_gf_submission_if_spam'``, 10, 2 );`<br>`function``abort_gf_submission_if_spam(``$do_abort``,``$form``) {`<br>```// If submission is already marked to be aborted early, don't change it.`<br>```if``(``$do_abort``) {`<br>```return``true;`<br>```}`<br>```// Create an entry like array of submitted values without saving them.`<br>```$entry``= GFFormsModel::get_current_lead();`<br>```// Abort if spam.`<br>```return``GFCommon::is_spam_entry(``$entry``,``$form``);`<br>`}` |
...
[Algolia](https://docs.gravityforms.com/gform_abort_submission_with_confirmation/# "Algolia")
