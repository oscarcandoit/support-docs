---
url: "https://docs.gravityforms.com/how-to-chain-forms-using-shortcodes/"
title: "How to Chain Forms Using Shortcodes - Gravity Forms Documentation"
---
...
- [Summary](https://docs.gravityforms.com/how-to-chain-forms-using-shortcodes/#h-summary)
- [Steps](https://docs.gravityforms.com/how-to-chain-forms-using-shortcodes/#h-steps)
- [Populating Fields](https://docs.gravityforms.com/how-to-chain-forms-using-shortcodes/#h-populating-fields)
- [Caveats](https://docs.gravityforms.com/how-to-chain-forms-using-shortcodes/#h-caveats)
...
![Gravity Forms - Chain Forms - Shortcode](https://docs.gravityforms.com/wp-content/uploads/2024/09/image-37.png)

|     |     |
| --- | --- |
| 1 | `[gravityform id=``"123"``title=``"false"``description=``"false"``ajax=``"true"``field_values=``"param_email={:2}"``theme=``"orbital"``]` |

**Note:** The **`ajax`** attribute must be included and set to **true** for the shortcode to the second form. If not, upon submission of the second form, the first form will load again.

**Note:** Styles from Form 1 can impact Form 2. Set the `theme` attribute in the shortcode for Form 2 to match the theme used in Form 1. Refer to [this](https://docs.gravityforms.com/gravity-forms-form-shortcode/#h-parameters) article for more information about the shortcode attributes.

### Populating Fields

To transfer data and populate fields on the second form, you need to include the `field_values` attribute in the shortcode. Make sure that the parameter names used match those created in the second form. The values will depend on the field IDs assigned to the second form. If you’re unsure about the field IDs or parameter names, you can complete this step after the second form has been set up.

Refer to [this](https://docs.gravityforms.com/using-dynamic-population/) article for information on the `field_values` attribute. Additionally, [this](https://docs.gravityforms.com/confirmation-query-string-populate-form/) article provides details about transferring data between forms using query strings.

1. Create a page for the first form.
2. Add the shortcode for the form to the page. The most important part here is that the **ajax** attribute is set to **false**, or not included because it defaults to false. If **ajax** is set to **true**, then submitting the second form will cause the first form to be loaded again.

|     |     |
| --- | --- |
| 1 | `[gravityform id="123" title="false" description="false" ajax="false" field_values="param_email={:2}" theme="orbital"]` |

4. Create a page for the first form.

3. Create the second form. If there is any data that should carry over from the first form to be displayed in fields on the second form, set the fields on the second form to **Allow field to be populated dynamically** and enter a **Parameter name**.

[![Chain Forms - Dynamic Population](https://docs.gravityforms.com/wp-content/uploads/2021/07/CleanShot-2021-07-06-at-12.50.12.png)](https://docs.gravityforms.com/wp-content/uploads/2021/07/CleanShot-2021-07-06-at-12.50.12.png)
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4 | `<script type=``"text/javascript"``>`<br>`var``$ = jQuery.noConflict();`<br>`$(``"h1"``).text(``''``);`<br>`</script>` |

## Caveats

It is not possible to chain a form in the confirmation message of a [Conversational Form](https://docs.gravityforms.com/using-the-conversational-forms-add-on/). As a workaround, you can:

1. Embed Form 2 on a page and select it as the page in the Form 1 confirmation.
...
[Algolia](https://docs.gravityforms.com/how-to-chain-forms-using-shortcodes/# "Algolia")
