---
url: "https://docs.gravityforms.com/gform_stripe_webhook/"
title: "gform_stripe_webhook - Gravity Forms Documentation"
---
...
# gform\_stripe\_webhook

- [Description](https://docs.gravityforms.com/gform_stripe_webhook/#h-description)
- [Usage](https://docs.gravityforms.com/gform_stripe_webhook/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_stripe_webhook/#h-parameters)
- [Examples](https://docs.gravityforms.com/gform_stripe_webhook/#h-examples)
- [1\. Add a new event type](https://docs.gravityforms.com/gform_stripe_webhook/#h-1-add-a-new-event-type)
- [Placement](https://docs.gravityforms.com/gform_stripe_webhook/#h-placement)
- [Since](https://docs.gravityforms.com/gform_stripe_webhook/#h-since)
- [Source Code](https://docs.gravityforms.com/gform_stripe_webhook/#h-source-code)

## Description

The gform\_stripe\_webhook filter can be used to add support for performing actions on additional Stripe events. The following event types are supported by default:
...
See the [Stripe API Reference](https://stripe.com/docs/api#event_types) for a full list of event types for which webhooks are sent.
...
- [gform\_post\_payment\_completed](https://docs.gravityforms.com/gform_post_payment_completed/) – Triggered after a payment is completed.
- [gform\_post\_payment\_refunded](https://docs.gravityforms.com/gform_post_payment_refunded/) – Triggered after a payment is refunded.
- [gform\_post\_payment\_transaction](https://docs.gravityforms.com/gform_post_payment_transaction/) – Triggered after a payment transaction is added to a Gravity Forms entry.
- [gform\_post\_subscription\_started](https://docs.gravityforms.com/gform_post_subscription_started/) – Triggered after a subscription is successfully created.
- [gform\_subscription\_canceled](https://docs.gravityforms.com/gform_subscription_canceled/) – Triggered after a subscription is canceled.
- [gform\_post\_add\_subscription\_payment](https://docs.gravityforms.com/gform_post_add_subscription_payment/) – Triggered after a subscription/recurring payment is completed.
- [gform\_post\_fail\_subscription\_payment](https://docs.gravityforms.com/gform_post_fail_subscription_payment/) – Triggered after a subscription/recurring payment fails.
- [gform\_post\_payment\_action](https://docs.gravityforms.com/gform_post_payment_action/) – Generic hook that is triggered after any payment and subscription related action is performed.
...
|     |
| --- |
| `$action``=``array``(`<br>```'type'``=>``''``,`<br>```'amount'``=>``''``,`<br>```'transaction_type'``=>``''``,`<br>```'transaction_id'``=>``''``,`<br>```'subscription_id'``=>``''``,`<br>```'entry_id'``=>``''``,`<br>```'payment_status'``=>``''``,`<br>```'note'``=>``''``,`<br>`);` |

- **$event** [\\Stripe\\Event](https://stripe.com/docs/api#event_object)
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>17<br>18<br>19<br>20<br>21<br>22 | `add_filter(``'gform_stripe_webhook'``,``'stripe_webhook_custom_action'``, 10, 2 );`<br>`function``stripe_webhook_custom_action(``$action``,``$event``) {`<br>```$type``= rgar(``$event``,``'type'``);`<br>```switch``(``$type``) {`<br>```case``'charge.failed'``:`<br>```$action``[``'transaction_id'``] = rgars(``$event``,``'data/object/id'``);`<br>```$entry_id``= gf_stripe()->get_entry_by_transaction_id(``$action``[``'transaction_id'``] );`<br>```if``( !``$entry_id``) {`<br>```return``new``WP_Error(``'entry_not_found'``, sprintf(``'Entry for transaction id: %s was not found. Webhook cannot be processed.'``,``$action``[``'transaction_id'``] ) );`<br>```}`<br>``<br>```$entry``= GFAPI::get_entry(``$entry_id``);`<br>``<br>```$action``[``'entry_id'``] =``$entry_id``;`<br>```$action``[``'type'``]     =``'fail_payment'``;`<br>```$action``[``'amount'``]   = gf_stripe()->get_amount_import( rgars(``$event``,``'data/object/amount'``),``$entry``[``'currency'``] );`<br>```break``;`<br>```}`<br>```return``$action``;`<br>`}` |
...
This hook is located in GFStripe::callback() in _class-gf-stripe.php_.
...
[Algolia](https://docs.gravityforms.com/gform_stripe_webhook/# "Algolia")
