---
url: "https://docs.gravityforms.com/gform_stripe_payment_element_updated_payment_information/"
title: "gform_stripe_payment_element_updated_payment_information - Gravity Forms Documentation"
---
...
- [Description](https://docs.gravityforms.com/gform_stripe_payment_element_updated_payment_information/#h-description)
- [Usage](https://docs.gravityforms.com/gform_stripe_payment_element_updated_payment_information/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_stripe_payment_element_updated_payment_information/#h-parameters)
- [Example](https://docs.gravityforms.com/gform_stripe_payment_element_updated_payment_information/#h-example)
- [Since](https://docs.gravityforms.com/gform_stripe_payment_element_updated_payment_information/#h-since)
- [Placement](https://docs.gravityforms.com/gform_stripe_payment_element_updated_payment_information/#h-placement)
- [Source Code](https://docs.gravityforms.com/gform_stripe_payment_element_updated_payment_information/#h-source-code)

## Description

Allows users to filter the payment information of the payment element before calling `elements.update()`. This is useful when altering payment details like currency and amount. For property details, see Stripe documentation for [elements.update(options)](https://docs.stripe.com/js/elements_object/update#elements_update-options).
...
|     |
| --- |
| `gform.addFilter(``'gform_stripe_payment_element_updated_payment_information'``,``function``( paymentInformation,feedId,formId ){`<br>```// do something with the information`<br>```return``paymentInformation;`<br>`} );` |

## Parameters

- **$paymentInformation** [javascript object](https://docs.gravityforms.com/javascript-object/)

The object that contains the updated payment information.
- **$feedId** [string](https://docs.gravityforms.com/integer)
...
|     |
| --- |
| `add_filter(``'gform_currency_pre_save_entry'``,``function``(``$currency``,``$form``) {`<br>```if``( rgar(``$form``,``'id'``) == 5 ) {`<br>```return``'EUR'``;`<br>```}`<br>```return``$currency``;`<br>`}, 10, 2);`<br>`add_filter(``'gform_stripe_charge_pre_create'``,``function``(``$charge``,``$feed``,``$submission_data``,``$form``,``$entry``) {`<br>```if``( rgar(``$form``,``'id'``) == 5 ) {`<br>```$charge``[``'currency'``] =``'eur'``;`<br>```}`<br>```return``$charge``;`<br>`}, 10, 5);`<br>`add_action(``'wp_footer'``,``function``() {`<br>`?>`<br>`<script type=``"text/javascript"``>`<br>```jQuery( document ).ready(``function``() {`<br>```gform.addFilter(``'gform_stripe_payment_element_updated_payment_information'``,``function``( paymentInformation, feedId, formId ) {`<br>```if``( formID == 5 ) {`<br>```paymentInformation.currency =``'eur'``;`<br>```}`<br>```return``paymentInformation;`<br>```} );`<br>```});`<br>`</script>`<br>`<?php`<br>`});` |
...
This action hook is located in _js/src/payment-element/stripe-payments-handler.js_.
...
[Algolia](https://docs.gravityforms.com/gform_stripe_payment_element_updated_payment_information/# "Algolia")
