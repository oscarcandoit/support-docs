---
url: "https://docs.gravityforms.com/gform_entry_field_value/"
title: "gform_entry_field_value - Gravity Forms Documentation"
---
...
- [Description](https://docs.gravityforms.com/gform_entry_field_value/#h-description)
- [Usage](https://docs.gravityforms.com/gform_entry_field_value/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_entry_field_value/#h-parameters)
- [Examples](https://docs.gravityforms.com/gform_entry_field_value/#h-examples)
- [1\. Display category name](https://docs.gravityforms.com/gform_entry_field_value/#h-1-display-category-name)
- [2\. Display choice label](https://docs.gravityforms.com/gform_entry_field_value/#h-2-display-choice-label)
- [3\. File Uploads](https://docs.gravityforms.com/gform_entry_field_value/#h-3-file-uploads)
- [Additional Notes](https://docs.gravityforms.com/gform_entry_field_value/#h-additional-notes)
- [Placement](https://docs.gravityforms.com/gform_entry_field_value/#h-placement)
- [Source Code](https://docs.gravityforms.com/gform_entry_field_value/#h-source-code)
...
|     |     |
| --- | --- |
| 1 | `add_filter(``'gform_entry_field_value'``,``'category_names'``, 10, 4 );` |
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15 | `add_filter(``'gform_entry_field_value'``,``'category_names'``, 10, 4 );`<br>`function``category_names(``$value``,``$field``,``$lead``,``$form``) {`<br>```if``(``$form``[``'id'``] != 104 ||``$field``->id != 3 )`<br>```return``$value``;`<br>```$newvalue``= *;`<br>```$categories``=``explode``(``','``,``$value``);`<br>```foreach``(``$categories``as``$category``) {`<br>```$new_value``.=``'<li>'``. get_cat_name(``$category``) .``'</li>'``;`<br>```}`<br>```return``'<ul>'``.``$new_value``.``'</ul>'``;`<br>`}` |
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>17 | `add_filter(``'gform_entry_field_value'``,``function``(``$value``,``$field``,``$entry``,``$form``) {`<br>```$classes``=``array``(`<br>```'GF_Field_Checkbox'``,`<br>```'GF_Field_MultiSelect'``,`<br>```'GF_Field_Radio'``,`<br>```'GF_Field_Select'``,`<br>```);`<br>```foreach``(``$classes``as``$class``) {`<br>```if``(``$field``instanceof``$class``) {`<br>```$value``=``$field``->get_value_entry_detail( RGFormsModel::get_lead_field_value(``$entry``,``$field``),``$currency``=``''``,``$use_text``= true,``$format``=``'html'``);`<br>```break``;`<br>```}`<br>```}`<br>```return``$value``;`<br>`}, 10, 4 );` |
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12 | `add_filter(``'gform_entry_field_value'``,``function``(``$value``,``$field``,``$entry``,``$form``) {`<br>```if``(``$field``->get_input_type() ==``'fileupload'``&&``$field``->multipleFiles && !``empty``(``$value``) ) {``// Multi-File upload field`<br>```$value``=``''``;`<br>```$raw_value``= rgar(``$entry``,``$field``->id );`<br>```$files``=``empty``(``$raw_value``) ?``array``() : json_decode(``$raw_value``, true );`<br>```foreach``(``$files``as``$file_url``) {`<br>```$value``.= sprintf(``"<a href='%s' target='_blank' title='%s'><img src='%s' width='100' /></a><br>"``,``$file_url``, __(``'Click to view'``,``'gravityforms'``),``$file_url``);`<br>```}`<br>```}`<br>```return``$value``;`<br>`}, 10, 4 );` |

And the following does the same for a single file upload field.

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8 | `add_filter(``'gform_entry_field_value'``,``function``(``$value``,``$field``,``$entry``,``$form``) {`<br>```if``(``$field``->get_input_type() ==``'fileupload'``&& !``empty``(``$value``) ) {``// Single file upload field`<br>```$file_url``= rgar(``$entry``,``$field``->id );`<br>```$value``= sprintf(``"<a href='%s' target='_blank' title='%s'><img src='%s' width='100' /></a><br>"``,``$file_url``, __(``'Click to view'``,``'gravityforms'``),``$file_url``);`<br>```}`<br>``<br>```return``$value``;`<br>`}, 10, 4 );` |
...
This filter is located in GFEntryDetail::lead\_detail\_grid() in _entry\_detail.php_.
...
[Algolia](https://docs.gravityforms.com/gform_entry_field_value/# "Algolia")
