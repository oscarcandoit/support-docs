---
url: "https://docs.gravityforms.com/gform_entries_column_filter/"
title: "gform_entries_column_filter - Gravity Forms Documentation"
---
...
# gform\_entries\_column\_filter

- [Description](https://docs.gravityforms.com/gform_entries_column_filter/#h-description)
- [Usage](https://docs.gravityforms.com/gform_entries_column_filter/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_entries_column_filter/#h-parameters)
- [Examples](https://docs.gravityforms.com/gform_entries_column_filter/#h-examples)
- [Placement](https://docs.gravityforms.com/gform_entries_column_filter/#h-placement)
- [Source Code](https://docs.gravityforms.com/gform_entries_column_filter/#h-source-code)
...
|     |     |
| --- | --- |
| 1 | `add_filter(``'gform_entries_column_filter'``,``'change_column_data'``, 10, 5 );` |
...
This example replaces the value when the field id is 2 and the form id is 1. **Note:** This example assumes that field with ID 2 is NOT placed in the first column on the entry grid. This hook does not fire for the first column in the grid. Look at [gform\_entries\_first\_column](https://docs.gravityforms.com/gform_entries_first_column) to add content to the first column.

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8 | `add_filter(``'gform_entries_column_filter'``,``'change_column_data'``, 10, 5 );`<br>`function``change_column_data(``$value``,``$form_id``,``$field_id``,``$entry``,``$query_string``) {`<br>```//only change the data when form id is 1 and field id is 2`<br>```if``(``$form_id``!= 1 &&``$field_id``!= 2 ) {`<br>```return``$value``;`<br>```}`<br>```return``"newdata"``;`<br>`}` |

This example displays text next to the existing value.

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8 | `add_filter(``'gform_entries_column_filter'``,``'change_column_data'``, 10, 5 );`<br>`function``change_column_data(``$value``,``$form_id``,``$field_id``,``$entry``,``$query_string``) {`<br>```//only change the data when form id is 1 and field id is 2`<br>```if``(``$form_id``!= 1 &&``$field_id``!= 2 ) {`<br>```return``$value``;`<br>```}`<br>```return``$value``.``" - newdata"``;`<br>`}` |
...
This hook is located in GFEntryList::leads\_page() in _entry\_list.php_.
...
[Algolia](https://docs.gravityforms.com/gform_entries_column_filter/# "Algolia")
