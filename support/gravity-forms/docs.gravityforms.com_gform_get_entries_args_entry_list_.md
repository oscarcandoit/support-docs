---
url: "https://docs.gravityforms.com/gform_get_entries_args_entry_list/"
title: "gform_get_entries_args_entry_list - Gravity Forms Documentation"
---
...
- [Description](https://docs.gravityforms.com/gform_get_entries_args_entry_list/#h-description)
- [Usage](https://docs.gravityforms.com/gform_get_entries_args_entry_list/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_get_entries_args_entry_list/#h-parameters)
- [Examples](https://docs.gravityforms.com/gform_get_entries_args_entry_list/#h-examples)
- [Filter Entry List by Entry Meta](https://docs.gravityforms.com/gform_get_entries_args_entry_list/#h-filter-entry-list-by-entry-meta)
- [Placement](https://docs.gravityforms.com/gform_get_entries_args_entry_list/#h-placement)
- [Since](https://docs.gravityforms.com/gform_get_entries_args_entry_list/#h-since)
- [Source Code](https://docs.gravityforms.com/gform_get_entries_args_entry_list/#h-source-code)

## Description

The gform\_get\_entries\_args\_entry\_list filter is executed immediately before entries are fetched for display in the Entry List view. It provides the ability to filter all arguments that are passed to the GFAPI::get\_entries() method thereby allowing you to filter which entries are displayed in the Entry List.
...
|     |     |
| --- | --- |
| 1 | `add_filter(``'gform_get_entries_args_entry_list'``,``'your_function_name'``, 10, 2 );` |

The following would apply to only to form ID 1.

|     |     |
| --- | --- |
| 1 | `add_filter(``'gform_get_entries_args_entry_list_1'``,``'your_function_name'``, 10, 2 );` |

## Parameters

- **$args** [array](https://docs.gravityforms.com/array)
Array of arguments that will be passed to GFAPI::get\_entries() to fetch the entries to be displayed.
  ...
  - **$sorting** [array](https://docs.gravityforms.com/array)

    An array containing properties that specify how the entries will be sorted.

  - **$paging** [array](https://docs.gravityforms.com/array)
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>17<br>18<br>19 | `add_filter(``'gform_get_entries_args_entry_list'``,``function``(``$args``) {`<br>```$meta_key``=``'my_meta_key'``;`<br>```$meta_value``= rgget(``$meta_key``);`<br>```if``( !``$meta_value``) {`<br>```return``$args``;`<br>```}`<br>```if``( ! isset(``$args``[``'search_criteria'``][``'field_filters'``] ) ) {`<br>```$args``[``'search_criteria'``][``'field_filters'``] =``array``();`<br>```}`<br>```$args``[``'search_criteria'``][``'field_filters'``][] =``array``(`<br>```'key'``=>``$meta_key``,`<br>```'value'``=>``$meta_value`<br>```);`<br>```return``$args``;`<br>`} );` |
...
This filter is located in GF\_Entry\_List\_Table::prepare\_items() in _entry\_list.php_.
...
[Algolia](https://docs.gravityforms.com/gform_get_entries_args_entry_list/# "Algolia")
