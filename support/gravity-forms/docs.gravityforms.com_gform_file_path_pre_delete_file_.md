---
url: "https://docs.gravityforms.com/gform_file_path_pre_delete_file/"
title: "gform_file_path_pre_delete_file - Gravity Forms Documentation"
---
...
# gform\_file\_path\_pre\_delete\_file

- [Description](https://docs.gravityforms.com/gform_file_path_pre_delete_file/#h-description)
- [Usage](https://docs.gravityforms.com/gform_file_path_pre_delete_file/#h-usage)
- [Parameters](https://docs.gravityforms.com/gform_file_path_pre_delete_file/#h-parameters)
- [Examples](https://docs.gravityforms.com/gform_file_path_pre_delete_file/#h-examples)
- [1\. Log the current path](https://docs.gravityforms.com/gform_file_path_pre_delete_file/#h-1-log-the-current-path)
- [2\. Change the path](https://docs.gravityforms.com/gform_file_path_pre_delete_file/#h-2-change-the-path)
- [Placement](https://docs.gravityforms.com/gform_file_path_pre_delete_file/#h-placement)
- [Since](https://docs.gravityforms.com/gform_file_path_pre_delete_file/#h-since)
- [Source Code](https://docs.gravityforms.com/gform_file_path_pre_delete_file/#h-source-code)

## Description

The gform\_file\_path\_pre\_delete\_file filter is executed before an uploaded file (from the File Upload or Post Image fields) is deleted from the entry. It can be used in conjunction with the [gform\_upload\_path](https://docs.gravityforms.com/gform_upload_path) filter when changing the location where uploaded files are stored.
...
|     |     |
| --- | --- |
| 1 | `add_filter(``'gform_file_path_pre_delete_file'``,``'your_function_name'``, 10, 2 );` |
...
You can use this example when [Logging And Debugging](https://docs.gravityforms.com/logging-and-debugging) to find out what the current path and url are before using the next example to change the path.

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6 | `add_filter(``'gform_file_path_pre_delete_file'``,``function``(``$file_path``,``$url``) {`<br>```GFCommon::log_debug(``'gform_file_path_pre_delete_file: path of file to be deleted => '``.``$file_path``);`<br>```GFCommon::log_debug(``'gform_file_path_pre_delete_file: url of file to be deleted => '``.``$url``);`<br>```return``$file_path``;`<br>`}, 1, 2 );` |
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6 | `add_filter(``'gform_file_path_pre_delete_file'``,``'change_file_path'``, 10, 2 );`<br>`function``change_file_path(``$file_path``,``$url``) {`<br>```$file_path``=``'/home/public_html/yourdomainfolder/new/path/filename.pdf'``;`<br>```return``$file_path``;`<br>`}` |
...
This filter is located in GFFormsModel::delete\_physical\_file() in _forms\_model.php_.
...
[Algolia](https://docs.gravityforms.com/gform_file_path_pre_delete_file/# "Algolia")
