---
url: "https://docs.gravityforms.com/how-to-load-forms-in-hidden-uis/"
title: "How To Load Forms In Hidden UIs - Gravity Forms Documentation"
---
...
- [Introduction](https://docs.gravityforms.com/how-to-load-forms-in-hidden-uis/#h-introduction)
- [Examples](https://docs.gravityforms.com/how-to-load-forms-in-hidden-uis/#h-examples)
- [Load a Form in a Hidden UI using Barba.js](https://docs.gravityforms.com/how-to-load-forms-in-hidden-uis/#h-load-a-form-in-a-hidden-ui-using-barba-js)
- [Placement](https://docs.gravityforms.com/how-to-load-forms-in-hidden-uis/#h-placement)
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>17<br>18<br>19<br>20<br>21<br>22<br>23<br>24<br>25<br>26<br>27<br>28<br>29<br>30 | `/**`<br>```* This action loads a specific form in all frontend pages without displaying it, so the scripts and styles are loaded.`<br>```*/`<br>`add_action(``'init'``,``function``(){`<br>```if``( is_admin() ) {`<br>```return``;`<br>```}`<br>```if``( !``class_exists``(``'GFFormDisplay'``) ) {`<br>```require_once``( GFCommon::get_base_path() .``'/form_display.php'``);`<br>```}`<br>```// Gets the form for rendering, but don't display it`<br>```GFForms::get_form( 100 );`<br>`}, 100 );`<br>`/**`<br>```* This action tells Barba.js to execute all script tags in the next page.`<br>```* Allowing GravityForms scripts to be executed.`<br>```*/`<br>`add_action(``'wp_footer'``,``function``(){`<br>`?>`<br>`<script>`<br>```barba.hooks.beforeEnter(({ next }) => {`<br>```const``js = next.container.querySelectorAll(``"script"``);`<br>```js.forEach(script => {`<br>```window.``eval``(script.innerHTML);`<br>```});`<br>```});`<br>`</script>`<br>`<?php`<br>`} );` |
...
[Algolia](https://docs.gravityforms.com/how-to-load-forms-in-hidden-uis/# "Algolia")
