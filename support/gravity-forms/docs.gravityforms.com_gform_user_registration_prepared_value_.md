---
url: "https://docs.gravityforms.com/gform_user_registration_prepared_value/"
title: "gform_user_registration_prepared_value - Gravity Forms"
---
...
# gform\_user\_registration\_prepared\_value

- [Description](https://docs.gravityforms.com/gform_user_registration_prepared_value/#h-description)
- [Usage](https://docs.gravityforms.com/gform_user_registration_prepared_value/#h-usage)
- [Examples](https://docs.gravityforms.com/gform_user_registration_prepared_value/#h-examples)
- [Country Code](https://docs.gravityforms.com/gform_user_registration_prepared_value/#h-country-code)
- [US State Code](https://docs.gravityforms.com/gform_user_registration_prepared_value/#h-us-state-code)
- [Checkboxes as serialized data](https://docs.gravityforms.com/gform_user_registration_prepared_value/#h-checkboxes-as-serialized-data)
- [Placement](https://docs.gravityforms.com/gform_user_registration_prepared_value/#h-placement)
- [Since](https://docs.gravityforms.com/gform_user_registration_prepared_value/#h-since)
- [Source Code](https://docs.gravityforms.com/gform_user_registration_prepared_value/#h-source-code)

## Description

This filter can be used to modify a field value before it is saved to the user meta. For a form using a **Update User** feed type, you may also want to use the [gform\_user\_registration\_user\_data\_pre\_populate](https://docs.gravityforms.com/gform_user_registration_user_data_pre_populate/) filter in combination with this one.

## Usage

|     |     |
| --- | --- |
| 1 | `add_filter(``'gform_user_registration_prepared_value'``,``'your_function_name'``, 10, 5 );` |
...
- **$is\_username** [bool](https://docs.gravityforms.com/bool)
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8 | `add_filter(``'gform_user_registration_prepared_value'``,``function``(``$value``,``$field``,``$input_id``,``$entry``,``$is_username``) {`<br>`$country_id``=``$field``->id .``'.6'``;`<br>`if``(``$field``->type ==``'address'``&&``$input_id``==``$country_id``) {`<br>`return``GF_Fields::get(``'address'``)->get_country_code(``$value``);`<br>`}`<br>``<br>`return``$value``;`<br>`}, 10, 5 );` |
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8 | `add_filter(``'gform_user_registration_prepared_value'``,``function``(``$value``,``$field``,``$input_id``,``$entry``,``$is_username``) {`<br>`$state_id``=``$field``->id .``'.4'``;`<br>`if``(``$field``->type ==``'address'``&&``$input_id``==``$state_id``) {`<br>`return``GF_Fields::get(``'address'``)->get_us_state_code(``$value``);`<br>`}`<br>``<br>`return``$value``;`<br>`}, 10, 5 );` |
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15 | `add_filter(``'gform_user_registration_prepared_value'``,``'gf_serialize_checkboxes'``, 10, 5 );`<br>`function``gf_serialize_checkboxes(``$value``,``$field``,``$input_id``,``$entry``,``$is_username``) {`<br>``<br>`if``(``$field``->id ==``'18'``&&``$field``->type ==``'checkbox'``) {``// Update 18 to your field id number`<br>`// Get a comma separated list of checkboxes checked`<br>`$checked``=``$field``->get_value_export(``$entry``);`<br>``<br>`// Convert to array`<br>`$value``=``explode``(``', '``,``$checked``);`<br>``<br>`// WordPress will automatically serialize this array before saving.`<br>`}`<br>``<br>`return``$value``;`<br>`}` |
...
|     |     |
| --- | --- |
| 1 | `$value``= apply_filters(``'gform_user_registration_prepared_value'``,``$value``,``$field``,``$input_id``,``$entry``,``$is_username``);` |

This filter is located in GF\_User\_Registration::get\_prepared\_value() in _class-gf-user-registration.php_.
...
[Algolia](https://docs.gravityforms.com/gform_user_registration_prepared_value/# "Algolia")
