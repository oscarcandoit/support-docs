---
url: "https://docs.gravityforms.com/gform_user_registration_user_data_pre_populate/"
title: "gform_user_registration_user_data_pre_populate - Gravity Forms"
---
...
# gform\_user\_registration\_user\_data\_pre\_populate

- [Description](https://docs.gravityforms.com/gform_user_registration_user_data_pre_populate/#h-description)
- [Usage](https://docs.gravityforms.com/gform_user_registration_user_data_pre_populate/#h-usage)
- [Examples](https://docs.gravityforms.com/gform_user_registration_user_data_pre_populate/#h-examples)
- [1\. Override a field value](https://docs.gravityforms.com/gform_user_registration_user_data_pre_populate/#h-1-override-a-field-value)
- [2\. Log the values](https://docs.gravityforms.com/gform_user_registration_user_data_pre_populate/#h-2-log-the-values)
- [3\. Replace country code](https://docs.gravityforms.com/gform_user_registration_user_data_pre_populate/#h-3-replace-country-code)
- [4\. Unserialize data](https://docs.gravityforms.com/gform_user_registration_user_data_pre_populate/#h-4-unserialize-data)
- [Placement](https://docs.gravityforms.com/gform_user_registration_user_data_pre_populate/#h-placement)
- [Since](https://docs.gravityforms.com/gform_user_registration_user_data_pre_populate/#h-since)
- [Source Code](https://docs.gravityforms.com/gform_user_registration_user_data_pre_populate/#h-source-code)
...
- **$mapped\_fields** [array](https://docs.gravityforms.com/array)
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11 | `add_filter(``'gform_user_registration_user_data_pre_populate'``,``function``(``$mapped_fields``,``$form``,``$feed``) {`<br>```gf_user_registration()->log_debug(``'Running for form id '``.``$form``[``'id'``] );`<br>```// Run only for form 3.`<br>```if``(``$form``[``'id'``] !=``'3'``) {`<br>```return``$mapped_fields``;`<br>```}`<br>```// Replace "12" with the field ID you want to display empty.`<br>```$mapped_fields``[12] =``'My Custom Value'``;`<br>```return``$mapped_fields``;`<br>`}, 10, 3 );` |

### 2\. Log the values

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5 | `add_filter(``'gform_user_registration_user_data_pre_populate'``,``function``(``$mapped_fields``,``$form``,``$feed``) {`<br>```gf_user_registration()->log_debug(``'gform_user_registration_user_data_pre_populate: $mapped_fields => '``. print_r(``$mapped_fields``, 1 ) );`<br>```return``$mapped_fields``;`<br>`}, 10, 3 );` |

### 3\. Replace country code

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16 | `add_filter(``'gform_user_registration_user_data_pre_populate'``,``function``(``$mapped_fields``,``$form``,``$feed``) {`<br>```$fields``= GFCommon::get_fields_by_type(``$form``,``array``(``'address'``) );`<br>```if``( !``empty``(``$fields``) ) {`<br>```$codes``= GF_Fields::get(``'address'``)->get_country_codes();`<br>```foreach``(``$fields``as``$field``) {`<br>```$country_id``=``$field``->id .``'.6'``;`<br>```$value``= rgar(``$mapped_fields``,``$country_id``);`<br>```if``( !``empty``(``$value``) ) {`<br>```$mapped_fields``[``$country_id``] =``array_search``(``$value``,``$codes``);`<br>```}`<br>```}`<br>```}`<br>```return``$mapped_fields``;`<br>`}, 10, 3 );` |

### 4\. Unserialize data

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8 | `add_filter(``'gform_user_registration_user_data_pre_populate'``,``'gf_unserialize_data'``, 10, 3 );`<br>`function``gf_unserialize_data(``$mapped_fields``,``$form``,``$feed``) {`<br>```// Replace "12" with the field ID you want to replace.`<br>```$mapped_fields``[12] = maybe_unserialize(``$mapped_fields``[12] );`<br>```return``$mapped_fields``;`<br>`}` |
...
This filter is located in GF\_User\_Registration::prepopulate\_form() in _class-gf-user-registration.php_.
...
[Algolia](https://docs.gravityforms.com/gform_user_registration_user_data_pre_populate/# "Algolia")
