---
url: "https://docs.gravityforms.com/gform_webhooks_post_request/"
title: "gform_webhooks_post_request - Gravity Forms Documentation"
---
...
- [Description](https://docs.gravityforms.com/gform_webhooks_post_request/#h-description)
- [Usage](https://docs.gravityforms.com/gform_webhooks_post_request/#h-usage)
- [Examples](https://docs.gravityforms.com/gform_webhooks_post_request/#h-examples)
- [Delete entry after successful WordPress response for Webhook processing](https://docs.gravityforms.com/gform_webhooks_post_request/#h-delete-entry-after-successful-wordpress-response-for-webhook-processing)
- [Send an email if WordPress returns an error for Webhook processing](https://docs.gravityforms.com/gform_webhooks_post_request/#h-send-an-email-if-wordpress-returns-an-error-for-webhook-processing)
- [Placement](https://docs.gravityforms.com/gform_webhooks_post_request/#h-placement)
- [Source Code](https://docs.gravityforms.com/gform_webhooks_post_request/#h-source-code)

## Description

The “gform\_webhooks\_post\_request” action fires after a Webhooks request has been executed and allows further actions to be performed.
...
To target a specific form, append the form id to the hook name. (format: gform\_webhooks\_post\_request\_FORMID)

```
add_action( 'gform_webhooks_post_request_78' 'your_function_name', 10, 4 );
```

To target a specific form’s feed, append the form id and feed id to the hook name. (format: gform\_webhooks\_post\_request\_FORMID\_FEEDID)
...
- **$response** WP\_Error or [array](https://docs.gravityforms.com/array)
The response from the request execution. The response will be the WP\_Error object if the execution fails, otherwise an array.
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5 | `add_action(``'gform_webhooks_post_request'``,``function``(``$response``,``$feed``,``$entry``,``$form``) {`<br>```if``( ! is_wp_error(``$response``) ) {`<br>```GFAPI::delete_entry(``$entry``[``'id'``] );`<br>```}`<br>`}, 10, 4 );` |

### Send an email if WordPress returns an error for Webhook processing

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9 | `add_action(``'gform_webhooks_post_request'``,``function``(``$response``,``$feed``,``$entry``,``$form``) {`<br>```if``( is_wp_error(``$response``) ) {`<br>```$error_message``=``$response``->get_error_message();`<br>```$to``=``'user@example.com'``;``// Change this to your email address.`<br>```$subject``=``'Webhook failed!'``;`<br>```$body``=``"Webhook for entry #$entry[id] failed. Error: $error_message"``;`<br>```wp_mail(``$to``,``$subject``,``$body``);`<br>```}`<br>`}, 10, 4 );` |
...
[Algolia](https://docs.gravityforms.com/gform_webhooks_post_request/# "Algolia")
