---
url: "https://docs.gravityforms.com/polls-change-log/"
title: "Polls Changelog - Gravity Forms Documentation"
---
...
- [Add-Ons](https://docs.gravityforms.com/category/add-ons-gravity-forms/)  - [Add-On Basics](https://docs.gravityforms.com/category/add-ons-gravity-forms/add-on-basics/)
  ...
  - [Polls Add-On](https://docs.gravityforms.com/category/add-ons-gravity-forms/polls-add-on/)    - [Using the Polls Add-On](https://docs.gravityforms.com/using-the-polls-add-on/ "Using the Polls Add-On")
    - [Polls Add-On Settings Reference](https://docs.gravityforms.com/poll-settings/ "Polls Add-On Settings Reference")
    - [Setting Up A Sample Poll Form](https://docs.gravityforms.com/setting-up-a-sample-poll-form/ "Setting Up A Sample Poll Form")
    - [Viewing Poll Results](https://docs.gravityforms.com/viewing-poll-results/ "Viewing Poll Results")
    - [Displaying Your Poll Results to Visitors](https://docs.gravityforms.com/displaying-your-poll-results-to-visitors/ "Displaying Your Poll Results to Visitors")
    - [How Poll Results Work and Why They May Be Delayed](https://docs.gravityforms.com/what-to-do-if-your-poll-results-not-current/ "How Poll Results Work and Why They May Be Delayed")
    - [Polls Block](https://docs.gravityforms.com/polls-block/ "Polls Block")
    - [Poll Widget](https://docs.gravityforms.com/poll-widget/ "Poll Widget")
    - [Poll Shortcode](https://docs.gravityforms.com/poll-shortcodes/ "Poll Shortcode")
    - [Poll Merge Tags](https://docs.gravityforms.com/poll-merge-tags/ "Poll Merge Tags")
    - [Polls Changelog](https://docs.gravityforms.com/polls-change-log/ "Polls Changelog")
...
- Fixed an issue where the poll result is unstyled when embedded via the Poll block.
- Fixed an issue where the poll result is displaying incorrectly in the Gravity Forms 2.5 Theme in RTL languages.
- Fixed an issue where the poll result is displaying percentage and count placeholders when those are disabled.
- Fixed an issue where the poll result page is not announced by screen readers.
- Fixed an issue where the percentage ratio box does not align correctly with the bar in the block editor.
- Updated Polls to be released with minified and non-minified assets.
- Updated Polls to support the new form theme asset loading system from GFAddOn.

### 4.3.0 \| 2024-10-02

- Fixed an issue that causes a PHP notice on the WordPress 6.6+ Appearance > Widgets page.
- Fixed an issue where Poll results are not animated.
- Fixed a spacing issue in the choices flyout in the form editor.
...
- Fixed a PHP notice which can occur when a third-party add-on triggers the gform\_entry\_field\_value filter without passing a field object.
...
- Added Poll block.
- Added the _[gform\_polls\_results\_ajax\_response](https://docs.gravityforms.com/gform_polls_results_ajax_response/)_ filter, including form specific version, enabling the results Ajax response to be overridden.
...
- Fixed an issue with the locations the gform\_merge\_tags hook was being used to include the merge tags.
...
- Added function gf\_polls to easily get a Poll instance.
...
- Added the _gform\_polls\_percentage\_precision_ hook so the percentages on the front-end results can be rounded to a custom number of decimal digits.
...
- Updated GFPolls::merge\_tag\_filter() to public static.
...
- Added the gform\_polls\_cron\_schedule filter so the frequency of the cron job can be modified.
Notes: The wp\_cron job will take over the calculation of the results if 5 seconds is not long enough at the time of form submission/update.
The default schedule is set to hourly to avoid heavy loads on the server but the frequency can be changed by using the _gform\_polls\_cron\_schedule filter_.
The format follows the same format required by the [WordPress cron\_schedules filter](http://codex.wordpress.org/Function_Reference/wp_get_schedules) **Important**: the Polls Add-On must be deactivated and reactivated in order to reschedule the task.
Example usage:
add\_filter( 'gform\_polls\_cron\_schedule', 'cron\_add\_twice\_hourly' );
function cron\_add\_twice\_hourly() {
// Adds once weekly to the existing schedules.
$schedule\['twicehourly'\] = array(
'interval' => 1800, // number of seconds in the interval
'display' => \_\_( 'Twice Hourly' )
);
return $schedule;
}
...
- Added _gform\_form\_pre\_results_ filter so the form object can be manipulated before the results are calculated for the admin results page.
- Added _gform\_polls\_form\_pre\_results_ so the form object can be manipulated before the results are calculated for the front and and entry detail page.
...
- Added results page.
- Updated load\_plugin\_textdomain() call to be outside the is\_admin() block so that front end strings can be translated.
...
- Removed poll settings from the widget by default and added gpoll\_widget\_override filter to display the option to override
Example usage:
add\_filter('gpoll\_widget\_override' , 'gpoll\_widget\_override');
function gpoll\_widget\_override($form\_ids){
array\_push($form\_ids, 7, 9);
return $form\_ids;
}
...
- Fixed performance issue in the display\_entries\_field\_value() function.
...
- Fixed an issue where results were being displayed when the cookie is set and display\_results=false.
...
[Algolia](https://docs.gravityforms.com/polls-change-log/# "Algolia")
