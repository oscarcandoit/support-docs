---
url: "https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/best-practices"
title: "Skill 작성 모범 사례 - Claude Docs"
---
...
[Log in](https://platform.claude.com/login?returnTo=%2Fdocs%2Fko%2Fagents-and-tools%2Fagent-skills%2Fbest-practices)
...
Skill이 어떻게 작동하는지에 대한 개념적 배경은 [Skill 개요](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/overview) 를 참조하세요.

## 핵심 원칙

### 간결함이 핵심입니다

[컨텍스트 윈도우](https://platform.claude.com/docs/ko/build-with-claude/context-windows) 는 공공재입니다. 당신의 Skill은 Claude가 알아야 할 다른 모든 것과 컨텍스트 윈도우를 공유합니다:
...
전체 Skill 구조 세부 정보는 [Skill 개요](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/overview#skill-structure) 를 참조하세요.
...
SKILL.md는 온보딩 가이드의 목차처럼 Claude를 필요에 따라 상세 자료로 안내하는 개요 역할을 합니다. 점진적 공개가 어떻게 작동하는지에 대한 설명은 개요의 [Skill이 어떻게 작동하는지](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/overview#how-skills-work) 를 참조하세요.
...
![메타데이터와 마크다운 본문을 보여주는 간단한 SKILL.md 파일](https://platform.claude.com/docs/images/agent-skills-simple-file.png)

Skill이 성장하면서 필요에 따라 Claude가 로드하는 추가 콘텐츠를 번들할 수 있습니다:

![reference.md 및 forms.md와 같은 추가 참조 파일 번들링.](https://platform.claude.com/docs/images/agent-skills-bundling-content.png)
...
이 파일 시스템 기반 아키텍처가 점진적 공개를 가능하게 하는 방법에 대한 세부 정보는 아래 고급 섹션의 [런타임 환경](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/best-practices#runtime-environment) 섹션을 참조하세요.
...
이는 스크립트 대신 참조 문서를 사용하는 검증 루프 패턴을 보여줍니다. "검증자"는 STYLE\_GUIDE.md이고 Claude는 읽고 비교하여 확인을 수행합니다.
...
아래 섹션은 실행 가능한 스크립트를 포함하는 Skill에 중점을 둡니다. Skill이 마크다운 지침만 사용하는 경우 [효과적인 Skill 체크리스트](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/best-practices#checklist-for-effective-skills) 로 건너뛰세요.
...
![지침 파일과 함께 실행 가능한 스크립트 번들링](https://platform.claude.com/docs/images/agent-skills-executable-scripts.png)
...
- **스크립트 실행** (가장 일반적): "analyze\_form.py를 실행하여 필드 추출"
- **참조로 읽기** (복잡한 로직의 경우): "필드 추출 알고리즘은 analyze\_form.py 참조"

대부분의 유틸리티 스크립트의 경우 실행이 선호됩니다. 더 신뢰할 수 있고 효율적이기 때문입니다. 스크립트 실행이 어떻게 작동하는지에 대한 세부 정보는 아래 [런타임 환경](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/best-practices#runtime-environment) 섹션을 참조하세요.
...
**구현 팁**: 검증 스크립트를 상세하게 만들고 "필드 'signature\_date'을(를) 찾을 수 없습니다. 사용 가능한 필드: customer\_name, order\_total, signature\_date\_signed"와 같은 구체적인 오류 메시지를 제공하여 Claude가 문제를 수정하는 데 도움을 줍니다.
...
SKILL.md에 필요한 패키지를 나열하고 [코드 실행 도구 문서](https://platform.claude.com/docs/ko/agents-and-tools/tool-use/code-execution-tool) 에서 사용 가능한지 확인하세요.

### 런타임 환경

Skill은 파일 시스템 액세스, bash 명령 및 코드 실행 기능이 있는 코드 실행 환경에서 실행됩니다. 이 아키텍처의 개념적 설명은 개요의 [Skill 아키텍처](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/overview#the-skills-architecture) 를 참조하세요.
...
- **실행 의도 명확히**:
  - "analyze\_form.py를 실행하여 필드 추출" (실행)
  - "추출 알고리즘은 analyze\_form.py 참조" (참조로 읽기)
...
기술 아키텍처에 대한 완전한 세부 정보는 Skill 개요의 [Skill이 어떻게 작동하는지](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/overview#how-skills-work) 를 참조하세요.
...
전체 구조 세부 정보는 [Skill 개요](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/overview#skill-structure) 를 참조하세요.

### 토큰 예산

최적의 성능을 위해 SKILL.md 본문을 500줄 이하로 유지하세요. 콘텐츠가 이를 초과하면 위에서 설명한 점진적 공개 패턴을 사용하여 별도 파일로 분할하세요. 아키텍처 세부 정보는 [Skill 개요](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/overview#how-skills-work) 를 참조하세요.
...
[Agent Skill 시작하기\\
\\
첫 번째 Skill 생성](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/quickstart) [Claude Code에서 Skill 사용\\
\\
Claude Code에서 Skill 생성 및 관리](https://code.claude.com/docs/skills) [Agent SDK에서 Skill 사용\\
\\
TypeScript 및 Python에서 프로그래밍 방식으로 Skill 사용](https://platform.claude.com/docs/ko/agent-sdk/skills) [API에서 Skill 사용\\
\\
프로그래밍 방식으로 Skill 업로드 및 사용](https://platform.claude.com/docs/ko/build-with-claude/skills-guide)

- [Skill 구조](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/best-practices#skill)
- [Claude와 함께 반복적으로 Skill 개발](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/best-practices#claude-skill)
- [Claude가 Skill을 탐색하는 방식 관찰](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/best-practices#claude-skill-2)
- [Windows 스타일 경로 피하기](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/best-practices#windows)
- [고급: 실행 가능한 코드가 있는 Skill](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/best-practices#skill-2)
- [MCP 도구 참조](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/best-practices#mcp)
- [YAML 프론트매터 요구사항](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/best-practices#yaml)
- [효과적인 Skill 체크리스트](https://platform.claude.com/docs/ko/agents-and-tools/agent-skills/best-practices#skill-3)
