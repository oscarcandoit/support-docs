---
url: "https://platform.claude.com/docs/ja/about-claude/models/whats-new-claude-4-5"
title: "Claude 4.5の新機能 - Claude Docs"
---
...
[Log in](https://platform.claude.com/login?returnTo=%2Fdocs%2Fja%2Fabout-claude%2Fmodels%2Fwhats-new-claude-4-5)
...
Claude Opus 4.5は、 [effortパラメータ](https://platform.claude.com/docs/ja/build-with-claude/effort) をサポートする唯一のモデルで、Claude応答時に使用するトークン数を制御できます。これにより、単一のモデルで応答の徹底性とトークン効率のトレードオフを調整できます。
...
Claude Opus 4.5は、 [強化されたコンピュータ使用機能](https://platform.claude.com/docs/ja/agents-and-tools/tool-use/computer-use-tool) を導入し、特定の画面領域を全解像度で詳細に検査できる新しいズームアクションを備えています。これにより、Claude は標準的なスクリーンショットでは不明確な可能性がある細粒度のUI要素、小さなテキスト、詳細な視覚情報を検査できます。
...
Claude Opus 4.5は、以前のOpusモデルよりも [アクセスしやすい価格設定](https://platform.claude.com/docs/ja/about-claude/pricing) でフラッグシップレベルの知能を提供し、より広い範囲のアプリケーションとユースケースで高度なAI機能を利用可能にしています。

### 思考ブロックの保持

Claude Opus 4.5は、 [以前のすべての思考ブロックを自動的に保持](https://platform.claude.com/docs/ja/build-with-claude/extended-thinking#thinking-block-preservation-in-claude-opus-4-5) し、拡張マルチターンインタラクションとツール使用セッション全体で推論の連続性を維持します。これにより、Claude は複雑で長時間実行されるタスクで作業する際に、完全な推論履歴を効果的に活用できます。
...
Claude Sonnet 4.5は、 [拡張思考](https://platform.claude.com/docs/ja/build-with-claude/extended-thinking) が有効な場合、コーディングタスクで大幅に優れたパフォーマンスを発揮します。拡張思考はデフォルトで無効ですが、複雑なコーディング作業では有効にすることをお勧めします。拡張思考は [プロンプトキャッシング効率](https://platform.claude.com/docs/ja/build-with-claude/prompt-caching#caching-with-thinking-blocks) に影響することに注意してください。設定の詳細については、 [移行ガイド](https://platform.claude.com/docs/ja/about-claude/models/migrating-to-claude-4#extended-thinking-recommendations) を参照してください。
...
コンテキスト認識は、Claude Sonnet 4、Sonnet 4.5、Haiku 4.5、Opus 4、Opus 4.1、およびOpus 4.5で利用可能です。
...
このコミュニケーションスタイルで作業するための詳細なガイダンスについては、 [Claude 4ベストプラクティス](https://platform.claude.com/docs/ja/build-with-claude/prompt-engineering/claude-4-best-practices) を参照してください。
...
拡張思考は、APIリクエストに`thinking`パラメータを追加することで明示的に有効にする必要があります。実装の詳細については、 [拡張思考ドキュメント](https://platform.claude.com/docs/ja/build-with-claude/extended-thinking) を参照してください。

Claude Haiku 4.5は、 [拡張思考](https://platform.claude.com/docs/ja/build-with-claude/extended-thinking) が有効な場合、コーディングと推論タスクで大幅に優れたパフォーマンスを発揮します。拡張思考はデフォルトで無効ですが、複雑な問題解決、コーディング作業、マルチステップ推論では有効にすることをお勧めします。拡張思考は [プロンプトキャッシング効率](https://platform.claude.com/docs/ja/build-with-claude/prompt-caching#caching-with-thinking-blocks) に影響することに注意してください。設定の詳細については、 [移行ガイド](https://platform.claude.com/docs/ja/about-claude/models/migrating-to-claude-4#extended-thinking-recommendations) を参照してください。

Claude Sonnet 3.7、Sonnet 4、Sonnet 4.5、Haiku 4.5、Opus 4、Opus 4.1、およびOpus 4.5で利用可能です。
...
これは、ネイティブコンテキスト認識機能を備えた最初のHaikuモデルです。プロンプティングガイダンスについては、 [Claude 4ベストプラクティス](https://platform.claude.com/docs/ja/build-with-claude/prompt-engineering/claude-4-best-practices#context-awareness-and-multi-window-workflows) を参照してください。

Claude Sonnet 4、Sonnet 4.5、Haiku 4.5、Opus 4、Opus 4.1、およびOpus 4.5で利用可能です。
...
[プログラマティックツール呼び出し](https://platform.claude.com/docs/ja/agents-and-tools/tool-use/programmatic-tool-calling) により、Claude はモデルを通じた各ツール呼び出しのラウンドトリップを必要とするのではなく、コード実行コンテナ内でプログラマティックにツールを呼び出すコードを記述できます。これにより、マルチツールワークフローのレイテンシが大幅に削減され、Claude がモデルのコンテキストウィンドウに到達する前にデータをフィルタリングまたは処理できるようにすることで、トークン消費が削減されます。
...
Claude Opus 4.5およびClaude Sonnet 4.5で利用可能です。 [ベータヘッダー](https://platform.claude.com/docs/ja/api/beta-headers) が必要です: `advanced-tool-use-2025-11-20`

### ツール検索ツール（ベータ）

[ツール検索ツール](https://platform.claude.com/docs/ja/agents-and-tools/tool-use/tool-search-tool) により、Claude は数百または数千のツールで作業でき、必要に応じてそれらを動的に発見してロードできます。すべてのツール定義をコンテキストウィンドウに事前にロードするのではなく、Claude はツールカタログを検索し、必要なツールのみをロードします。
...
[effortパラメータ](https://platform.claude.com/docs/ja/build-with-claude/effort) により、Claude 応答時に使用するトークン数を制御でき、応答の徹底性とトークン効率のトレードオフを調整できます。
...
Claude Opus 4.5でのみ利用可能です。 [ベータヘッダー](https://platform.claude.com/docs/ja/api/beta-headers) が必要です: `effort-2025-11-24`

### ツール使用例（ベータ）

[ツール使用例](https://platform.claude.com/docs/ja/agents-and-tools/tool-use/implement-tool-use#providing-tool-use-examples) により、有効なツール入力の具体的な例を提供して、Claude がツールをより効果的に使用する方法を理解するのに役立てることができます。これは、ネストされたオブジェクト、オプションパラメータ、または形式に敏感な入力を持つ複雑なツールに特に役立ちます。
...
Claude Sonnet 4.5、Haiku 4.5、Opus 4.5、Opus 4.1、およびOpus 4で利用可能です。 [ベータヘッダー](https://platform.claude.com/docs/ja/api/beta-headers) が必要です: `advanced-tool-use-2025-11-20`。

### メモリツール（ベータ）

新しい [メモリツール](https://platform.claude.com/docs/ja/agents-and-tools/tool-use/memory-tool) により、Claude はコンテキストウィンドウ外で情報を保存および取得できます。
...
Claude Sonnet 4、Sonnet 4.5、Haiku 4.5、Opus 4、Opus 4.1、およびOpus 4.5で利用可能です。 [ベータヘッダー](https://platform.claude.com/docs/ja/api/beta-headers) が必要です: `context-management-2025-06-27`

### コンテキスト編集

[コンテキスト編集](https://platform.claude.com/docs/ja/build-with-claude/context-editing) を使用して、自動ツール呼び出しクリアリングを通じたインテリジェントなコンテキスト管理を行います。
...
Claude 4モデルを使用する場合、 [`refusal`停止理由を処理](https://platform.claude.com/docs/ja/test-and-evaluate/strengthen-guardrails/handle-streaming-refusals) するようにアプリケーションを更新する必要があります。
...
詳細については、 [拡張思考ドキュメント](https://platform.claude.com/docs/ja/build-with-claude/extended-thinking#summarized-thinking) を参照してください。

### インターリーブされた思考

Claude 4モデルは、ツール使用と拡張思考のインターリーブをサポートし、ツール使用と応答を通常のメッセージと混在させることができるより自然な会話を可能にします。

インターリーブされた思考はベータ版です。インターリーブされた思考を有効にするには、APIリクエストに [ベータヘッダー](https://platform.claude.com/docs/ja/api/beta-headers)`interleaved-thinking-2025-05-14`を追加してください。

詳細については、 [拡張思考ドキュメント](https://platform.claude.com/docs/ja/build-with-claude/extended-thinking#interleaved-thinking) を参照してください。
...
これらのモデルで作業するための包括的なガイダンスについては、 [Claude 4プロンプトエンジニアリングベストプラクティス](https://platform.claude.com/docs/ja/build-with-claude/prompt-engineering/claude-4-best-practices) を参照してください。
...
詳細については、 [テキストエディタツールドキュメント](https://platform.claude.com/docs/ja/agents-and-tools/tool-use/text-editor-tool) を参照してください。
...
移行手順については、 [コード実行ツールドキュメント](https://platform.claude.com/docs/ja/agents-and-tools/tool-use/code-execution-tool#upgrade-to-latest-tool-version) を参照してください。
...
詳細については、 [価格ドキュメント](https://platform.claude.com/docs/ja/about-claude/pricing) を参照してください。
...
- [AWS Bedrockグローバル対リージョナルエンドポイント](https://platform.claude.com/docs/ja/build-with-claude/claude-on-amazon-bedrock#global-vs-regional-endpoints)
- [Google Vertex AIグローバル対リージョナルエンドポイント](https://platform.claude.com/docs/ja/build-with-claude/claude-on-vertex-ai#global-vs-regional-endpoints)
...
破壊的な変更と移行要件は、アップグレード元のモデルによって異なります。詳細な移行手順（ステップバイステップガイド、破壊的な変更、移行チェックリストを含む）については、 [Claude 4.5への移行](https://platform.claude.com/docs/ja/about-claude/models/migrating-to-claude-4) を参照してください。
...
[ベストプラクティス\\
\\
Claude 4.5モデルのプロンプトエンジニアリング技術を学ぶ](https://platform.claude.com/docs/ja/build-with-claude/prompt-engineering/claude-4-best-practices) [モデル概要\\
\\
Claude 4.5モデルと他のClaudeモデルを比較](https://platform.claude.com/docs/ja/about-claude/models/overview) [移行ガイド\\
\\
以前のモデルからアップグレード](https://platform.claude.com/docs/ja/about-claude/models/migrating-to-claude-4)

- [Opus 4.5がOpus 4.1より改善された主な点](https://platform.claude.com/docs/ja/about-claude/models/whats-new-claude-4-5#opus-4-5-opus-4-1)
- [Effortパラメータ](https://platform.claude.com/docs/ja/about-claude/models/whats-new-claude-4-5#effort)
- [Sonnet 4.5がSonnet 4より改善された主な点](https://platform.claude.com/docs/ja/about-claude/models/whats-new-claude-4-5#sonnet-4-5-sonnet-4)
- [Haiku 4.5がHaiku 3.5より改善された主な点](https://platform.claude.com/docs/ja/about-claude/models/whats-new-claude-4-5#haiku-4-5-haiku-3-5)
- [新しいAPI機能](https://platform.claude.com/docs/ja/about-claude/models/whats-new-claude-4-5#api)
- [Effortパラメータ（ベータ）](https://platform.claude.com/docs/ja/about-claude/models/whats-new-claude-4-5#effort-2)
- [Claude 4で導入された機能](https://platform.claude.com/docs/ja/about-claude/models/whats-new-claude-4-5#claude-4)
