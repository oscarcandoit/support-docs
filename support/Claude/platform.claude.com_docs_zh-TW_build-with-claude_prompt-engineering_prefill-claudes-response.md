---
url: "https://platform.claude.com/docs/zh-TW/build-with-claude/prompt-engineering/prefill-claudes-response"
title: "預填 Claude 的回應以獲得更好的輸出控制 - Claude Docs"
---
...
[Log in](https://platform.claude.com/login?returnTo=%2Fdocs%2Fzh-TW%2Fbuild-with-claude%2Fprompt-engineering%2Fprefill-claudes-response)
...
**進階使用者提示**：預填 `{` 會強制 Claude 跳過前言並直接輸出 JSON 物件。這樣更簡潔、更簡潔，程式更容易解析，無需額外處理。
為了保證輸出符合特定架構的 JSON，請考慮使用 [結構化輸出](https://platform.claude.com/docs/zh-TW/build-with-claude/structured-outputs) 而不是預填。結構化輸出確保 Claude 的回應始終符合您定義的 JSON 架構，非常適合需要嚴格格式合規性的生產應用程式。
...
[使用訊息\\
\\
查看更多預填和其他訊息 API 模式的範例。](https://platform.claude.com/docs/zh-TW/build-with-claude/working-with-messages) [提示庫\\
\\
從為各種任務和使用案例精選的提示中獲得靈感。](https://platform.claude.com/docs/zh-TW/resources/prompt-library/library) [GitHub 提示教程\\
\\
一個充滿範例的教程，涵蓋我們文件中的提示工程概念。](https://github.com/anthropics/prompt-eng-interactive-tutorial) [Google Sheets 提示教程\\
\\
通過互動式試算表提供的更輕量級版本的提示工程教程。](https://docs.google.com/spreadsheets/d/19jzLgRruG9kjUQNKtCg1ZjdD6l6weA6qRXG5zLIAhC8)

- [如何預填 Claude 的回應](https://platform.claude.com/docs/zh-TW/build-with-claude/prompt-engineering/prefill-claudes-response#claude)
