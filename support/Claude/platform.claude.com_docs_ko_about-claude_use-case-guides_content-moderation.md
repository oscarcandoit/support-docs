---
url: "https://platform.claude.com/docs/ko/about-claude/use-case-guides/content-moderation"
title: "콘텐츠 조정 - Claude Docs"
---
...
[Log in](https://platform.claude.com/login?returnTo=%2Fdocs%2Fko%2Fabout-claude%2Fuse-case-guides%2Fcontent-moderation)
...
이 가이드는 애플리케이션 내에서 사용자 생성 콘텐츠를 조정하는 데 중점을 둡니다. Claude와의 상호작용을 조정하는 지침을 찾고 있다면 [가드레일 가이드](https://platform.claude.com/docs/ko/test-and-evaluate/strengthen-guardrails/reduce-hallucinations) 를 참조하세요.
...
### 의미적 이해와 빠른 결정을 모두 원하는 경우

### 일관된 정책 결정이 필요한 경우

### 조정 정책이 시간이 지남에 따라 변경되거나 발전할 가능성이 있는 경우

### 조정 결정에 대한 해석 가능한 추론이 필요한 경우

### 별도의 모델을 유지하지 않고 다국어 지원이 필요한 경우

### 멀티모달 지원이 필요한 경우

Anthropic은 모든 Claude 모델을 정직하고 도움이 되며 무해하도록 훈련했습니다. 이로 인해 사용된 프롬프트에 관계없이 Claude가 특히 위험하다고 간주되는 콘텐츠( [허용 가능한 사용 정책](https://www.anthropic.com/legal/aup) 에 따라)를 조정할 수 있습니다. 예를 들어, 사용자가 노골적인 성적 콘텐츠를 게시할 수 있도록 허용하려는 성인 웹사이트는 프롬프트에서 노골적인 성적 콘텐츠를 조정하지 않도록 지정하더라도 Claude가 여전히 노골적인 콘텐츠를 조정이 필요한 것으로 플래그를 지정할 수 있습니다. 조정 솔루션을 구축하기 전에 AUP를 미리 검토하는 것을 권장합니다.
...
실제 비용은 이러한 추정치와 다를 수 있습니다. 이러한 추정치는 [배치 처리](https://platform.claude.com/docs/ko/about-claude/use-case-guides/content-moderation#consider-batch-processing) 섹션에서 강조된 프롬프트를 기반으로 합니다. 응답에서 `explanation` 필드를 제거하여 출력 토큰을 더욱 줄일 수 있습니다.
...
복잡한 시나리오에서는 표준 [프롬프트 엔지니어링 기법](https://platform.claude.com/docs/ko/build-with-claude/prompt-engineering/overview) 을 넘어서는 추가 전략을 고려하여 성능을 향상시키는 것이 도움이 될 수 있습니다. 다음은 몇 가지 고급 전략입니다:
...
특정 요구 사항에 대한 최적의 배치 크기를 찾는 것은 약간의 실험이 필요할 수 있다는 점을 염두에 두세요. 더 큰 배치 크기는 비용을 낮출 수 있지만 품질이 약간 감소할 수도 있습니다. 또한 더 긴 응답을 수용하기 위해 Claude API 호출에서 `max_tokens` 매개변수를 늘려야 할 수도 있습니다. 선택한 모델이 출력할 수 있는 최대 토큰 수에 대한 자세한 내용은 [모델 비교 페이지](https://platform.claude.com/docs/ko/about-claude/models#model-comparison-table) 를 참조하세요.

[콘텐츠 조정 쿡북\\
\\
Claude를 콘텐츠 조정에 사용하는 방법에 대한 완전히 구현된 코드 기반 예제를 확인하세요.](https://github.com/anthropics/anthropic-cookbook/blob/main/misc/building%5Fmoderation%5Ffilter.ipynb) [가드레일 가이드\\
\\
Claude와의 상호작용을 조정하는 기법에 대한 가드레일 가이드를 탐색하세요.](https://platform.claude.com/docs/ko/test-and-evaluate/strengthen-guardrails/reduce-hallucinations)

- [Claude로 구축하기 전에](https://platform.claude.com/docs/ko/about-claude/use-case-guides/content-moderation#claude)
- [Claude를 콘텐츠 조정에 사용할지 결정하기](https://platform.claude.com/docs/ko/about-claude/use-case-guides/content-moderation#claude-2)
- [Claude를 사용하여 콘텐츠를 조정하는 방법](https://platform.claude.com/docs/ko/about-claude/use-case-guides/content-moderation#claude-3)
- [적절한 Claude 모델 선택](https://platform.claude.com/docs/ko/about-claude/use-case-guides/content-moderation#claude-4)
