---
url: "https://platform.claude.com/docs/zh-TW/about-claude/models/whats-new-claude-4-5"
title: "Claude 4.5 的新功能 - Claude Docs"
---
...
[Log in](https://platform.claude.com/login?returnTo=%2Fdocs%2Fzh-TW%2Fabout-claude%2Fmodels%2Fwhats-new-claude-4-5)
...
Claude Opus 4.5 是唯一支持 [努力參數](https://platform.claude.com/docs/zh-TW/build-with-claude/effort) 的模型，允許您控制 Claude 在響應時使用多少 token。這使您能夠在單個模型中權衡響應的詳盡程度和 token 效率。
...
Claude Opus 4.5 引入了 [增強的計算機使用功能](https://platform.claude.com/docs/zh-TW/agents-and-tools/tool-use/computer-use-tool)，配備新的縮放操作，可以以全分辨率詳細檢查特定屏幕區域。這使 Claude 能夠檢查細粒度的 UI 元素、小文本和詳細的視覺信息，這些信息在標準屏幕截圖中可能不清楚。
...
Claude Opus 4.5 以 [更親民的價格點](https://platform.claude.com/docs/zh-TW/about-claude/pricing) 提供旗艦級智能，相比之前的 Opus 模型，使先進的 AI 功能可用於更廣泛的應用和用例。

### 思考塊保留

Claude Opus 4.5 [自動保留整個對話中的所有先前思考塊](https://platform.claude.com/docs/zh-TW/build-with-claude/extended-thinking#thinking-block-preservation-in-claude-opus-4-5)，在擴展的多輪交互和工具使用會話中保持推理連續性。這確保 Claude 在處理複雜的長期運行任務時能夠有效地利用其完整的推理歷史。
...
當 [擴展思考](https://platform.claude.com/docs/zh-TW/build-with-claude/extended-thinking) 啟用時，Claude Sonnet 4.5 在編碼任務上的表現明顯更好。擴展思考默認禁用，但我們建議為複雜編碼工作啟用它。請注意，擴展思考會影響 [提示緩存效率](https://platform.claude.com/docs/zh-TW/build-with-claude/prompt-caching#caching-with-thinking-blocks)。有關配置詳情，請參閱 [遷移指南](https://platform.claude.com/docs/zh-TW/about-claude/models/migrating-to-claude-4#extended-thinking-recommendations)。
...
- **上下文感知**：Claude 現在在整個對話中跟蹤其 token 使用情況，在每次工具調用後接收更新。這種感知有助於防止過早放棄任務，並在長期運行的任務上實現更有效的執行。有關技術詳情，請參閱 [上下文感知](https://platform.claude.com/docs/zh-TW/build-with-claude/context-windows#context-awareness-in-claude-sonnet-4-5)，以及 [提示指導](https://platform.claude.com/docs/zh-TW/build-with-claude/prompt-engineering/claude-4-best-practices#context-awareness-and-multi-window-workflows)。
...
上下文感知在 Claude Sonnet 4、Sonnet 4.5、Haiku 4.5、Opus 4、Opus 4.1 和 Opus 4.5 中可用。

### 通信和交互風格

Claude Sonnet 4.5 具有精煉的通信方式，簡潔、直接且自然。它提供基於事實的進度更新，並可能在工具調用後跳過冗長的摘要以保持工作流動力（儘管這可以通過提示調整）。

有關使用此通信風格的詳細指導，請參閱 [Claude 4 最佳實踐](https://platform.claude.com/docs/zh-TW/build-with-claude/prompt-engineering/claude-4-best-practices)。
...
必須通過向 API 請求添加 `thinking` 參數來顯式啟用擴展思考。有關實現詳情，請參閱 [擴展思考文檔](https://platform.claude.com/docs/zh-TW/build-with-claude/extended-thinking)。

當 [擴展思考](https://platform.claude.com/docs/zh-TW/build-with-claude/extended-thinking) 啟用時，Claude Haiku 4.5 在編碼和推理任務上的表現明顯更好。擴展思考默認禁用，但我們建議為複雜問題解決、編碼工作和多步推理啟用它。請注意，擴展思考會影響 [提示緩存效率](https://platform.claude.com/docs/zh-TW/build-with-claude/prompt-caching#caching-with-thinking-blocks)。有關配置詳情，請參閱 [遷移指南](https://platform.claude.com/docs/zh-TW/about-claude/models/migrating-to-claude-4#extended-thinking-recommendations)。

在 Claude Sonnet 3.7、Sonnet 4、Sonnet 4.5、Haiku 4.5、Opus 4、Opus 4.1 和 Opus 4.5 中可用。
...
這是第一個具有原生上下文感知功能的 Haiku 模型。有關提示指導，請參閱 [Claude 4 最佳實踐](https://platform.claude.com/docs/zh-TW/build-with-claude/prompt-engineering/claude-4-best-practices#context-awareness-and-multi-window-workflows)。

在 Claude Sonnet 4、Sonnet 4.5、Haiku 4.5、Opus 4、Opus 4.1 和 Opus 4.5 中可用。
...
[程序化工具調用](https://platform.claude.com/docs/zh-TW/agents-and-tools/tool-use/programmatic-tool-calling) 允許 Claude 在代碼執行容器中以程序方式編寫調用您的工具的代碼，而不是需要為每次工具調用往返模型。這大大降低了多工具工作流的延遲，並通過允許 Claude 在數據到達模型的上下文窗口之前過濾或處理數據來減少 token 消耗。
...
在 Claude Opus 4.5 和 Claude Sonnet 4.5 中可用。需要 [測試版標頭](https://platform.claude.com/docs/zh-TW/api/beta-headers)：`advanced-tool-use-2025-11-20`

### 工具搜索工具（測試版）

[工具搜索工具](https://platform.claude.com/docs/zh-TW/agents-and-tools/tool-use/tool-search-tool) 使 Claude 能夠通過動態發現和按需加載工具來處理數百或數千個工具。Claude 搜索您的工具目錄並僅加載它需要的工具，而不是將所有工具定義預先加載到上下文窗口中。
...
[努力參數](https://platform.claude.com/docs/zh-TW/build-with-claude/effort) 允許您控制 Claude 在響應時使用多少 token，在響應詳盡程度和 token 效率之間進行權衡：
...
僅在 Claude Opus 4.5 中可用。需要 [測試版標頭](https://platform.claude.com/docs/zh-TW/api/beta-headers)：`effort-2025-11-24`

### 工具使用示例（測試版）

[工具使用示例](https://platform.claude.com/docs/zh-TW/agents-and-tools/tool-use/implement-tool-use#providing-tool-use-examples) 允許您提供有效工具輸入的具體示例，以幫助 Claude 更有效地理解如何使用您的工具。這對於具有嵌套對象、可選參數或格式敏感輸入的複雜工具特別有用。
...
在 Claude Sonnet 4.5、Haiku 4.5、Opus 4.5、Opus 4.1 和 Opus 4 中可用。需要 [測試版標頭](https://platform.claude.com/docs/zh-TW/api/beta-headers)：`advanced-tool-use-2025-11-20`。

### 記憶工具（測試版）

新的 [記憶工具](https://platform.claude.com/docs/zh-TW/agents-and-tools/tool-use/memory-tool) 使 Claude 能夠在上下文窗口外存儲和檢索信息：
...
在 Claude Sonnet 4、Sonnet 4.5、Haiku 4.5、Opus 4、Opus 4.1 和 Opus 4.5 中可用。需要 [測試版標頭](https://platform.claude.com/docs/zh-TW/api/beta-headers)：`context-management-2025-06-27`

### 上下文編輯

使用 [上下文編輯](https://platform.claude.com/docs/zh-TW/build-with-claude/context-editing) 通過自動工具調用清除進行智能上下文管理：
...
使用 Claude 4 模型時，您應該更新應用程序以 [處理 `refusal` 停止原因](https://platform.claude.com/docs/zh-TW/test-and-evaluate/strengthen-guardrails/handle-streaming-refusals)。
...
有關更多信息，請參閱 [擴展思考文檔](https://platform.claude.com/docs/zh-TW/build-with-claude/extended-thinking#summarized-thinking)。

### 交錯思考

Claude 4 模型支持將工具使用與擴展思考交錯，允許更自然的對話，其中工具使用和響應可以與常規消息混合。

交錯思考處於測試版。要啟用交錯思考，請向 API 請求添加 [測試版標頭](https://platform.claude.com/docs/zh-TW/api/beta-headers)`interleaved-thinking-2025-05-14`。

有關更多信息，請參閱 [擴展思考文檔](https://platform.claude.com/docs/zh-TW/build-with-claude/extended-thinking#interleaved-thinking)。
...
有關使用這些模型的全面指導，請參閱 [Claude 4 提示工程最佳實踐](https://platform.claude.com/docs/zh-TW/build-with-claude/prompt-engineering/claude-4-best-practices)。
...
有關更多信息，請參閱 [文本編輯器工具文檔](https://platform.claude.com/docs/zh-TW/agents-and-tools/tool-use/text-editor-tool)。
...
有關遷移說明，請參閱 [代碼執行工具文檔](https://platform.claude.com/docs/zh-TW/agents-and-tools/tool-use/code-execution-tool#upgrade-to-latest-tool-version)。
...
有關更多詳情，請參閱 [定價文檔](https://platform.claude.com/docs/zh-TW/about-claude/pricing)。
...
- [AWS Bedrock 全球與區域端點](https://platform.claude.com/docs/zh-TW/build-with-claude/claude-on-amazon-bedrock#global-vs-regional-endpoints)
- [Google Vertex AI 全球與區域端點](https://platform.claude.com/docs/zh-TW/build-with-claude/claude-on-vertex-ai#global-vs-regional-endpoints)
...
破壞性變化和遷移要求因您升級的模型而異。有關詳細的遷移說明，包括分步指南、破壞性變化和遷移檢查清單，請參閱 [遷移到 Claude 4.5](https://platform.claude.com/docs/zh-TW/about-claude/models/migrating-to-claude-4)。
...
[最佳實踐\\
\\
學習 Claude 4.5 模型的提示工程技術](https://platform.claude.com/docs/zh-TW/build-with-claude/prompt-engineering/claude-4-best-practices) [模型概述\\
\\
將 Claude 4.5 模型與其他 Claude 模型進行比較](https://platform.claude.com/docs/zh-TW/about-claude/models/overview) [遷移指南\\
\\
從之前的模型升級](https://platform.claude.com/docs/zh-TW/about-claude/models/migrating-to-claude-4)

- [Opus 4.5 相比 Opus 4.1 的主要改進](https://platform.claude.com/docs/zh-TW/about-claude/models/whats-new-claude-4-5#opus-4-5-opus-4-1)
- [Sonnet 4.5 相比 Sonnet 4 的主要改進](https://platform.claude.com/docs/zh-TW/about-claude/models/whats-new-claude-4-5#sonnet-4-5-sonnet-4)
- [Haiku 4.5 相比 Haiku 3.5 的主要改進](https://platform.claude.com/docs/zh-TW/about-claude/models/whats-new-claude-4-5#haiku-4-5-haiku-3-5)
- [新 API 功能](https://platform.claude.com/docs/zh-TW/about-claude/models/whats-new-claude-4-5#api)
- [Token 計數優化](https://platform.claude.com/docs/zh-TW/about-claude/models/whats-new-claude-4-5#token)
- [Claude 4 中引入的功能](https://platform.claude.com/docs/zh-TW/about-claude/models/whats-new-claude-4-5#claude-4)
