---
url: "https://platform.claude.com/docs/ru/build-with-claude/prompt-engineering/system-prompts"
title: "Предоставление Claude роли с помощью системного промпта - Claude Docs"
---
...
[Log in](https://platform.claude.com/login?returnTo=%2Fdocs%2Fru%2Fbuild-with-claude%2Fprompt-engineering%2Fsystem-prompts)
...
**Советы по системным промптам**: Используйте параметр `system` для установки роли Claude. Поместите всё остальное, например инструкции для конкретной задачи, в `user` ход вместо этого.
...
Используйте параметр `system` в [Messages API](https://platform.claude.com/docs/ru/api/messages) для установки роли Claude:
...
**Совет по role prompting**: Экспериментируйте с ролями! `data scientist` может увидеть другие закономерности, чем `marketing strategist` для одних и тех же данных. `data scientist specializing in customer insight analysis for Fortune 500 companies` может дать ещё другие результаты!
...
### Анализ юридического контракта без role prompting

С ролью Claude выявляет критические проблемы, которые могут стоить миллионы:

### Анализ юридического контракта с role prompting

### Пример 2: Финансовый анализ

Без роли анализ Claude недостаточно глубок:

### Финансовый анализ без role prompting

С ролью Claude предоставляет практические рекомендации:

### Финансовый анализ с role prompting

* * *

[Библиотека промптов\\
\\
Вдохновитесь тщательно отобранной коллекцией промптов для различных задач и вариантов использования.](https://platform.claude.com/docs/ru/resources/prompt-library/library) [Учебник по промптингу на GitHub\\
\\
Полный примерами учебник, охватывающий концепции инженерии промптов, найденные в нашей документации.](https://github.com/anthropics/prompt-eng-interactive-tutorial) [Учебник по промптингу в Google Sheets\\
\\
Более лёгкая версия нашего учебника по инженерии промптов через интерактивную электронную таблицу.](https://docs.google.com/spreadsheets/d/19jzLgRruG9kjUQNKtCg1ZjdD6l6weA6qRXG5zLIAhC8)

- [Почему использовать role prompting?](https://platform.claude.com/docs/ru/build-with-claude/prompt-engineering/system-prompts#pochemu-ispolzovat-role-prompting)
- [Как предоставить Claude роль](https://platform.claude.com/docs/ru/build-with-claude/prompt-engineering/system-prompts#kak-predostavit-claude-rol)
- [Примеры](https://platform.claude.com/docs/ru/build-with-claude/prompt-engineering/system-prompts#primery)
- [Пример 1: Анализ юридического контракта](https://platform.claude.com/docs/ru/build-with-claude/prompt-engineering/system-prompts#primer-1-analiz-yuridicheskogo-kontrakta)
- [Пример 2: Финансовый анализ](https://platform.claude.com/docs/ru/build-with-claude/prompt-engineering/system-prompts#primer-2-finansoviy-analiz)
