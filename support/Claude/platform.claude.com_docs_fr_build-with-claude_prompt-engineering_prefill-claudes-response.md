---
url: "https://platform.claude.com/docs/fr/build-with-claude/prompt-engineering/prefill-claudes-response"
title: "Préremplissez la réponse de Claude pour un meilleur contrôle de la sortie - Claude Docs"
---
...
[Log in](https://platform.claude.com/login?returnTo=%2Fdocs%2Ffr%2Fbuild-with-claude%2Fprompt-engineering%2Fprefill-claudes-response)
...
**Conseil pour les utilisateurs avancés** : Le préremplissage de `{` force Claude à ignorer le préambule et à produire directement l'objet JSON. C'est plus propre, plus concis et plus facile pour les programmes à analyser sans traitement supplémentaire.
Pour une sortie JSON garantie qui se conforme à un schéma spécifique, envisagez d'utiliser [Structured Outputs](https://platform.claude.com/docs/fr/build-with-claude/structured-outputs) au lieu du préremplissage. Les sorties structurées garantissent que la réponse de Claude correspond toujours à votre schéma JSON défini, ce qui la rend idéale pour les applications de production qui nécessitent une conformité stricte du format.

### Exemple : Extraction de données structurées sans préremplissage

### Exemple : Extraction de données structurées avec préremplissage

#### Exemple 2 : Maintenir le personnage dans les scénarios de jeu de rôle

**Conseil de jeu de rôle** : Le préremplissage d'un `[ROLE_NAME]` entre crochets peut rappeler à Claude de rester dans le rôle, même pour les conversations plus longues et plus complexes. C'est particulièrement puissant lorsqu'il est combiné avec le rôle prompting dans le paramètre `system`.

### Exemple : Maintenir le personnage sans rôle prompting

### Exemple : Maintenir le personnage avec rôle prompting

* * *

[Travailler avec les Messages\\
\\
Voir plus d'exemples de préremplissage et d'autres modèles d'API Messages.](https://platform.claude.com/docs/fr/build-with-claude/working-with-messages) [Bibliothèque de prompts\\
\\
Inspirez-vous d'une sélection organisée de prompts pour diverses tâches et cas d'usage.](https://platform.claude.com/docs/fr/resources/prompt-library/library) [Tutoriel de prompting GitHub\\
\\
Un tutoriel rempli d'exemples qui couvre les concepts d'ingénierie des prompts trouvés dans notre documentation.](https://github.com/anthropics/prompt-eng-interactive-tutorial) [Tutoriel de prompting Google Sheets\\
\\
Une version plus légère de notre tutoriel d'ingénierie des prompts via une feuille de calcul interactive.](https://docs.google.com/spreadsheets/d/19jzLgRruG9kjUQNKtCg1ZjdD6l6weA6qRXG5zLIAhC8)

- [Comment préremplir la réponse de Claude](https://platform.claude.com/docs/fr/build-with-claude/prompt-engineering/prefill-claudes-response#comment-preremplir-la-reponse-de-claude)
- [Exemples](https://platform.claude.com/docs/fr/build-with-claude/prompt-engineering/prefill-claudes-response#exemples)
