...
title: "Membangun dengan pemikiran yang diperluas - Claude Docs"
---
...
[Log in](https://platform.claude.com/login?returnTo=%2Fdocs%2Fid%2Fbuild-with-claude%2Fextended-thinking)
...
- Claude Sonnet 3.7 (`claude-3-7-sonnet-20250219`) ( [tidak direkomendasikan lagi](https://platform.claude.com/docs/id/about-claude/model-deprecations))
...
Untuk informasi lebih lanjut, lihat [Perbedaan dalam pemikiran di seluruh versi model](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#differences-in-thinking-across-model-versions).
...
Untuk informasi lebih lanjut tentang format respons pemikiran yang diperluas, lihat [Referensi API Pesan](https://platform.claude.com/docs/id/api/messages).
...
Parameter `budget_tokens` menentukan jumlah maksimum token yang diizinkan Claude gunakan untuk proses penalaran internalnya. Dalam model Claude 4, batas ini berlaku untuk token pemikiran penuh, dan bukan untuk [keluaran ringkasan](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#summarized-thinking). Anggaran yang lebih besar dapat meningkatkan kualitas respons dengan memungkinkan analisis yang lebih menyeluruh untuk masalah kompleks, meskipun Claude mungkin tidak menggunakan seluruh anggaran yang dialokasikan, terutama pada rentang di atas 32k.

`budget_tokens` harus diatur ke nilai yang kurang dari `max_tokens`. Namun, ketika menggunakan [pemikiran yang disisipi dengan alat](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#interleaved-thinking), Anda dapat melampaui batas ini karena batas token menjadi seluruh jendela konteks Anda (200k token).
...
Dalam kasus langka di mana Anda memerlukan akses ke keluaran pemikiran penuh untuk model Claude 4, [hubungi tim penjualan kami](https://platform.claude.com/cdn-cgi/l/email-protection#1764767b7264577679637f6578677e743974787a).
...
Untuk dokumentasi lebih lanjut tentang streaming melalui API Pesan, lihat [Pesan Streaming](https://platform.claude.com/docs/id/build-with-claude/streaming).
...
[Coba di Konsol](https://platform.claude.com/workbench/new?user=Berapa+27+*+453%3F&thinking.budget_tokens=16000)
...
Pemikiran yang diperluas dapat digunakan bersama dengan [penggunaan alat](https://platform.claude.com/docs/id/agents-and-tools/tool-use/overview), memungkinkan Claude untuk bernalar melalui pemilihan alat dan pemrosesan hasil.
...
Mengalihkan mode pemikiran juga membatalkan caching prompt untuk riwayat pesan. Untuk detail lebih lanjut, lihat bagian [Pemikiran yang diperluas dengan caching prompt](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#extended-thinking-with-prompt-caching).

### Contoh: Meneruskan blok pemikiran dengan hasil alat
...
Ketika mengalihkan mode pemikiran selama percakapan, ingat bahwa seluruh giliran asisten (termasuk loop penggunaan alat) harus beroperasi dalam mode pemikiran tunggal. Untuk detail lebih lanjut, lihat [Mengalihkan mode pemikiran dalam percakapan](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#toggling-thinking-modes-in-conversations).
...
Untuk mengaktifkan pemikiran yang tersisip, tambahkan [header beta](https://platform.claude.com/docs/id/api/beta-headers)`interleaved-thinking-2025-05-14` ke permintaan API Anda.
...
- Pemikiran yang tersisip hanya didukung untuk [alat yang digunakan melalui Messages API](https://platform.claude.com/docs/id/agents-and-tools/tool-use/overview).
...
- Di platform pihak ketiga (misalnya, [Amazon Bedrock](https://platform.claude.com/docs/id/build-with-claude/claude-on-amazon-bedrock) dan [Vertex AI](https://platform.claude.com/docs/id/build-with-claude/claude-on-vertex-ai)), jika Anda melewatkan `interleaved-thinking-2025-05-14` ke model apa pun selain Claude Opus 4.5, Claude Opus 4.1, Opus 4, atau Sonnet 4, permintaan Anda akan gagal.

### Penggunaan alat tanpa pemikiran yang tersisip

### Penggunaan alat dengan pemikiran yang tersisip

## Pemikiran yang diperluas dengan penyimpanan prompt

[Penyimpanan prompt](https://platform.claude.com/docs/id/build-with-claude/prompt-caching) dengan pemikiran memiliki beberapa pertimbangan penting:

Tugas pemikiran yang diperluas sering kali memakan waktu lebih dari 5 menit untuk diselesaikan. Pertimbangkan menggunakan [durasi cache 1 jam](https://platform.claude.com/docs/id/build-with-claude/prompt-caching#1-hour-cache-duration) untuk mempertahankan cache hit di seluruh sesi pemikiran yang lebih lama dan alur kerja multi-langkah.
...
- [Pemikiran yang tersisip](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#interleaved-thinking) memperkuat invalidasi cache, karena blok pemikiran dapat terjadi di antara beberapa [panggilan alat](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#extended-thinking-with-tool-use)
- Prompt sistem dan alat tetap disimpan dalam cache meskipun ada perubahan parameter pemikiran atau penghapusan blok

Meskipun blok pemikiran dihapus untuk penyimpanan cache dan perhitungan konteks, mereka harus dipertahankan saat melanjutkan percakapan dengan [penggunaan alat](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#extended-thinking-with-tool-use), terutama dengan [pemikiran yang tersisip](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#interleaved-thinking).
...
### Penyimpanan pesan (dibatalkan saat pemikiran berubah)
...
Anda dapat membaca [panduan kami tentang jendela konteks](https://platform.claude.com/docs/id/build-with-claude/context-windows) untuk penyelaman yang lebih mendalam.
...
![Diagram jendela konteks dengan pemikiran yang diperluas](https://platform.claude.com/docs/images/context-window-thinking.svg)
...
Kami merekomendasikan menggunakan [API penghitungan token](https://platform.claude.com/docs/id/build-with-claude/token-counting) untuk mendapatkan penghitungan token yang akurat untuk kasus penggunaan spesifik Anda, terutama saat bekerja dengan percakapan multi-giliran yang mencakup pemikiran.
...
![Diagram jendela konteks dengan pemikiran yang diperluas dan penggunaan alat](https://platform.claude.com/docs/images/context-window-thinking-tools.svg)
...
- Berpotensi menggunakan [endpoint penghitungan token](https://platform.claude.com/docs/id/build-with-claude/token-counting) lebih sering
...
Hanya benar-benar perlu mengirim kembali blok pemikiran saat menggunakan [alat dengan pemikiran yang diperluas](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#extended-thinking-with-tool-use). Jika tidak, Anda dapat menghilangkan blok pemikiran dari giliran sebelumnya, atau membiarkan API menghapusnya untuk Anda jika Anda melewatkannya kembali.
...
- Saat [streaming respons](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#streaming-thinking), tanda tangan ditambahkan melalui `signature_delta` di dalam acara `content_block_delta` tepat sebelum acara `content_block_stop`.
...
- Nilai `signature` kompatibel di seluruh platform (Claude API, [Amazon Bedrock](https://platform.claude.com/docs/id/build-with-claude/claude-on-amazon-bedrock), dan [Vertex AI](https://platform.claude.com/docs/id/build-with-claude/claude-on-vertex-ai)). Nilai yang dihasilkan di satu platform akan kompatibel dengan platform lain.
...
Untuk model sebelumnya (Claude Sonnet 4.5, Opus 4.1, dll.), blok pemikiran dari putaran sebelumnya terus dihapus dari konteks. Perilaku yang ada yang dijelaskan dalam bagian [Extended thinking with prompt caching](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#extended-thinking-with-prompt-caching) berlaku untuk model tersebut.

## Harga

Untuk informasi harga lengkap termasuk tarif dasar, penulisan cache, cache hits, dan token output, lihat [halaman harga](https://platform.claude.com/docs/id/about-claude/pricing).
...
- Pemikiran tidak kompatibel dengan modifikasi `temperature` atau `top_k` serta [forced tool use](https://platform.claude.com/docs/id/agents-and-tools/tool-use/implement-tool-use#forcing-tool-use).
...
- **Prompt engineering:** Tinjau [extended thinking prompting tips](https://platform.claude.com/docs/id/build-with-claude/prompt-engineering/extended-thinking-tips) kami jika Anda ingin memaksimalkan kemampuan pemikiran Claude.

## Langkah berikutnya

[Coba buku resep pemikiran yang diperluas\\
\\
Jelajahi contoh praktis pemikiran dalam buku resep kami.](https://github.com/anthropics/anthropic-cookbook/tree/main/extended_thinking) [Extended thinking prompting tips\\
\\
Pelajari praktik terbaik prompt engineering untuk pemikiran yang diperluas.](https://platform.claude.com/docs/id/build-with-claude/prompt-engineering/extended-thinking-tips)

- [Model yang didukung](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#model-yang-didukung)
- [Cara kerja pemikiran yang diperluas](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#cara-kerja-pemikiran-yang-diperluas)
- [Cara menggunakan pemikiran yang diperluas](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#cara-menggunakan-pemikiran-yang-diperluas)
- [Pemikiran ringkasan](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#pemikiran-ringkasan)
- [Pemikiran streaming](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#pemikiran-streaming)
- [Pemikiran yang diperluas dengan penggunaan alat](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#pemikiran-yang-diperluas-dengan-penggunaan-alat)
- [Mengalihkan mode pemikiran dalam percakapan](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#mengalihkan-mode-pemikiran-dalam-percakapan)
- [Mempertahankan blok pemikiran](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#mempertahankan-blok-pemikiran)
- [Pemikiran yang tersisip](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#pemikiran-yang-tersisip)
- [Pemikiran yang diperluas dengan penyimpanan prompt](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#pemikiran-yang-diperluas-dengan-penyimpanan-prompt)
- [Memahami perilaku penyimpanan blok pemikiran](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#memahami-perilaku-penyimpanan-blok-pemikiran)
- [Token maksimal dan ukuran jendela konteks dengan pemikiran yang diperluas](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#token-maksimal-dan-ukuran-jendela-konteks-dengan-pemikiran-yang-diperluas)
- [Jendela konteks dengan pemikiran yang diperluas](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#jendela-konteks-dengan-pemikiran-yang-diperluas)
- [Jendela konteks dengan pemikiran yang diperluas dan penggunaan alat](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#jendela-konteks-dengan-pemikiran-yang-diperluas-dan-penggunaan-alat)
- [Mengelola token dengan pemikiran yang diperluas](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#mengelola-token-dengan-pemikiran-yang-diperluas)
- [Enkripsi pemikiran](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#enkripsi-pemikiran)
- [Redaksi pemikiran](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#redaksi-pemikiran)
- [Perbedaan pemikiran di seluruh versi model](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#perbedaan-pemikiran-di-seluruh-versi-model)
- [Preservasi blok pemikiran di Claude Opus 4.5](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#preservasi-blok-pemikiran-di-claude-opus-4-5)
- [Harga](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#harga)
- [Praktik terbaik dan pertimbangan untuk pemikiran yang diperluas](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#praktik-terbaik-dan-pertimbangan-untuk-pemikiran-yang-diperluas)
- [Bekerja dengan anggaran pemikiran](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#bekerja-dengan-anggaran-pemikiran)
- [Pertimbangan kinerja](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#pertimbangan-kinerja)
- [Kompatibilitas fitur](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#kompatibilitas-fitur)
- [Panduan penggunaan](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#panduan-penggunaan)
- [Langkah berikutnya](https://platform.claude.com/docs/id/build-with-claude/extended-thinking#langkah-berikutnya)
