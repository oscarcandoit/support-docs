---
url: "https://platform.claude.com/docs/zh-TW/about-claude/use-case-guides/legal-summarization"
...
[Log in](https://platform.claude.com/login?returnTo=%2Fdocs%2Fzh-TW%2Fabout-claude%2Fuse-case-guides%2Flegal-summarization)
...
有關更多資訊，請參閱我們的 [建立成功標準](https://platform.claude.com/docs/zh-TW/test-and-evaluate/define-success) 指南。
...
實際成本可能與這些估計不同。這些估計基於 [提示](https://platform.claude.com/docs/zh-TW/about-claude/use-case-guides/legal-summarization#build-a-strong-prompt) 部分中突出顯示的範例。
...
提示通常需要測試和優化才能準備好用於生產。為了確定解決方案的準備情況，請使用結合定量和定性方法的系統流程評估摘要的品質。根據您定義的成功標準建立 [強大的經驗評估](https://platform.claude.com/docs/zh-TW/build-with-claude/develop-tests#building-evals-and-test-cases) 將允許您優化提示。以下是您可能希望在經驗評估中包含的一些指標：
...
3. **並行化對 Claude 的 API 呼叫**：具有大量代幣的長文件可能需要長達一分鐘的時間才能讓 Claude 生成摘要。對於大型文件集合，您可能希望並行向 Claude 發送 API 呼叫，以便摘要可以在合理的時間範圍內完成。請參閱 Anthropic 的 [速率限制](https://platform.claude.com/docs/zh-TW/api/rate-limits#rate-limits) 以確定可以並行執行的最大 API 呼叫數。
...
在複雜的情況下，除了標準 [提示工程技術](https://platform.claude.com/docs/zh-TW/build-with-claude/prompt-engineering/overview) 外，考慮其他策略來改進效能可能會有所幫助。以下是一些進階策略：
...
微調目前僅通過 Amazon Bedrock 提供。其他詳細資訊可在 [AWS 啟動部落格](https://aws.amazon.com/blogs/machine-learning/fine-tune-anthropics-claude-3-haiku-in-amazon-bedrock-to-boost-model-accuracy-and-quality/) 中取得。

[摘要食譜\\
\\
查看如何使用 Claude 總結合約的完整實現代碼範例。](https://github.com/anthropics/anthropic-cookbook/blob/main/skills/summarization/guide.ipynb) [引用食譜\\
\\
探索我們的引用食譜配方，以獲得有關如何確保資訊準確性和可解釋性的指導。](https://github.com/anthropics/anthropic-cookbook/blob/main/misc/using_citations.ipynb)

- [使用 Claude 進行構建前](https://platform.claude.com/docs/zh-TW/about-claude/use-case-guides/legal-summarization#claude)
- [決定是否使用 Claude 進行法律文件摘要](https://platform.claude.com/docs/zh-TW/about-claude/use-case-guides/legal-summarization#claude-2)
- [如何使用 Claude 總結法律文件](https://platform.claude.com/docs/zh-TW/about-claude/use-case-guides/legal-summarization#claude-3)
- [選擇正確的 Claude 模型](https://platform.claude.com/docs/zh-TW/about-claude/use-case-guides/legal-summarization#claude-4)
- [將文件轉換為 Claude 可以處理的格式](https://platform.claude.com/docs/zh-TW/about-claude/use-case-guides/legal-summarization#claude-5)
- [微調 Claude 以從您的數據集中學習](https://platform.claude.com/docs/zh-TW/about-claude/use-case-guides/legal-summarization#claude-6)
