---
url: "https://platform.claude.com/docs/ko/about-claude/models/whats-new-claude-4-5"
title: "Claude 4.5의 새로운 기능 - Claude Docs"
---
...
[Log in](https://platform.claude.com/login?returnTo=%2Fdocs%2Fko%2Fabout-claude%2Fmodels%2Fwhats-new-claude-4-5)
...
Claude Opus 4.5는 [effort 파라미터](https://platform.claude.com/docs/ko/build-with-claude/effort) 를 지원하는 유일한 모델이며, Claude가 응답할 때 사용하는 토큰 수를 제어할 수 있습니다. 이를 통해 단일 모델로 응답의 철저함과 토큰 효율성 사이의 트레이드오프를 조정할 수 있습니다.
...
Claude Opus 4.5는 [향상된 컴퓨터 사용 기능](https://platform.claude.com/docs/ko/agents-and-tools/tool-use/computer-use-tool) 을 도입하며, 특정 화면 영역을 전체 해상도로 자세히 검사할 수 있는 새로운 줌 작업을 제공합니다. 이를 통해 Claude는 표준 스크린샷에서 불명확할 수 있는 세밀한 UI 요소, 작은 텍스트 및 상세한 시각 정보를 검사할 수 있습니다.
...
Claude Opus 4.5는 이전 Opus 모델보다 [더 접근 가능한 가격대](https://platform.claude.com/docs/ko/about-claude/pricing) 에서 플래그십 지능을 제공하므로, 더 광범위한 애플리케이션 및 사용 사례에 고급 AI 기능을 사용할 수 있습니다.

### 사고 블록 보존

Claude Opus 4.5는 [이전의 모든 사고 블록을 자동으로 보존](https://platform.claude.com/docs/ko/build-with-claude/extended-thinking#thinking-block-preservation-in-claude-opus-4-5) 하여 대화 전체에서 추론 연속성을 유지하고 확장된 다중 턴 상호 작용 및 도구 사용 세션 전반에 걸쳐 추론 연속성을 유지합니다. 이를 통해 Claude는 복잡하고 오래 실행되는 작업에서 작업할 때 전체 추론 기록을 효과적으로 활용할 수 있습니다.
...
Claude Sonnet 4.5는 [확장 사고](https://platform.claude.com/docs/ko/build-with-claude/extended-thinking) 가 활성화되었을 때 코딩 작업에서 훨씬 더 나은 성능을 발휘합니다. 확장 사고는 기본적으로 비활성화되어 있지만, 복잡한 코딩 작업에는 활성화하는 것을 권장합니다. 확장 사고가 [프롬프트 캐싱 효율성](https://platform.claude.com/docs/ko/build-with-claude/prompt-caching#caching-with-thinking-blocks) 에 영향을 미친다는 점에 유의하세요. 구성 세부 사항은 [마이그레이션 가이드](https://platform.claude.com/docs/ko/about-claude/models/migrating-to-claude-4#extended-thinking-recommendations) 를 참조하세요.
...
- **컨텍스트 인식**: Claude는 이제 대화 전체에서 토큰 사용량을 추적하며, 각 도구 호출 후 업데이트를 받습니다. 이러한 인식은 조기 작업 포기를 방지하고 오래 실행되는 작업에서 더 효과적인 실행을 가능하게 합니다. 기술 세부 사항은 [컨텍스트 인식](https://platform.claude.com/docs/ko/build-with-claude/context-windows#context-awareness-in-claude-sonnet-4-5) 을 참조하고, 프롬프팅 지침은 [프롬프트 엔지니어링](https://platform.claude.com/docs/ko/build-with-claude/prompt-engineering/claude-4-best-practices#context-awareness-and-multi-window-workflows) 을 참조하세요.
...
컨텍스트 인식은 Claude Sonnet 4, Sonnet 4.5, Haiku 4.5, Opus 4, Opus 4.1 및 Opus 4.5에서 사용 가능합니다.
...
이 통신 스타일로 작업하기 위한 자세한 지침은 [Claude 4 모범 사례](https://platform.claude.com/docs/ko/build-with-claude/prompt-engineering/claude-4-best-practices) 를 참조하세요.
...
확장 사고는 API 요청에 `thinking` 파라미터를 추가하여 명시적으로 활성화해야 합니다. 구현 세부 사항은 [확장 사고 문서](https://platform.claude.com/docs/ko/build-with-claude/extended-thinking) 를 참조하세요.

Claude Haiku 4.5는 [확장 사고](https://platform.claude.com/docs/ko/build-with-claude/extended-thinking) 가 활성화되었을 때 코딩 및 추론 작업에서 훨씬 더 나은 성능을 발휘합니다. 확장 사고는 기본적으로 비활성화되어 있지만, 복잡한 문제 해결, 코딩 작업 및 다중 단계 추론에는 활성화하는 것을 권장합니다. 확장 사고가 [프롬프트 캐싱 효율성](https://platform.claude.com/docs/ko/build-with-claude/prompt-caching#caching-with-thinking-blocks) 에 영향을 미친다는 점에 유의하세요. 구성 세부 사항은 [마이그레이션 가이드](https://platform.claude.com/docs/ko/about-claude/models/migrating-to-claude-4#extended-thinking-recommendations) 를 참조하세요.

Claude Sonnet 3.7, Sonnet 4, Sonnet 4.5, Haiku 4.5, Opus 4, Opus 4.1 및 Opus 4.5에서 사용 가능합니다.
...
이것은 기본 컨텍스트 인식 기능을 갖춘 첫 번째 Haiku 모델입니다. 프롬프팅 지침은 [Claude 4 모범 사례](https://platform.claude.com/docs/ko/build-with-claude/prompt-engineering/claude-4-best-practices#context-awareness-and-multi-window-workflows) 를 참조하세요.

Claude Sonnet 4, Sonnet 4.5, Haiku 4.5, Opus 4, Opus 4.1 및 Opus 4.5에서 사용 가능합니다.
...
[프로그래밍 방식의 도구 호출](https://platform.claude.com/docs/ko/agents-and-tools/tool-use/programmatic-tool-calling) 을 통해 Claude는 각 도구 호출에 대해 모델을 통한 왕복을 요구하기보다는 코드 실행 컨테이너 내에서 프로그래밍 방식으로 도구를 호출하는 코드를 작성할 수 있습니다. 이는 다중 도구 워크플로우의 지연 시간을 크게 줄이고 Claude가 모델의 컨텍스트 윈도우에 도달하기 전에 데이터를 필터링하거나 처리할 수 있도록 하여 토큰 소비를 감소시킵니다.
...
Claude Opus 4.5 및 Claude Sonnet 4.5에서 사용 가능합니다. [베타 헤더](https://platform.claude.com/docs/ko/api/beta-headers) 필요: `advanced-tool-use-2025-11-20`

### 도구 검색 도구(베타)

[도구 검색 도구](https://platform.claude.com/docs/ko/agents-and-tools/tool-use/tool-search-tool) 를 통해 Claude는 수백 또는 수천 개의 도구로 작업할 수 있으며, 필요에 따라 동적으로 도구를 발견하고 로드할 수 있습니다. 모든 도구 정의를 미리 컨텍스트 윈도우에 로드하는 대신, Claude는 도구 카탈로그를 검색하고 필요한 도구만 로드합니다.
...
[effort 파라미터](https://platform.claude.com/docs/ko/build-with-claude/effort) 를 통해 Claude가 응답할 때 사용하는 토큰 수를 제어하여 응답의 철저함과 토큰 효율성 사이의 트레이드오프를 조정할 수 있습니다:
...
Claude Opus 4.5에서만 사용 가능합니다. [베타 헤더](https://platform.claude.com/docs/ko/api/beta-headers) 필요: `effort-2025-11-24`

### 도구 사용 예제(베타)

[도구 사용 예제](https://platform.claude.com/docs/ko/agents-and-tools/tool-use/implement-tool-use#providing-tool-use-examples) 를 통해 유효한 도구 입력의 구체적인 예제를 제공하여 Claude가 도구를 더 효과적으로 사용하는 방법을 이해하도록 도울 수 있습니다. 이는 중첩된 객체, 선택적 파라미터 또는 형식에 민감한 입력이 있는 복잡한 도구에 특히 유용합니다.
...
Claude Sonnet 4.5, Haiku 4.5, Opus 4.5, Opus 4.1 및 Opus 4에서 사용 가능합니다. [베타 헤더](https://platform.claude.com/docs/ko/api/beta-headers) 필요: `advanced-tool-use-2025-11-20`

### 메모리 도구(베타)

새로운 [메모리 도구](https://platform.claude.com/docs/ko/agents-and-tools/tool-use/memory-tool) 를 통해 Claude는 컨텍스트 윈도우 외부에 정보를 저장하고 검색할 수 있습니다:
...
Claude Sonnet 4, Sonnet 4.5, Haiku 4.5, Opus 4, Opus 4.1 및 Opus 4.5에서 사용 가능합니다. [베타 헤더](https://platform.claude.com/docs/ko/api/beta-headers) 필요: `context-management-2025-06-27`

### 컨텍스트 편집

[컨텍스트 편집](https://platform.claude.com/docs/ko/build-with-claude/context-editing) 을 사용하여 자동 도구 호출 지우기를 통한 지능형 컨텍스트 관리:
...
Claude 4 모델을 사용할 때는 [거부 중지 이유를 처리](https://platform.claude.com/docs/ko/test-and-evaluate/strengthen-guardrails/handle-streaming-refusals) 하도록 애플리케이션을 업데이트해야 합니다.
...
자세한 내용은 [확장 사고 문서](https://platform.claude.com/docs/ko/build-with-claude/extended-thinking#summarized-thinking) 를 참조하세요.

### 인터리브된 사고

Claude 4 모델은 확장 사고와 도구 사용을 인터리브하는 것을 지원하여 도구 사용 및 응답을 일반 메시지와 혼합할 수 있는 더 자연스러운 대화를 가능하게 합니다.

인터리브된 사고는 베타 상태입니다. 인터리브된 사고를 활성화하려면 API 요청에 [베타 헤더](https://platform.claude.com/docs/ko/api/beta-headers)`interleaved-thinking-2025-05-14`를 추가하세요.

자세한 내용은 [확장 사고 문서](https://platform.claude.com/docs/ko/build-with-claude/extended-thinking#interleaved-thinking) 를 참조하세요.
...
이러한 모델로 작업하기 위한 포괄적인 지침은 [Claude 4 프롬프트 엔지니어링 모범 사례](https://platform.claude.com/docs/ko/build-with-claude/prompt-engineering/claude-4-best-practices) 를 참조하세요.
...
자세한 내용은 [텍스트 편집기 도구 문서](https://platform.claude.com/docs/ko/agents-and-tools/tool-use/text-editor-tool) 를 참조하세요.
...
마이그레이션 지침은 [코드 실행 도구 문서](https://platform.claude.com/docs/ko/agents-and-tools/tool-use/code-execution-tool#upgrade-to-latest-tool-version) 를 참조하세요.
...
자세한 내용은 [가격 문서](https://platform.claude.com/docs/ko/about-claude/pricing) 를 참조하세요.
...
- [AWS Bedrock 글로벌 대 지역 엔드포인트](https://platform.claude.com/docs/ko/build-with-claude/claude-on-amazon-bedrock#global-vs-regional-endpoints)
- [Google Vertex AI 글로벌 대 지역 엔드포인트](https://platform.claude.com/docs/ko/build-with-claude/claude-on-vertex-ai#global-vs-regional-endpoints)
...
주요 변경 사항 및 마이그레이션 요구 사항은 업그레이드하는 모델에 따라 다릅니다. 단계별 가이드, 주요 변경 사항 및 마이그레이션 체크리스트를 포함한 자세한 마이그레이션 지침은 [Claude 4.5로 마이그레이션](https://platform.claude.com/docs/ko/about-claude/models/migrating-to-claude-4) 을 참조하세요.
...
[모범 사례\\
\\
Claude 4.5 모델을 위한 프롬프트 엔지니어링 기법 학습](https://platform.claude.com/docs/ko/build-with-claude/prompt-engineering/claude-4-best-practices) [모델 개요\\
\\
Claude 4.5 모델을 다른 Claude 모델과 비교](https://platform.claude.com/docs/ko/about-claude/models/overview) [마이그레이션 가이드\\
\\
이전 모델에서 업그레이드](https://platform.claude.com/docs/ko/about-claude/models/migrating-to-claude-4)

- [Opus 4.5의 Opus 4.1 대비 주요 개선 사항](https://platform.claude.com/docs/ko/about-claude/models/whats-new-claude-4-5#opus-4-5-opus-4-1)
- [Effort 파라미터](https://platform.claude.com/docs/ko/about-claude/models/whats-new-claude-4-5#effort)
- [Sonnet 4.5의 Sonnet 4 대비 주요 개선 사항](https://platform.claude.com/docs/ko/about-claude/models/whats-new-claude-4-5#sonnet-4-5-sonnet-4)
- [Haiku 4.5의 Haiku 3.5 대비 주요 개선 사항](https://platform.claude.com/docs/ko/about-claude/models/whats-new-claude-4-5#haiku-4-5-haiku-3-5)
- [새로운 API 기능](https://platform.claude.com/docs/ko/about-claude/models/whats-new-claude-4-5#api)
- [Effort 파라미터(베타)](https://platform.claude.com/docs/ko/about-claude/models/whats-new-claude-4-5#effort-2)
- [Claude 4에서 도입된 기능](https://platform.claude.com/docs/ko/about-claude/models/whats-new-claude-4-5#claude-4)
- [제3자 플랫폼 가격](https://platform.claude.com/docs/ko/about-claude/models/whats-new-claude-4-5#3)
