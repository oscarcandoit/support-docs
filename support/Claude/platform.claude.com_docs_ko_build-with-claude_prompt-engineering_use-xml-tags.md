---
url: "https://platform.claude.com/docs/ko/build-with-claude/prompt-engineering/use-xml-tags"
title: "XML 태그를 사용하여 프롬프트 구조화하기 - Claude Docs"
---
...
[Log in](https://platform.claude.com/login?returnTo=%2Fdocs%2Fko%2Fbuild-with-claude%2Fprompt-engineering%2Fuse-xml-tags)
...
**XML 팁**: `<instructions>`, `<example>`, `<formatting>`과 같은 태그를 사용하여 프롬프트의 다양한 부분을 명확하게 분리하세요. 이렇게 하면 Claude가 지시사항을 예제나 컨텍스트와 혼동하지 않습니다.
...
Claude가 특별히 학습한 정규적인 "최고의" XML 태그는 없지만, 태그 이름이 주변 정보와 의미 있게 연결되도록 하는 것을 권장합니다.
...
**파워 유저 팁**: XML 태그를 멀티샷 프롬팅(`<examples>`) 또는 사고의 연쇄(`<thinking>`, `<answer>`)와 같은 다른 기법과 결합하세요. 이렇게 하면 매우 구조화되고 고성능의 프롬프트가 만들어집니다.
...
[프롬프트 라이브러리\\
\\
다양한 작업 및 사용 사례에 대한 엄선된 프롬프트 선택으로 영감을 얻으세요.](https://platform.claude.com/docs/ko/resources/prompt-library/library) [GitHub 프롬팅 튜토리얼\\
\\
당사 문서에서 찾을 수 있는 프롬프트 엔지니어링 개념을 다루는 예제가 풍부한 튜토리얼입니다.](https://github.com/anthropics/prompt-eng-interactive-tutorial) [Google Sheets 프롬팅 튜토리얼\\
\\
...
- [XML 태그를 사용하는 이유는 무엇인가요?](https://platform.claude.com/docs/ko/build-with-claude/prompt-engineering/use-xml-tags#xml)
