---
url: "https://docs.digitalocean.com/support/why-cant-my-vpc-native-pods-connect-to-my-droplets/"
title: "Why can't my VPC-native pods connect to my Droplets? | DigitalOcean Documentation"
---
...
[VPC-native networking](https://docs.digitalocean.com/products/kubernetes/details/features/#VPC-Native-networking) allows for communication directly from pods to Droplets and other resources within VPC networks. For this to work, the Droplets need to have the necessary routes enabled.

If your pods in VPC-native clusters are unable to connect to Droplets created on or before 2 October 2024, but can connect with `hostNetwork: true`, you may need to [enable route injection for the Droplets](https://docs.digitalocean.com/products/networking/vpc/how-to/update-peering-routes/).
...
This example VPC contains two Droplets: one with [route injection configured](https://docs.digitalocean.com/products/networking/vpc/how-to/update-peering-routes/), and one without.
...
The following output of `ip route` shows the routing table of `native-vpc-droplet`:

`native-vpc-droplet` routing table
...
The highlighted lines (2, 6, and 7) are the routes configured by route injection. `legacy-vpc-droplet` does not have these routes.
...
If `vpc-native-pod` (`10.150.0.103`) attempts to reach out to `native-vpc-droplet` (`10.2.0.2`) with `curl --connect-timeout 1 native-vpc-droplet`
, the connection succeeds and `native-vpc-droplet` sees an inbound connection from `10.150.0.103` (the pod IP).

Output of `curl` to `native-vpc-droplet` on VPC-native pod
...
However, if the pod attempts to reach `legacy-vpc-droplet` with `curl --connect-timeout 1 legacy-vpc-droplet`
, the connection fails instead:

Output of `curl` to `legacy-vpc-droplet` on VPC-native pod
...
The connection fails because `legacy-vpc-droplet` does not have the necessary routes to handle a connection from `10.150.0.103`. Instead, it sends it via the default gateway (towards the internet, rather than through the VPC).
...
- [Explanation of VPC-Native vs Legacy Cluster Networking](https://docs.digitalocean.com/support/why-cant-my-vpc-native-pods-connect-to-my-droplets/#explanation-of-vpc-native-vs-legacy-cluster-networking)
  - [Networking Example](https://docs.digitalocean.com/support/why-cant-my-vpc-native-pods-connect-to-my-droplets/#networking-example)
