---
url: "https://docs.digitalocean.com/support/i-lost-the-ssh-key-for-my-droplet/"
title: "I lost the SSH key for my Droplet | DigitalOcean Documentation"
---
...
Warning **On FreeBSD, Fedora 28, Fedora Atomic, and RancherOS**, which are operating systems with internally-managed passwords, it’s currently not possible to reset the root password externally. If you lose access in this case, we instead recommend [creating a snapshot](https://docs.digitalocean.com/products/snapshots/how-to/snapshot-droplets/) of that Droplet and redeploying to a new Droplet to which you have access.
...
There are detailed instructions on [how to connect to Droplets with the Recovery Console](https://docs.digitalocean.com/products/droplets/how-to/recovery/recovery-console/#log-in-with-the-console) for a more explicit walk-through, but here’s a brief summary:
...
After you restart the SSH daemon, you can [connect to the Droplet via SSH](https://docs.digitalocean.com/products/droplets/how-to/connect-with-ssh/) as `root` with the newly-created root password. You can also then [create a new SSH key pair and add it to the Droplet](https://docs.digitalocean.com/products/droplets/how-to/add-ssh-keys/). If you use the root user to append a new SSH key to another user’s `authorized_keys` file, ensure the file maintains the proper ownership. [How to Upload an SSH Public Key to an Existing Droplet](https://docs.digitalocean.com/products/droplets/how-to/add-ssh-keys/to-existing-droplet/) provides detailed guidance.
...
- [Reset the Droplet’s Root Password](https://docs.digitalocean.com/support/i-lost-the-ssh-key-for-my-droplet/#reset-the-droplets-root-password)
- [Log in to the Droplet via the Recovery Console](https://docs.digitalocean.com/support/i-lost-the-ssh-key-for-my-droplet/#log-in-to-the-droplet-via-the-recovery-console)
- [Enable Password Authentication](https://docs.digitalocean.com/support/i-lost-the-ssh-key-for-my-droplet/#enable-password-authentication)
